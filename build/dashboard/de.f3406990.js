;
!function() {
  try {
    var e = "undefined" != typeof window ? window : "undefined" != typeof global ? global : "undefined" != typeof self ? self : {}, n = new Error().stack;
    n && (e._sentryDebugIds = e._sentryDebugIds || {}, e._sentryDebugIds[n] = "601ef8b0-727c-4be2-8bd1-81d968623fd9", e._sentryDebugIdIdentifier = "sentry-dbid-601ef8b0-727c-4be2-8bd1-81d968623fd9");
  } catch (e2) {
  }
}();
const PRXpBm = {
  context: "dialog header",
  string: "Bestellung abbrechen"
};
const ll2dE6 = {
  context: "PageTypeDeleteWarningDialog multiple assigned items description",
  string: "Are you sure you want to delete selected page types? If you remove them you won\u2019t be able to assign them to created pages."
};
const qu8b3v = {
  context: "PageTypeDeleteWarningDialog multiple consent label",
  string: "Yes, I want to delete those pages types and assigned pages"
};
const tQxBXs = {
  context: "PageTypeDeleteWarningDialog single assigned items description",
  string: "You are about to delete page type <b>{typeName}</b>. It is assigned to {assignedItemsCount} {assignedItemsCount,plural,one{page} other{pages}}. Deleting this page type will also delete those pages. Are you sure you want to do this?"
};
const RZ32u5 = {
  context: "PageTypeDeleteWarningDialog single consent label",
  string: "Yes, I want to delete this page type and assigned pages"
};
const TnyLrZ = {
  context: "PageTypeDeleteWarningDialog with items multiple description",
  string: "You are about to delete multiple page types. Some of them are assigned to pages. Deleting those page types will also delete those pages"
};
const aPqizA = {
  context: "ProductTypeDeleteWarningDialog multiple assigned items description",
  string: "Are you sure you want to delete selected product types? If you remove them you won\u2019t be able to assign them to created products."
};
const ZFfG4L = {
  context: "ProductTypeDeleteWarningDialog single assigned items description",
  string: "You are about to delete product type <b>{typeName}</b>. It is assigned to {assignedItemsCount} {assignedItemsCount,plural,one{product} other{products}}. Deleting this product type will also delete those products. Are you sure you want to do this?"
};
const bk9KUX = {
  context: "ProductTypeDeleteWarningDialog single consent label",
  string: "Yes, I want to delete this product type and assigned products"
};
const HivFnX = {
  context: "ProductTypeDeleteWarningDialog single no assigned items description",
  string: "Are you sure you want to delete <b>{typeName}</b>? If you remove it you won\u2019t be able to assign it to created products."
};
const nngeI3 = {
  context: "amount title",
  string: "Refunded amount"
};
const xrPv2K = {
  context: "by preposition",
  string: "by"
};
const beuxAP = {
  context: "channel publication status",
  string: "Versteckt"
};
const DIrxt7 = {
  context: "channel publication date",
  string: "Visible since {date}"
};
const nfbabo = {
  context: "channel publication date",
  string: "Will become available on {date}"
};
const mDgOmP = {
  context: "channel publication status",
  string: "Sichtbar"
};
const hpMcW8 = {
  string: "Legen Sie fest, wie Benutzer durch den Shop navigieren"
};
const ivJ1qt = {
  string: "Verwaltung von Berechtigungsgruppen und deren Berechtigungen"
};
const n0RwMK = {
  string: "Produkttypen im Shop verwalten"
};
const zxs6G3 = {
  string: "Verwaltung von Versandoptionen"
};
const RQUkVW = {
  string: "Verwaltung von Mitarbeitern und Berechtigungen"
};
const EIULpW = {
  string: "Einstellungen zur Berechnung von Steuern"
};
const m19JfL = {
  string: "Plugins anzeigen und verwalten."
};
const yJynYK = {
  context: "discount value",
  string: "discount"
};
const ojHyj3 = {
  context: "discount value label",
  string: "Rabatt"
};
const sHON47 = {
  context: "refunded products list title",
  string: "Produkte zur\xFCckerstattet"
};
const L5io1l = {
  context: "returned products list title",
  string: "Products returned"
};
const a1uffz = {
  context: "draft created from replace products list title",
  string: "Products replaced"
};
const AQSmqG = {
  context: "order discount was updated automatically event title",
  string: "Order discount was updated automatically updated"
};
const Zptsep = {
  context: "order was discounted event title",
  string: "Order was discounted by"
};
const nayZY0 = {
  context: "returned event title",
  string: "Products were returned by"
};
const Zhxu58 = {
  context: "Fixed amount subtitle",
  string: "Fixed amount"
};
const T4wa2Y = {
  context: "global config plugin status popup title",
  string: "Global Plugin"
};
const BXkF8Z = {
  context: "header",
  string: "Aktivit\xE4t"
};
const wWTUrM = {
  string: "Keine Aktivit\xE4ten gefunden"
};
const zWgbGg = {
  string: "Heute"
};
const By5ZBp = {
  context: "header",
  string: "Hallo, {userName}"
};
const aCX8rl = {
  context: "subheader",
  string: "Hier finden Sie einige Informationen zu Ihrem Shop"
};
const rr8fyf = {
  context: "header",
  string: "Top Produkte"
};
const Q1Uzbb = {
  string: "Keine Produkte gefunden"
};
const G7mu0y = {
  string: "Die GraphQL API hat Beta-Qualit\xE4t. Sie ist noch nicht vollst\xE4ndig optimiert und es k\xF6nnten Mutations und Queries fehlen."
};
const QBxN6z = {
  context: "is filter range or value",
  string: "zwischen"
};
const QzseV7 = {
  context: "dialog header",
  string: "Men\xFC l\xF6schen"
};
const E54eoT = {
  string: "Creating the navigation structure is done by dragging and dropping. Simply create a new menu item and then drag it into its destined place. You can move items inside one another to create a tree structure and drag items up and down to create a hierarchy"
};
const H3Uirw = {
  context: "create new menu item, header",
  string: "Artikel hinzuf\xFCgen"
};
const fzDI3A = {
  context: "add link to navigation",
  string: "Verlinkung zu: {url}"
};
const KKQUMK = {
  context: "edit menu item, header",
  string: "Artikel bearbeiten"
};
const Urh2N3 = {
  context: "label",
  string: "Link"
};
const Uf3oHA = {
  context: "add new menu item",
  string: "Neuen Men\xFCpunkt erstellen"
};
const dEUZg2 = {
  context: "header",
  string: "Men\xFCpunkte"
};
const WwZfNK = {
  string: "F\xFCge ein neues Produkt hinzu um ein Menu zu erstellen"
};
const ugnggZ = {
  string: "Erstelltes Men\xFC"
};
const bj1U23 = {
  string: "M\xF6chten Sie wirklich das Menu {menuName} l\xF6schen?"
};
const DWs4ba = {
  string: "Keine Men\xFCs gefunden"
};
const JXRYQg = {
  context: "button",
  string: "Men\xFC erstellen"
};
const MTl5o6 = {
  context: "new discount label",
  string: "New discount value"
};
const saKXY3 = {
  context: "product label",
  string: "Nicht ver\xF6ffentlicht"
};
const YI6Fhj = {
  context: "no address is set in draft order",
  string: "Nicht gesetzt"
};
const PX2zWy = {
  context: "customer is not set in draft order",
  string: "Nicht gesetzt"
};
const e7yOai = {
  context: "shipping address is not set in draft order",
  string: "Nicht gesetzt"
};
const BftZHy = {
  context: "window title",
  string: "Create Page Type"
};
const W5SK5c = {
  string: "Select content type"
};
const gz9v22 = {
  context: "PluginChannelConfigurationCell channel title",
  string: "Per channel"
};
const KupNHw = {
  context: "product field",
  string: "Kategorie"
};
const jxoMLL = {
  context: "product field",
  string: "Sammlungen"
};
const YVIajc = {
  context: "product field",
  string: "Beschreibung"
};
const W8i2Ez = {
  context: "product field",
  string: "Name"
};
const QVNg8A = {
  context: "product field",
  string: "Versteuern"
};
const HYHLsB = {
  context: "product field",
  string: "Export Variant ID"
};
const Uo5MoU = {
  context: "product field",
  string: "Bilder der Variante"
};
const XBwpUv = {
  context: "product field",
  string: "Varianten-Gewicht exportieren"
};
const SSWFo8 = {
  context: "window title",
  string: "Produkttyp erstellen"
};
const bq1eEx = {
  context: "header",
  string: "Produkttyp erstellen"
};
const mUb8Gt = {
  context: "section header",
  string: "Steuern"
};
const OgFBAj = {
  context: "input label",
  string: "Preis"
};
const vuKrlW = {
  string: "Lager"
};
const ABgQcF = {
  context: "variant stock, header",
  string: "Lager"
};
const oIMMcO = {
  context: "no warehouses info",
  string: "There are no warehouses set up for your store. You can configure variants without providing stock quantities."
};
const Gjo89T = {
  context: "header",
  string: "Lagerh\xE4user"
};
const D8nsBc = {
  context: "no warehouses info",
  string: "There are no warehouses set up for your store. To add stock quantity to the variant please <a>configure a warehouse</a>"
};
const RLBLPQ = {
  context: "no warehouses info",
  string: "Es wurden f\xFCr diesen Shop noch keine Lager angelegt. Bitte <a>legen Sie ein Lager an</a>, um die verf\xFCgbare Menge des Produktes einzutragen."
};
const qJedl0 = {
  context: "product label",
  string: "Ver\xF6ffentlicht"
};
const ppLwx3 = {
  context: "number of categories",
  string: "Kategorien ({quantity})"
};
const QdGzUf = {
  context: "number of collections",
  string: "Kollektion ({quantity})"
};
const bNw8PM = {
  context: "number of products",
  string: "Produkte ({quantity})"
};
const HVlMK2 = {
  context: "number of variants",
  string: "Variants ({quantity})"
};
const YFQBs1 = {
  context: "product availability date label",
  string: "Set availability date"
};
const mLZMb6 = {
  context: "ChannelsSection select field label",
  string: "Channel"
};
const yOaNWB = {
  context: "delete shipping method",
  string: "Sind Sie sicher, dass Sie {name} l\xF6schen wollen?"
};
const LsgHmZ = {
  context: "delete shipping zone",
  string: "Sind Sie sicher, dass Sie {name} l\xF6schen wollen?"
};
const PV0SQd = {
  context: "WarehousesSection select field label",
  string: "Lagerhaus"
};
const skPoVe = {
  context: "button",
  string: "Akzeptieren"
};
const Y7UlMR = {
  context: "app extensions subsection",
  string: "Apps"
};
const USO8PB = {
  context: "snackbar text",
  string: "App deactivated"
};
const YHNozE = {
  context: "dialog header",
  string: "Apps aktivieren"
};
const Q47ovw = {
  context: "activate app",
  string: "Are you sure you want to activate this app? Activating will start gathering events."
};
const JbUg2b = {
  context: "activate app",
  string: "Are you sure you want to activate {name}? Activating will start gathering events."
};
const D3E2b5 = {
  context: "button label",
  string: "Aktivieren"
};
const yMi8I8 = {
  context: "dialog header",
  string: "App deaktivieren"
};
const Np7J92 = {
  context: "deactivate local named app",
  string: "Are you sure you want to disable {name}? Your data will be kept until you reactivate the app."
};
const w6Gau0 = {
  context: "deactivate named app",
  string: "Are you sure you want to disable {name}? Your data will be kept until you reactivate the app. You will be still billed for the app."
};
const LtqrM8 = {
  context: "delete custom app",
  string: "Deleting {name}, you will delete all the data and webhooks regarding this app. Are you sure you want to do that?"
};
const zQX6xO = {
  context: "dialog header",
  string: "App l\xF6schen"
};
const whTEcF = {
  context: "link",
  string: "Deaktivieren"
};
const Go50v2 = {
  context: "app privacy policy link",
  string: "View this app\u2019s privacy policy"
};
const a55zOn = {
  context: "section header",
  string: "Data privacy"
};
const Gjb6eq = {
  context: "link",
  string: "Get Support"
};
const P5twxk = {
  context: "link",
  string: "Aktivieren"
};
const VsGcdP = {
  context: "section header",
  string: "App Berechtigungen"
};
const jDIRQV = {
  context: "section header",
  string: "About this app"
};
const MSItJD = {
  string: "You are about to leave the Dashboard. Do you want to continue?"
};
const WnlZMO = {
  context: "title",
  string: "There\u2019s a problem with app."
};
const k5lHFp = {
  context: "app data privacy link",
  string: "Learn more about data privacy"
};
const PkCmGU = {
  context: "install button",
  string: "Install App"
};
const Id7C0X = {
  context: "section header",
  string: "You are about to install {name}"
};
const UCHtG6 = {
  context: "button",
  string: "About"
};
const hdcGSJ = {
  context: "button",
  string: "Support/FAQ"
};
const llC1q8 = {
  context: "button",
  string: "App home page"
};
const nIrjSR = {
  context: "section header",
  string: "Ongoing Installations"
};
const JufWFT = {
  context: "app installation error",
  string: "There was a problem during installation"
};
const D4nzdD = {
  context: "checkbox label",
  string: "Grant this app full access to the store"
};
const flP8Hj = {
  context: "card description",
  string: "Expand or restrict app permissions to access certain part of Saleor system."
};
const GjH9uy = {
  context: "header",
  string: "Create New App"
};
const Kxiige = {
  string: "Generated Token"
};
const DGCzal = {
  string: "This token gives you access to your shop's API, which you'll find here: {url}"
};
const r86alc = {
  context: "button",
  string: "Kopiert"
};
const ixjvkM = {
  string: "Ihr Standard-Token wurde erfolgreich erstellt. Kopieren Sie ihren pers\xF6nlichen Token jetzt. Eine Einsicht zu einem sp\xE4teren Zeitpunkt ist leider nicht mehr m\xF6glich."
};
const imYxM9 = {
  context: "header",
  string: "App Information"
};
const foNlhn = {
  context: "custom app name",
  string: "App Name"
};
const MAsLIT = {
  context: "custom app token key",
  string: "Schl\xFCssel"
};
const bsP4f3 = {
  string: "No tokens found"
};
const RMB6fU = {
  context: "button",
  string: "Token erstellen"
};
const VHuzgq = {
  context: "table actions",
  string: "Aktionen"
};
const XB2Jj9 = {
  context: "create app button",
  string: "Create App"
};
const voRaz3 = {
  context: "custom apps content",
  string: "Your custom-created apps will be shown here."
};
const TBaMo2 = {
  context: "about app",
  string: "About"
};
const ZeD2TK = {
  context: "section header",
  string: "Third-party Apps"
};
const SwISVH = {
  context: "section header",
  string: "Saleor Marketplace"
};
const NskBjH = {
  context: "marketplace content",
  string: "Marketplace is coming soon"
};
const wxFwUW = {
  context: "marketplace button",
  string: "Visit Marketplace"
};
const LATpSE = {
  context: "marketplace content",
  string: "Discover great free and paid apps in our Saleor Marketplace."
};
const k0rGBI = {
  string: "Zugriffstoken wird zur Authentifizierung von Servicekonten verwendet."
};
const t9a9GQ = {
  string: "Ihr Token wurde erfolgreich erstellt. Kopieren Sie ihren pers\xF6nlichen Token jetzt. Eine Einsicht zu einem sp\xE4teren Zeitpunkt ist leider nicht mehr m\xF6glich."
};
const T5nU7u = {
  context: "header",
  string: "Token erstellen"
};
const isM94c = {
  context: "create service token, button",
  string: "Erstellen"
};
const quV5zH = {
  context: "dialog title",
  string: "Delete Token"
};
const ZprV2g = {
  context: "app has been installed",
  string: "{name} is ready to be used"
};
const uIPD1i = {
  context: "app has been removed",
  string: "App successfully removed"
};
const agZQkB = {
  context: "window title",
  string: "Create App"
};
const oiuwOl = {
  context: "button",
  string: "Assign"
};
const l2oVCF = {
  context: "attributes section name",
  string: "Attribute"
};
const eWV760 = {
  string: "Attribute with this slug already exists"
};
const rKf4LU = {
  context: "dialog title",
  string: "Delete attributes"
};
const JI2Xwp = {
  context: "dialog title",
  string: "Attribut l\xF6schen"
};
const h1rPPg = {
  context: "dialog content",
  string: "Are you sure you want to delete {attributeName}?"
};
const jBu2yj = {
  context: "acre-inch unit",
  string: "acre-inch"
};
const A9QSur = {
  context: "area units type",
  string: "Gegend"
};
const xOEZjV = {
  context: "attribute's label",
  string: "Standard Kennzeichnung"
};
const P79U4b = {
  context: "attribute's slug short code label",
  string: "Attribute Code"
};
const Q7uuDr = {
  context: "attribute slug input field helper text",
  string: "This is used internally. Make sure you don\u2019t use spaces"
};
const l5V0QT = {
  context: "boolean attribute type",
  string: "Boolean"
};
const DzPVnj = {
  context: "date time attribute type",
  string: "Date Time"
};
const bZksto = {
  context: "product attribute type",
  string: "Dropdown"
};
const LnRlch = {
  context: "attribute's editor component entity",
  string: "Entity"
};
const z1y9oL = {
  context: "file attribute type",
  string: "Datei"
};
const YgE6ga = {
  context: "imperial unit system",
  string: "Imperial"
};
const oIvtua = {
  context: "attribute's editor component",
  string: "Catalog Input type for Store Owner"
};
const ZayvsI = {
  context: "metric unit system",
  string: "Metric"
};
const cKjFfl = {
  context: "product attribute type",
  string: "Multiple Select"
};
const SNiyXb = {
  context: "numeric attribute type",
  string: "Numeric"
};
const Iafyt5 = {
  context: "page attribute entity type",
  string: "Seiten"
};
const B0PaVS = {
  context: "pint unit",
  string: "pint"
};
const PiSXjb = {
  context: "check to require numeric attribute unit",
  string: "Select unit"
};
const gx4wCT = {
  context: "swatch attribute type",
  string: "Swatch"
};
const fdbqIs = {
  context: "text attribute type",
  string: "Text"
};
const Orgqv4 = {
  context: "numeric attribute unit",
  string: "Unit"
};
const zWM89r = {
  context: "numeric attribute units of",
  string: "Units of"
};
const ghje1I = {
  context: "numeric attribute unit system",
  string: "System"
};
const njBulj = {
  context: "check to require attribute to have value",
  string: "Value Required"
};
const cy8sV7 = {
  context: "volume units types",
  string: "Volume"
};
const Vdy5g7 = {
  context: "weight units type",
  string: "Gewicht"
};
const dKPMyh = {
  context: "tab name",
  string: "All Attributes"
};
const IGvQ8k = {
  context: "button",
  string: "Create attribute"
};
const rvk9ls = {
  context: "attribute can be used only in variants",
  string: "Variant Only"
};
const HQR2y0 = {
  context: "attribute value is required",
  string: "Value Required"
};
const cvbqJu = {
  context: "attribute",
  string: "Sichtbar auf der Produktseite"
};
const ztQgD8 = {
  string: "No attributes found"
};
const yKuba7 = {
  context: "attribute can be searched in dashboard",
  string: "Searchable"
};
const oJkeS6 = {
  string: "Attribute Code"
};
const k6WDZl = {
  context: "attribute is visible",
  string: "Sichtbar"
};
const HjUoHK = {
  context: "attribute's label'",
  string: "Standard Kennzeichnung"
};
const ErNH3D = {
  string: "Define where this attribute should be used in Saleor system"
};
const nwvQPg = {
  context: "section header",
  string: "Organization"
};
const v1pNHW = {
  string: "Attribute Class"
};
const zbJHl7 = {
  context: "attribute type",
  string: "Content Attribute"
};
const qkRuT0 = {
  context: "attribute type",
  string: "Product Attribute"
};
const jswILH = {
  context: "add attribute as column in product list table",
  string: "Add to Column Options"
};
const AzMSmb = {
  context: "caption",
  string: "Wenn aktiviert, kann dieses Attribut als Spalte in der Produkttabelle verwendet werden."
};
const lCxfDe = {
  context: "attribute properties regarding dashboard",
  string: "Dashboard Eigenschaften"
};
const Q9wTrz = {
  context: "caption",
  string: "Wenn aktiviert, k\xF6nnen Sie dieses Attribut zum Filtern von Produkten in der Produktliste verwenden."
};
const AgY5Mv = {
  context: "attribute properties regarding storefront",
  string: "Storefront Properties"
};
const cJ5ASN = {
  context: "attribute position in storefront filters",
  string: "Position in faceted navigation"
};
const h2Hta6 = {
  context: "caption",
  string: "If enabled, attribute will be accessible to customers."
};
const I2wCwj = {
  context: "swatch attribute image label",
  string: "Bild"
};
const JyQoES = {
  context: "delete attribute value",
  string: 'Are you sure you want to delete "{name}" value?'
};
const WWV8aZ = {
  context: "dialog title",
  string: "Delete attribute value"
};
const no3Ygn = {
  string: 'Are you sure you want to delete "{name}" value? If you delete it you won\u2019t be able to assign it to any of the products with "{attributeName}" attribute.'
};
const XYhE8p = {
  context: "edit attribute value",
  string: "Edit Value"
};
const PqMbma = {
  context: "add attribute value",
  string: "Wert hinzuf\xFCgen"
};
const UhcALJ = {
  context: "attribute name",
  string: "Name"
};
const H60H6L = {
  context: "attribute values list: name column header",
  string: "Standard Shopansicht"
};
const g5zIpS = {
  context: "No attribute values found",
  string: "No values found"
};
const J3uE0t = {
  context: "section header",
  string: "Attribute Values"
};
const NUevU9 = {
  context: "attribute values list: slug column header",
  string: "Swatch"
};
const xVn5B0 = {
  context: "added new attribute value",
  string: "Added new value"
};
const lw9WIk = {
  context: "deleted multiple attributes",
  string: "Attributes successfully delete"
};
const vzgZ3U = {
  context: "card header",
  string: "Sign In"
};
const ENBELI = {
  context: "description",
  string: "or login using"
};
const M4q0Ye = {
  context: "error message",
  string: "Sorry, login went wrong. Please try again."
};
const tTtoKd = {
  context: "error message",
  string: "Sorry, your username and/or password are incorrect. Please try again."
};
const ChGI4V = {
  context: "error message",
  string: "Saleor is unavailable, please check your network connection and try again."
};
const Ev6SEF = {
  string: "Neues Passwort"
};
const S22jIs = {
  context: "button",
  string: "Set new password"
};
const WhKGPA = {
  context: "page title",
  string: "Set up new password"
};
const lm9NSK = {
  context: "password reset, button",
  string: "Send an email with reset link"
};
const C0JLNW = {
  string: "Provided email address does not exist in our database."
};
const hOxIeP = {
  string: "Verf\xFCgbarkeit"
};
const dOQB9o = {
  context: "payment status",
  string: "Abgebrochen"
};
const o8f4Sg = {
  context: "disabled option description",
  string: "Can\u2019t fulfill until payment is captured"
};
const GOdq5V = {
  string: "Katalog"
};
const sK1FPw = {
  context: "categories section name",
  string: "Kategorien"
};
const DP6b8U = {
  context: "section header",
  string: "Background Image (optional)"
};
const wQdR8M = {
  string: "F\xFCgen Sie Titel und -Beschreibung hinzu, um diese Kategorie leichter zu finden (F\xFCr Suchmaschinen)."
};
const xo5UIb = {
  context: "dialog title",
  string: "Kategorie l\xF6schen"
};
const dJQxHt = {
  context: "delete category",
  string: "Wollen Sie Kategorie {categoryName} wirklich l\xF6schen?"
};
const vEYtiq = {
  string: "Kategorie Name"
};
const vof5TR = {
  context: "button",
  string: "Kategorie anlegen"
};
const JiXNEV = {
  string: "Kategorie suchen"
};
const vy7fjd = {
  context: "tab name",
  string: "Alle Kategorien"
};
const dM86a2 = {
  string: "No categories found"
};
const rrbzZt = {
  string: "No subcategories found"
};
const BHQrgz = {
  context: "number of subcategories",
  string: "Subkategorien"
};
const k8ZJ5L = {
  context: "number of products",
  string: "Anzahl Produkte"
};
const VQLIXd = {
  context: "product",
  string: "Name"
};
const z8jo8h = {
  context: "button",
  string: "View products"
};
const JDz5h8 = {
  context: "number of subcategories in category",
  string: "Subkategorien"
};
const NivJal = {
  context: "section header",
  string: "All Subcategories"
};
const UycVMp = {
  context: "button",
  string: "Unterkategorie anlegen"
};
const Irflxf = {
  context: "window title",
  string: "Kategorie anlegen"
};
const xl7Fag = {
  string: "Kategorie erstellt am"
};
const HvJPcU = {
  string: "Kategorie gel\xF6scht am"
};
const KCjd1o = {
  context: "dialog title",
  string: "Produkte l\xF6schen"
};
const sG0w22 = {
  context: "dialog title",
  string: "Delete categories"
};
const xRkj2h = {
  string: "Wollen Sie Kategorie {categoryName} wirklich l\xF6schen?"
};
const KeO51o = {
  string: "Channel"
};
const NlSJMM = {
  context: "channels section name",
  string: "Channels"
};
const CT5PAn = {
  context: "CannotDefineChannelsAvailabilityCard title",
  string: "Verf\xFCgbarkeit"
};
const QZoU0r = {
  context: "dialog header",
  string: "Delete Channel"
};
const sidKce = {
  context: "delete channel",
  string: "All order information from this channel need to be moved to a different channel. Please select channel orders need to be moved to:."
};
const BXMSl4 = {
  context: "currency channel",
  string: "There is no available channel to move order information to. Please create a channel with same currency so that information can be moved to it."
};
const SZJhvK = {
  context: "dialog header",
  string: "Select Channel"
};
const ZhaXLU = {
  context: "button",
  string: "Copy"
};
const UymotP = {
  context: "channel name",
  string: "Channel name"
};
const nKwgxY = {
  context: "select label",
  string: "Channel name"
};
const X8qjg3 = {
  context: "inactive",
  string: "Inaktiv"
};
const MHVglr = {
  context: "deactivate",
  string: "Deaktivieren"
};
const TSJRiZ = {
  context: "channel status title",
  string: "Channel Status"
};
const QiN4hv = {
  context: "active",
  string: "Aktiv"
};
const MQwT1W = {
  context: "activate",
  string: "Aktivieren"
};
const yHaQWG = {
  context: "variants selected label",
  string: "{variantsAmount} variants selected"
};
const Ic7Wln = {
  context: "card subtitle",
  string: "Select shipping zones that will be supplied via this channel. You can assign shipping zones to multiple channels."
};
const gtKcPf = {
  context: "title",
  string: "{zonesCount} / {totalCount} shipping zones"
};
const PTW56s = {
  context: "alert",
  string: "Channel limit reached"
};
const rZMT44 = {
  context: "created channels counter",
  string: "{count}/{max} channels used"
};
const OGm8wO = {
  context: "button",
  string: "Create Channel"
};
const ZMy18J = {
  string: "You have reached your channel limit, you will be no longer able to add channels to your store. If you would like to up your limit, contact your administration staff about raising your limits."
};
const J7mFhU = {
  context: "currency code select",
  string: "{code} - {countries}"
};
const DnghuS = {
  context: "channel create",
  string: "New Channel"
};
const AkyGP2 = {
  string: "Channel deleted"
};
const eWcvOc = {
  context: "button",
  string: "Choose file"
};
const NNT3Lp = {
  context: "collections section name",
  string: "Sammlungen"
};
const V8FhTt = {
  context: "collection label",
  string: "Versteckt"
};
const Rj8LxK = {
  string: "F\xFCgen Sie Titel und -Beschreibung hinzu, um diese Kollektion leichter zu finden (F\xFCr Suchmaschinen)."
};
const Fxa6xp = {
  context: "page header",
  string: "Kollektion hinzuf\xFCgen"
};
const G4g5Ii = {
  context: "tab name",
  string: "Alle Kollektionen"
};
const s97tLq = {
  string: "Search Collections"
};
const jyaAlB = {
  context: "button",
  string: "Kollektion anlegen"
};
const lL3YJO = {
  context: "collection",
  string: "Ver\xF6ffentlicht"
};
const mWQt3s = {
  string: "Anzahl Produkte"
};
const VZsE96 = {
  string: "Kollektion Name"
};
const UxdBmI = {
  context: "collection availability",
  string: "Verf\xFCgbarkeit"
};
const dpY94C = {
  context: "collection publication date",
  string: "Published on {date}"
};
const FcVEpe = {
  context: "collection publication date",
  string: "Unpublished"
};
const Mee46w = {
  context: "collection publication date",
  string: "Becomes published on {date}"
};
const Oe62bR = {
  context: "product availability",
  string: "Verf\xFCgbarkeit"
};
const scHVdW = {
  context: "button",
  string: "Assign product"
};
const Q8wHwJ = {
  string: "Sammlung gel\xF6scht"
};
const MxhVZv = {
  string: "Wollen Sie wirklich das Bild der Kollektion l\xF6schen?"
};
const I1Mz7h = {
  string: "Verf\xFCgbarkeit in Vertriebskan\xE4len verwalten"
};
const pVFoOk = {
  string: "Wollen Sie Kollektion {collectionName} wirklich l\xF6schen?"
};
const ttMauu = {
  context: "window title",
  string: "Kollektion anlegen"
};
const fzk04H = {
  context: "dialog title",
  string: "Bils l\xF6schen"
};
const yT5zvU = {
  string: "{counter,plural,one {Wollen Sie diese Kollektion wirklich l\xF6schen?} other {Wollen Sie wirklich {displayQuantity} Kollektionen l\xF6schen?}}"
};
const Ykw8k5 = {
  context: "dialog title",
  string: "Kollektionen l\xF6schen"
};
const C7eDb9 = {
  string: "Berechtigungsgruppen"
};
const Fbr4Vp = {
  context: "dialog header",
  string: "Rechte"
};
const MVU6ol = {
  context: "exceeded permissions description",
  string: "Die Berechtigungen dieser Gruppe \xFCberschreiten Ihre Berechtigungen. Sie k\xF6nnen nur Berechtigungen verwalten, die Sie selbst besitzen."
};
const VmMDLN = {
  context: "permission list item description",
  string: "This group is last source of that permission"
};
const O95R3Z = {
  string: "Telefon"
};
const TE4fIS = {
  string: "Stadt"
};
const oYGfnY = {
  string: "ZIP / Postleitzahl"
};
const oQY0a2 = {
  string: "Adresszeile 2"
};
const AuwpCm = {
  string: "Country area"
};
const PkjQS6 = {
  context: "description",
  string: "No results found"
};
const fP9FXB = {
  context: "input label",
  string: "Search Attributes"
};
const auxEP1 = {
  context: "input placeholder",
  string: "Search by attribute name"
};
const QM9P8G = {
  context: "dialog header",
  string: "Assign Attribute"
};
const ylobu9 = {
  context: "assign reference to product, button",
  string: "Assign"
};
const GUlwXU = {
  context: "dialog header",
  string: "Assign Attribute Value"
};
const RoKOQJ = {
  context: "label",
  string: "Search Attribute Value"
};
const NsgWhZ = {
  context: "placeholder",
  string: "Search by value name, etc..."
};
const sW9IyX = {
  context: "dialog header",
  string: "Kategorie zuordnen"
};
const sf6FMK = {
  context: "dialog search placeholder",
  string: "Suchen nach Kategorie, etc..."
};
const XLYssG = {
  context: "assign categories to sale and save",
  string: "Assign and save"
};
const f0hXz4 = {
  context: "dialog header",
  string: "Kollektion suchen"
};
const JiRKgJ = {
  context: "dialog search placeholder",
  string: "Suchen nach Kollektion, etc..."
};
const cryH86 = {
  context: "assign collections to sale and save",
  string: "Assign and save"
};
const SHm7ee = {
  string: "Search by product name, attribute, product type etc..."
};
const dTCDMn = {
  context: "dialog header",
  string: "Assign Product"
};
const WQnltU = {
  string: "No products available in order channel matching given query"
};
const xAHOGV = {
  context: "dialog header",
  string: "Assign Variant"
};
const j8PV7E = {
  context: "attribute values",
  string: "Werte"
};
const fgHLXc = {
  context: "attribute value",
  string: "Wert"
};
const jHJmjf = {
  string: "No results"
};
const XwrKOi = {
  context: "unassign multiple attributes from item",
  string: "{counter,plural,one {Are you sure you want to unassign this attribute from {itemTypeName}?} other {Are you sure you want to unassign {attributeQuantity} attributes from {itemTypeName}?}}"
};
const B2LE7A = {
  context: "menu item loading",
  string: "working..."
};
const vY2lpx = {
  context: "channels availability text",
  string: "Available at {selectedChannelsCount} out of {allChannelsCount, plural, one {# channel} other {# channels}}"
};
const UjsI4o = {
  context: "date",
  string: "seit {date}"
};
const Jt3DwJ = {
  context: "publish on date",
  string: "Publish on"
};
const Y7Vy19 = {
  context: "available on date",
  string: "Set available on"
};
const U3BQKA = {
  string: "Set publication date"
};
const tQuE1q = {
  string: "Select channels you want for {contentType} to be available on"
};
const B9yrkK = {
  string: "No Channels found"
};
const EWCUdP = {
  string: "Channels A to Z"
};
const ybaLoZ = {
  string: "Search through channels"
};
const PctLol = {
  context: "no channels found title",
  string: "No Channels Found"
};
const zR9Ozi = {
  context: "select all channels label",
  string: "Select All Channels"
};
const cFVgOo = {
  context: "Channel label",
  string: "Channel"
};
const T0Mfxq = {
  context: "product status title",
  string: "{channelCount} {channelCount,plural, =1 {Channel} other {Channels}}"
};
const JgXBAw = {
  context: "dropdown label when there are no channels assigned",
  string: "No channels"
};
const sdA14A = {
  context: "Status label when object is published in a channel",
  string: "Ver\xF6ffentlicht"
};
const GzbSQk = {
  context: "Status label when object is scheduled to publish in a channel",
  string: "Scheduled to publish"
};
const s2y5eG = {
  context: "Status label",
  string: "Status"
};
const rHoRbE = {
  context: "Status label when object is unpublished in a channel",
  string: "Unpublished"
};
const TOMgXz = {
  context: "button",
  string: "Reset"
};
const zZCCqz = {
  context: "button",
  string: "Assign countries"
};
const UaYJJ8 = {
  string: "Are you sure you want to delete {name} search tab?"
};
const LmKz3g = {
  string: "Storefront"
};
const xwEc8K = {
  string: "API"
};
const ypW4Mi = {
  context: "button refreshing page",
  string: "Refresh page"
};
const p0SUJj = {
  string: "Don\u2019t worry we will fix it soon. Try to refresh the page or go back to dashboard."
};
const QCwBUI = {
  context: "button",
  string: "Delete Search"
};
const qIgdO6 = {
  string: "Benutzerdefinierter Filter"
};
const DEa1T1 = {
  context: "button",
  string: "Save Search"
};
const zSOvI0 = {
  string: "Filter"
};
const HnVtSS = {
  context: "search",
  string: "No results"
};
const FNpv6K = {
  context: "button",
  string: "Filter"
};
const erC44f = {
  context: "filters error messages dependencies missing",
  string: "Filter requires other filters: {dependencies}"
};
const USS3Q7 = {
  context: "filters error messages unknown error",
  string: "Unknown error occurred"
};
const XkX56I = {
  context: "filters error messages value required",
  string: "Choose a value"
};
const sn2awN = {
  context: "ExitFormPrompt cancel button",
  string: "Discard changes"
};
const MPOj0I = {
  context: "ExitFormPrompt confirm button",
  string: "\xC4nderungen speichern"
};
const FWWliu = {
  context: "ExitFormPrompt title",
  string: "You have unsaved changes"
};
const NxeDbG = {
  context: "image upload",
  string: "Drop here to upload"
};
const LkuDEb = {
  context: "metadata field value, header",
  string: "Wert"
};
const ETHnjq = {
  context: "header",
  string: "Private Metadata"
};
const nudPsY = {
  context: "metadata field name, header",
  string: "Field"
};
const cY6H2C = {
  context: "empty metadata text",
  string: "No metadata created for this element. Use the button below to add new metadata field."
};
const GiDxS4 = {
  context: "add metadata field,button",
  string: "Add Field"
};
const nEixpu = {
  context: "table action",
  string: "Aktionen"
};
const hptDxW = {
  context: "money",
  string: "nach {money}"
};
const zTdwWM = {
  context: "money",
  string: "{fromMoney} - {toMoney}"
};
const lW5uJO = {
  context: "money",
  string: "von {money}"
};
const U2WgwW = {
  context: "add custom select input option",
  string: "Add new value: {value}"
};
const hX5PAb = {
  string: "No results found"
};
const NqxvFh = {
  context: "navigator placeholder",
  string: "Type Command"
};
const TpPx7V = {
  context: "navigator placeholder",
  string: "Search Customer"
};
const BooQvo = {
  context: "navigator placeholder",
  string: "Type {key} to see available actions"
};
const AOI4LW = {
  context: "navigator placeholder",
  string: "Search in Catalog"
};
const YYkkhx = {
  context: "navigator section header",
  string: "Navigate to"
};
const me585h = {
  context: "navigator section header",
  string: "Quick Actions"
};
const Gxm7Qx = {
  context: "navigator notification title",
  string: "Navigator is here to help"
};
const ccXLVi = {
  string: "Kategorie"
};
const phAZoj = {
  string: "Sammlung"
};
const M1uijW = {
  context: "collection",
  string: "Nicht ver\xF6ffentlicht"
};
const CjSRT1 = {
  context: "button",
  string: "Kategorie anlegen"
};
const VdDcxc = {
  context: "button",
  string: "Kollektion anlegen"
};
const cfQf0w = {
  context: "button",
  string: "Create Order"
};
const V1mqpZ = {
  context: "button",
  string: "Create Permission Group"
};
const JFmOfi = {
  context: "button",
  string: "Produkt erstellen"
};
const y9cvqE = {
  context: "button",
  string: "Create Voucher"
};
const IyHQr0 = {
  context: "navigator action",
  string: "Go to order #{orderNumber}"
};
const kbkjEc = {
  context: "navigator catalog mode description",
  string: "Search in Catalog"
};
const DHBlFi = {
  context: "navigator customer mode description",
  string: "Search Customers"
};
const iAvKNf = {
  context: "navigator help mode description",
  string: "Display Help"
};
const usSkzP = {
  context: "navigator order mode description",
  string: "Search Orders"
};
const k8bltk = {
  string: "No Results"
};
const bj6pTd = {
  string: "Something's missing"
};
const p3eRUm = {
  context: "indicator that feature is in preview mode",
  string: "Preview"
};
const KGsaGO = {
  context: "tooltip",
  string: "This feature is in a preview state and can be subject to changes at later point"
};
const fNFEkh = {
  string: "Zeilen:"
};
const liLrVs = {
  context: "save filter tab, header",
  string: "Save Custom Search"
};
const QcIFCs = {
  context: "save search tab",
  string: "Search Name"
};
const w2Cewo = {
  string: "Search engine title"
};
const CXTIq8 = {
  string: "Search engine description"
};
const s5Imt5 = {
  context: "button",
  string: "Edit website SEO"
};
const TGX4T1 = {
  string: "Search Engine Preview"
};
const IoDlcd = {
  string: "Slug"
};
const ChAjJu = {
  context: "character limit",
  string: "{numberOfCharacters} von {maxCharacters} Zeichen"
};
const zyXcBP = {
  string: "Sorting by this column requires active filter: {filterName}"
};
const qLbse5 = {
  context: "button",
  string: "Abmelden"
};
const akXDST = {
  context: "section header",
  string: "Sichtbarkeit"
};
const qMB6d2 = {
  context: "weight",
  string: "nach {value} {unit}"
};
const LICZeR = {
  context: "weight",
  string: "von {value} {unit}"
};
const NtFVFS = {
  context: "weight",
  string: "{value} {unit}"
};
const s8FlDW = {
  context: "hide error log label in notification",
  string: "Hide log"
};
const w9xgN9 = {
  context: "see error log label in notification",
  string: "See error log"
};
const xfGZsi = {
  context: "configuration section name",
  string: "Konfiguration"
};
const YZl6cv = {
  string: "Miscellaneous"
};
const gTr0qE = {
  string: "Shipping Settings"
};
const HjXnIf = {
  string: "Content Management"
};
const jFrdB5 = {
  string: "Product Settings"
};
const MWSacl = {
  string: "Multichannel"
};
const DJFPzq = {
  context: "button",
  string: "Best\xE4tigen"
};
const yHeZRQ = {
  string: "Product Export Failed"
};
const CJEIRC = {
  string: "Product export has finished and was sent to your email address."
};
const JTcz2G = {
  context: "csv file exporting has finished, header",
  string: "Exporting CSV finished"
};
const ryAyPr = {
  string: "Requested Invoice was generated. It was added to the top of the invoice list on this view. Enjoy!"
};
const Dhherd = {
  context: "dialog header, title",
  string: "Invoice Generation"
};
const tEdFmZ = {
  context: "notification title",
  string: "System update required"
};
const XCIV8H = {
  context: "button",
  string: "Neu laden"
};
const Rjs1CD = {
  context: "button",
  string: "Weiter"
};
const H5NKfr = {
  context: "button",
  string: "Erstellen"
};
const gQGUsN = {
  context: "dialog title",
  string: "Adresse bearbeiten"
};
const kErneR = {
  string: "This customer doesn\u2019t have any adresses added to his address book. You can add address using the button below."
};
const n5vskv = {
  context: "customer's address book, header",
  string: "{fullName}'s Adressbuch"
};
const MpR4zK = {
  context: "customer details, header",
  string: "{fullName} Details"
};
const CWqmRU = {
  context: "customer's address book when no customer name is available, header",
  string: "Address Book"
};
const hMRP6J = {
  string: "Standardadresse"
};
const VyzsWZ = {
  string: "Standard-Rechnungsadresse"
};
const nLML8Y = {
  string: "Standard-Lieferadresse"
};
const puikeb = {
  context: "button",
  string: "Adresse l\xF6schen"
};
const hLOEeb = {
  context: "button",
  string: "Als standard Rechnungsadresse setzen"
};
const BfJGij = {
  context: "header",
  string: "Adressinformationen"
};
const Zd3Eew = {
  context: "subsection header",
  string: "Versandadresse"
};
const bHdFph = {
  context: "subsection header",
  string: "Anschrift"
};
const biVFKU = {
  context: "subsection header",
  string: "Rechnungsadresse"
};
const jGGnSZ = {
  context: "page header",
  string: "Primary Address"
};
const fjPWOA = {
  context: "header",
  string: "\xDCbersicht Kunden"
};
const qNcoRY = {
  context: "notes about customer header",
  string: "Notizen"
};
const w3sGrD = {
  string: "Enter any extra infotmation regarding this customer."
};
const N76zUg = {
  context: "page header",
  string: "Kunden anlegen"
};
const MjUyhA = {
  context: "section subheader",
  string: "Active member since {date}"
};
const SMakqb = {
  context: "customer contact section, header",
  string: "Kontaktinformationen"
};
const QLVddq = {
  context: "button",
  string: "Kunden anlegen"
};
const xQK2EC = {
  context: "tab name",
  string: "Alle Kunden"
};
const fhksPD = {
  string: "Number of Orders"
};
const E8VDeH = {
  string: "Anzahl Bestellungen"
};
const FpIcp9 = {
  string: "No customers found"
};
const Gr1SAu = {
  string: "Kundenname"
};
const pURrk1 = {
  context: "order status",
  string: "Status"
};
const nTF6tG = {
  context: "number of order",
  string: "No. of Order"
};
const ri3kK9 = {
  context: "order placement date",
  string: "Datum"
};
const RlfqSV = {
  string: "Keine Bestellungen gefunden"
};
const FNAZoh = {
  string: "Last login"
};
const e7Nyu7 = {
  context: "section header",
  string: "Kundenhistorie"
};
const ey0lZj = {
  context: "dialog header",
  string: "Kundenkonto l\xF6schen"
};
const qLOBff = {
  context: "dialog header",
  string: "Adresse l\xF6schen"
};
const nX2pCU = {
  context: "window title",
  string: "Kunden anlegen"
};
const PXatmC = {
  string: "Kundenprofil entfernt"
};
const ftcHpD = {
  string: "Kundenprofil erstellt"
};
const N2SbNc = {
  string: "{counter,plural,one {Are you sure you want to delete this customer?} other {Are you sure you want to delete {displayQuantity} customers?}}"
};
const q8ep2I = {
  context: "dialog header",
  string: "Kundenkontos l\xF6schen"
};
const hzSNj4 = {
  string: "Dashboard"
};
const P7PLVj = {
  string: "Datum"
};
const JqiqNj = {
  string: "Something went wrong"
};
const ufmuTp = {
  context: "button",
  string: "L\xF6schen"
};
const i0AcKY = {
  context: "notification message after log in",
  string: "Just to let you know... You're in demo mode. You can play around with the dashboard but can't save changes."
};
const Q8Qw5B = {
  string: "Beschreibung"
};
const bZenNC = {
  string: "Description (optional)"
};
const g3qjSf = {
  context: "button",
  string: "Assign categories"
};
const AbyDC7 = {
  context: "section header",
  string: "Eligible Categories"
};
const Uu76vj = {
  context: "no categories",
  string: "No categories found"
};
const fV6yX5 = {
  context: "table head",
  string: "Kategorie Name"
};
const QGjJcT = {
  context: "number of products",
  string: "Produkte"
};
const XNeJay = {
  context: "section header",
  string: "Geeignete Kollektionen"
};
const IoCMjg = {
  context: "no collections",
  string: "Keine Kollektion gefunden."
};
const ht9yOD = {
  context: "table head",
  string: "Kollektion Name"
};
const dGqEJ9 = {
  context: "search box placeholder",
  string: "Search by country name"
};
const wgA48T = {
  context: "country selection",
  string: "Countries A to Z"
};
const AVF5T5 = {
  context: "voucher end date, switch button",
  string: "Set end date"
};
const zKOGkU = {
  context: "time during discount is active, header",
  string: "Active Dates"
};
const U8eeLW = {
  context: "button",
  string: "Produkte zuweisen"
};
const OrR3Qy = {
  context: "no products",
  string: "Keine Produkte gefunden"
};
const bPFp8B = {
  context: "product type",
  string: "Product Type"
};
const ySqrUU = {
  context: "button",
  string: "Assign variants"
};
const lehGc9 = {
  context: "section header",
  string: "Eligible Variants"
};
const Q1HhPk = {
  context: "table head",
  string: "Product Type"
};
const lHccsy = {
  context: "table head",
  string: "Variant Name"
};
const F56hOz = {
  context: "sale name",
  string: "Name"
};
const JHfbXR = {
  context: "button",
  string: "Angebot erstellen"
};
const Yjhgle = {
  context: "tab name",
  string: "Alle Angebote"
};
const AnqH4p = {
  context: "sale status",
  string: "Aktiv"
};
const RBxYJf = {
  context: "sale status",
  string: "Expired"
};
const s17U7u = {
  context: "discount type",
  string: "Percentage"
};
const BanAhF = {
  context: "sale status",
  string: "Scheduled"
};
const zjHH6b = {
  context: "sale start date",
  string: "Started"
};
const SpngiS = {
  context: "sale status",
  string: "Status"
};
const KHZlmi = {
  string: "Discount Type"
};
const XZR590 = {
  context: "sale value",
  string: "Wert"
};
const giF5UV = {
  context: "sale end date",
  string: "Ends"
};
const iBSq6l = {
  context: "sale start date",
  string: "Started"
};
const JnzDrI = {
  context: "discount type",
  string: "Fixed Amount"
};
const wHdMAX = {
  context: "sale value, header",
  string: "Wert"
};
const x3g4Ry = {
  context: "sale discount",
  string: "Discount Value"
};
const cehiWu = {
  context: "channels sale info",
  string: "Channels that don\u2019t have assigned discounts will use their parent channel to define the price. Price will be converted to channel\u2019s currency"
};
const Hj3T7P = {
  context: "column title",
  string: "Channel name"
};
const PsclSa = {
  context: "page header",
  string: "Create Voucher"
};
const YjcN9w = {
  context: "time during voucher is active, header",
  string: "Active Dates"
};
const glT6fm = {
  string: "Der Gutschein ist auf diese L\xE4nder beschr\xE4nkt"
};
const ibnmEd = {
  context: "voucher country range",
  string: "L\xE4nder"
};
const jvKNMP = {
  string: "Discount Code"
};
const mSLr9d = {
  context: "voucher code, button",
  string: "Generate Code"
};
const vTgRTZ = {
  context: "limit voucher",
  string: "Limit to one use per customer"
};
const s51tHd = {
  context: "limit voucher",
  string: "Limit of Uses"
};
const o8S0Ac = {
  context: "usage limit uses left caption",
  string: "Uses left"
};
const pNrF72 = {
  context: "tab name",
  string: "All Vouchers"
};
const IruP2T = {
  string: "Search Voucher"
};
const GbhZJ4 = {
  context: "button",
  string: "Create voucher"
};
const amQg6f = {
  context: "voucher status",
  string: "Aktiv"
};
const NLybdq = {
  context: "voucher channel",
  string: "Channel"
};
const t7UwLY = {
  context: "voucher status",
  string: "Expired"
};
const Jj0de8 = {
  context: "voucher status",
  string: "Scheduled"
};
const ujFo4A = {
  context: "voucher start date",
  string: "Started"
};
const h75GAF = {
  context: "voucher",
  string: "Times used"
};
const tuYPlG = {
  context: "minimum amount of spent money to activate voucher",
  string: "Min. Spent"
};
const b6L9n7 = {
  context: "voucher is active until date",
  string: "Ends"
};
const yHwvLL = {
  context: "voucher uses",
  string: "Verwendet"
};
const JsPIOX = {
  context: "voucher code",
  string: "Code"
};
const U2mOqA = {
  string: "Keine Gutscheine gefunden"
};
const GVinbz = {
  context: "column title",
  string: "Wert"
};
const yhv3HX = {
  context: "voucher requirements, header",
  string: "Minimum Requirements"
};
const bcf60I = {
  context: "voucher",
  string: "Applies to"
};
const HLqWXA = {
  context: "voucher value requirement",
  string: "Nutzungsbeschr\xE4nkung"
};
const sS5aVm = {
  context: "voucher discount type",
  string: "Kostenlose Lieferung"
};
const fEfCtO = {
  context: "voucher discount type",
  string: "Percentage"
};
const vXFPD6 = {
  context: "voucher discount type",
  string: "Fixed Amount"
};
const mmcHeH = {
  string: "Discount Value"
};
const ObRk1O = {
  string: "Wenn diese Option deaktiviert ist, wird der Rabatt f\xFCr jedes in Frage kommende Produkt gez\xE4hlt"
};
const bP7ZLP = {
  context: "voucher discount",
  string: "Gesamte Bestellung"
};
const WasHjQ = {
  context: "voucher discount",
  string: "Versand"
};
const n7Fg8i = {
  string: "Angebot erfolgreich angelegt"
};
const SqcGBK = {
  context: "channel availability dialog header",
  string: "Manage Channel Availability"
};
const V3fvcD = {
  context: "dialog content",
  string: "Angebot entfernt"
};
const GiJm1v = {
  context: "dialog content",
  string: "{counter,plural,one {Wollen Sie die Zuordnung dieser Kategorie wirklich aufheben?} other {Wollen Sie die Zuordnung von {displayQuantity} Kategorien wirklich aufheben?}}"
};
const B5yE8S = {
  context: "dialog header",
  string: "Kategorien vom Angebot ausschlie\xDFen"
};
const UjoSZB = {
  context: "dialog content",
  string: "{counter,plural,one {Wollen Sie die Zuordnung dieser Kollektion wirklich aufheben?} other {Wollen Sie die Zuordnung von {displayQuantity} Kollektionen wirklich aufheben?}}"
};
const rNzkIt = {
  context: "dialog content",
  string: "Wollen Sie {saleName} wirklich l\xF6schen?"
};
const opIDox = {
  context: "unassign product from sale and save, button",
  string: "Unassign and save"
};
const AHK0K9 = {
  context: "dialog content",
  string: "{counter,plural,one {Wollen Sie wirklich die Zuordnung des Produktes aufheben?} other {Wollen Sie wirklich die Zuordnung von {displayQuantity} Produkten aufheben?}}"
};
const n7GIKB = {
  context: "dialog header",
  string: "Unassign Product From Sale"
};
const Uj2hj8 = {
  context: "unassign variant from sale and save, button",
  string: "Unassign and save"
};
const iWyoZn = {
  context: "dialog content",
  string: "{counter,plural,one {Are you sure you want to unassign this variant?} other {Are you sure you want to unassign {displayQuantity} variants?}}"
};
const Kyyr7D = {
  context: "dialog header",
  string: "Unassign Variant From Sale"
};
const ZWIjvr = {
  context: "dialog header",
  string: "Angebote l\xF6schen"
};
const FPzzh7 = {
  context: "dialog content",
  string: "{counter,plural,one {Wollen Sie dieses Angebot wirklich l\xF6schen?} other {Wollen Sie wirklich {displayQuantity} Angebote l\xF6schen?}}"
};
const Eau5AV = {
  string: "Verf\xFCgbarkeit von Produkten im Channel verwalten"
};
const Q8mpW3 = {
  string: "Gutschein erfolgreich angelegt"
};
const MmGlkp = {
  context: "dialog header",
  string: "Kollektionen vom Gutschein ausschlie\xDFen"
};
const cNSLLO = {
  context: "button",
  string: "Unassign and save"
};
const cKCfSW = {
  context: "dialog header",
  string: "Zuordnung der Produkte vom Gutschein aufheben"
};
const LOSNq0 = {
  context: "dialog header",
  string: "Unassign Categories From Voucher"
};
const NEJo1I = {
  context: "dialog content",
  string: "Are you sure you want to delete {voucherCode}?"
};
const EM730i = {
  string: "Manage Channel Availability"
};
const Hgz44z = {
  context: "dialog header",
  string: "Delete Voucher"
};
const O9QPe1 = {
  context: "dialog content",
  string: "{counter,plural,one {Are you sure you want to delete this voucher?} other {Are you sure you want to delete {displayQuantity} vouchers?}}"
};
const Q0JJ4F = {
  context: "dialog header",
  string: "Delete Vouchers"
};
const eOrLzG = {
  context: "button",
  string: "Fertig"
};
const toDL5R = {
  context: "order status",
  string: "Entwurf"
};
const YMBn8d = {
  context: "draft orders section name",
  string: "Bestellentw\xFCrfe"
};
const Ja7gHc = {
  context: "button",
  string: "Bearbeiten"
};
const hJZwTS = {
  string: "Email address"
};
const T4GOiX = {
  string: "End Date"
};
const KN7zKn = {
  string: "Fehler"
};
const j0ugM4 = {
  context: "Manage and Update your warehouse information",
  string: "Exchange Rates"
};
const Q6wcZ5 = {
  string: "Vorname"
};
const pkjXPD = {
  context: "order status",
  string: "Abgeschlossen"
};
const pkUbrL = {
  string: "General Information"
};
const mbj1J5 = {
  context: "bulk issue gift cards success alert description",
  string: "Requested {cardsAmount} were successfully issued"
};
const YBqLHs = {
  context: "expiry date label",
  string: "Exact date"
};
const fzl482 = {
  context: "expires on label",
  string: "Will expire on:"
};
const rzhcpD = {
  context: "set expiry date selected label",
  string: "Set gift card expiry date"
};
const n9JOI3 = {
  context: "money amount input label",
  string: "Enter amount"
};
const vDnheO = {
  context: "gift card bulk create success dialog accept button",
  string: "Ok"
};
const NZtcLb = {
  context: "gift card bulk create success dialog content",
  string: "We have issued all of your requested gift cards. You can download the list of new gift cards using the button below."
};
const IVOjqW = {
  context: "gift card bulk create success dialog export button",
  string: "Export To Email"
};
const WyPitj = {
  context: "gift card bulk create success dialog title",
  string: "Bulk Issue Gift Cards"
};
const hnBvH7 = {
  context: "copied to clipboard alert title",
  string: "Copied to clipboard"
};
const RXbkle = {
  context: "copy code button label",
  string: "Copy code"
};
const zjZuhM = {
  context: "created gift card code label",
  string: "This is the code of a created gift card:"
};
const WzHfj8 = {
  context: "successfully created gift card alert title",
  string: "Successfully created gift card"
};
const MgdgpT = {
  context: "customer input label",
  string: "Kunde"
};
const uilt7q = {
  context: "issued cards amount label",
  string: "Cards Issued"
};
const PilTI6 = {
  context: "issue gift card button label",
  string: "Issue"
};
const UKgP89 = {
  context: "note input label",
  string: "Notiz"
};
const ZuqkSp = {
  context: "note input subtitle",
  string: "Why was this gift card issued. This note will not be shown to the customer. Note will be stored in gift card history"
};
const ArctEg = {
  context: "requires activation checkbox caption",
  string: "All issued cards require activation by staff before use."
};
const vCw7BP = {
  context: "requires activation checkbox label",
  string: "Requires activation"
};
const JftRtx = {
  context: "issue gift card dialog title",
  string: "Issue gift card"
};
const uQk8gB = {
  context: "export all items to csv file",
  string: "All gift cards ({number})"
};
const n97Ii0 = {
  context: "export selected items to csv file",
  string: "Selected giftCards ({number})"
};
const CRAfpd = {
  context: "gift card export dialog confirm button label",
  string: "Export codes"
};
const bDHiYK = {
  context: "gift card export success alert description",
  string: "We are currently exporting your gift card codes. As soon as your file is available it will be sent to your email address"
};
const YEpYMB = {
  context: "gift card export csv success alert title",
  string: "Exporting CSV"
};
const DQJnB4 = {
  context: "gift card export dialog title",
  string: "Export Gift Card Codes"
};
const KqbfFa = {
  context: "expiry date selection info message",
  string: "You can set gift cards to expire after a certain time after their purchase. Remember that in some countries, gift cards expiry is prohibited by law."
};
const xHj9Qe = {
  context: "gift card settings header",
  string: "Gift Cards Settings"
};
const pCy5EP = {
  context: "gift card history message",
  string: "Gift card was activated"
};
const aEc9Ar = {
  context: "gift card history message",
  string: "Gift card balance was reset by {resetBy}"
};
const PcQRxi = {
  context: "gift card history message",
  string: "Gift card was bought in order {orderNumber}"
};
const gAqkrG = {
  context: "gift card history message",
  string: "Gift card was deactivated by {deactivatedBy}"
};
const vQunFc = {
  context: "gift card history message",
  string: "Gift card expiry date was updated by {expiryUpdatedBy}"
};
const fLhj3a = {
  context: "gift card history message",
  string: "Gift card expiry date was updated"
};
const jDovoJ = {
  context: "gift card history message",
  string: "Gift card was issued"
};
const JgNb8X = {
  context: "notifier message",
  string: "There was an error adding a note"
};
const WS4ov0 = {
  context: "notifier message",
  string: "Note was added sucessfully"
};
const gj3MUg = {
  context: "gift card history message",
  string: "Gift card was resent"
};
const tsL3IW = {
  context: "gift card history message",
  string: "Gift card was sent to customer"
};
const vkAWwY = {
  context: "gift card history message",
  string: "Gift card tags were updated"
};
const Uu2B2G = {
  context: "gift card history message",
  string: "Gift card was used as a payment method on order {orderLink} <buyer>by</buyer>"
};
const ttk0w7 = {
  context: "resend code to customer description",
  string: "Gift Card Code will be resent to email provided during checkout. You can provide a different email address if you want to:"
};
const AqHafs = {
  context: "provided email input placeholder",
  string: "Provided email address"
};
const NLNonj = {
  context: "send to channel select label",
  string: "Send to channel"
};
const s1IQuN = {
  context: "resend button label",
  string: "Resend"
};
const mslSpp = {
  context: "resend code to customer title",
  string: "Resend code to customer"
};
const STJx8N = {
  context: "change button label",
  string: "\xC4ndern"
};
const kFkMoG = {
  context: "set balance dialog subtitle",
  string: "What would you like to set cards balance to. When you change the balance both values will be changed"
};
const tKxCld = {
  context: "set balance dialog title label",
  string: "Set balance"
};
const XAPFHP = {
  context: "card update success alert title",
  string: "Successfully updated card balance"
};
const PJDcQs = {
  context: "set balance button label",
  string: "Set Balance"
};
const JKbpH9 = {
  context: "expiry date checkbox label",
  string: "Gift card expires"
};
const nFTvQW = {
  context: "expiration date label",
  string: "Expiration date"
};
const Jo01VZ = {
  context: "bought by label",
  string: "Bought by"
};
const kc4c4d = {
  context: "product label",
  string: "Product bought to get gift card"
};
const Eh6KYS = {
  context: "info card title",
  string: "Card information"
};
const kS5Qgk = {
  context: "used by label",
  string: "Used by"
};
const EA7rjI = {
  context: "disabled status label",
  string: "Disabled"
};
const aPYFO1 = {
  context: "expired status label",
  string: "Expired"
};
const lCPxtT = {
  context: "resend code label",
  string: "Resend code"
};
const GibKGn = {
  context: "success gift card disable message",
  string: "Successfully disabled gift card"
};
const bDO9DN = {
  context: "success gift card enable message",
  string: "Successfully enabled gift card"
};
const xPnZ0R = {
  context: "title",
  string: "Details"
};
const kcMVsB = {
  context: "balance amound missing error message",
  string: "Balance amount is missing"
};
const tXIgmR = {
  context: "balance curreny missing error message",
  string: "Balance currency is missing"
};
const osPBn1 = {
  context: "currency filter label",
  string: "Currency"
};
const tTuCYj = {
  context: "all gift cards label",
  string: "All Gift Cards"
};
const vC8vyb = {
  context: "enabled status option label",
  string: "Enabled"
};
const VceXrc = {
  context: "initial balance filter label",
  string: "Initial balance"
};
const Sjd7wm = {
  context: "product filter label",
  string: "Produkt"
};
const labkPK = {
  context: "search gift card placeholder",
  string: "Search Gift Cards, e.g {exampleGiftCardCode}"
};
const D4CsYK = {
  context: "status filter label",
  string: "Status"
};
const WMGoqz = {
  context: "used by filter label",
  string: "Used by"
};
const IzEVek = {
  context: "bulk disable label",
  string: "Deaktivieren"
};
const KcsJKm = {
  context: "error with activatation alert message",
  string: "Error activating gift {count,plural,one{card} other{cards}}"
};
const C90nKP = {
  context: "error with deactivatation alert message",
  string: "Errors deactivating gift {count,plural,one{card} other{cards}}"
};
const IwEQvz = {
  context: "success activate alert message",
  string: "Successfully activated gift {count,plural,one{card} other{cards}}"
};
const SO56cv = {
  context: "success deactivate alert message",
  string: "Successfully deactivated gift {count,plural,one{card} other{cards}}"
};
const HqeqEV = {
  context: "create gift card product alert message",
  string: "Create a gift card product"
};
const rCy3Fe = {
  context: "invalid date in expirydate field content",
  string: "Gift Card with past expiration date cannot be created"
};
const AJgl5u = {
  context: "gift card product message",
  string: "gift card product"
};
const MbZHXE = {
  context: "column title balance",
  string: "Balance"
};
const MJBAqv = {
  context: "column title used by/customer",
  string: "Used by"
};
const eLJQSh = {
  context: "column title gift card",
  string: "Gift Card"
};
const bwJc6V = {
  context: "column title product",
  string: "Produkt"
};
const RfPJ1E = {
  context: "issue card button label",
  string: "Issue card"
};
const E22x4H = {
  context: "no card defuned alert message",
  string: "You haven\u2019t defined a gift card product!"
};
const Rd0s80 = {
  context: "no cards found title message",
  string: "No gift cards found"
};
const jmh0rV = {
  context: "no gift card products alert message",
  string: "{createGiftCardProduct} to start selling gift cards in your store."
};
const U9o2bV = {
  context: "no gift card products and types alert message",
  string: "{createGiftCardProductType} and {giftCardProduct} to start selling gift cards in your store."
};
const F69lwk = {
  context: "settings menu item",
  string: "Einstellungen"
};
const vWTM6Y = {
  context: "customer gift cards card no cards subtitle",
  string: "There are no gift cards assigned to this customer"
};
const Utq1fE = {
  context: "customer gift cards card title",
  string: "Gift Cards"
};
const mbOuGl = {
  context: "customer gift cards card issue button",
  string: "Issue new card"
};
const vzce9B = {
  context: "customer gift cards card subtitle",
  string: "Only five newest gift cards are shown here"
};
const FQc5z6 = {
  context: "customer gift cards card view all button",
  string: "View All"
};
const Yxihwg = {
  context: "consent removal of gift cards with balance button label",
  string: "{selectedItemsCount,plural,one {I am aware that I am removing a gift card with balance} other {I am aware that I am removing gift cards with balance}}"
};
const S52JMl = {
  context: "default gift card delete description",
  string: "{selectedItemsCount,plural,one {Are you sure you want to delete this gift card?} other {Are you sure you want to delete {selectedItemsCount} giftCards?}}"
};
const zLtb4N = {
  context: "gift card removed success alert message",
  string: "{selectedItemsCount,plural,one {Successfully deleted gift card} other {Successfully deleted gift cards}}"
};
const RLZ1jd = {
  context: "delete gift cards with balance description",
  string: "{selectedItemsCount,plural,one {The gift card you are about to delete has available balance. By deleting this card you may remove balance available to your customer.} other {You are about to delete gift cards with available balance. Are you sure you want to do that?}}"
};
const LA13a5 = {
  context: "channel select label",
  string: "Channel"
};
const FRJRmi = {
  context: "selected customer gift card is sent to subtitle",
  string: "Selected customer will be sent the generated gift card code. Someone else can redeem the gift card code. Gift card will be assigned to account which redeemed the code."
};
const Um3g00 = {
  context: "send to customer selected label",
  string: "Send gift card to customer"
};
const j2l6cL = {
  context: "checkbox label",
  string: "Set gift card expiration period"
};
const XFtKV5 = {
  context: "input placeholder tag",
  string: "Tag"
};
const mzASqs = {
  context: "days after label",
  string: "days after issue"
};
const Qe4XHv = {
  context: "years after label",
  string: "years after issue"
};
const sjRXXz = {
  string: "Bestellung #{orderId} wurde aus Entwurf aufgegeben von {userEmail}"
};
const BNTZLv = {
  string: "Bestellung #{orderId} wurde aus Entwurf aufgegeben"
};
const E9Jssl = {
  string: "Keine Bestellungen versandfertig"
};
const bFhzRX = {
  string: "No products are out of stock"
};
const c0H45L = {
  string: "{amount, plural,one {One order is ready to fulfill} other {{amount} orders are ready to fulfill}}"
};
const md326v = {
  string: "{amount, plural,one {One payment to capture} other {{amount} payments to capture}}"
};
const cdxwA8 = {
  string: "{amount, plural,one {One product out of stock} other {{amount} products out of stock}}"
};
const gSQ0Ge = {
  string: "Variant {name} has been set as default."
};
const DOkfyZ = {
  string: "Insufficient permissions"
};
const aheQdn = {
  string: "Nachname"
};
const IbVKSH = {
  context: "button",
  string: "Manage"
};
const wlQTfb = {
  context: "go to next step, button",
  string: "Next"
};
const oUWADl = {
  string: "Nein"
};
const s9sOcC = {
  context: "button",
  string: "OK"
};
const lzdvwp = {
  context: "field is optional",
  string: "Optional"
};
const Ta9j04 = {
  context: "orders section name",
  string: "Bestellungen"
};
const TPW2tP = {
  string: "You have reached your order limit, you will be billed extra for orders above limit. If you would like to up your limit, contact your administration staff about raising your limits."
};
const aY0HAT = {
  context: "section header",
  string: "Vertriebskanal"
};
const NJbzcP = {
  context: "dialog header",
  string: "Bestellung abbrechen"
};
const VSztEE = {
  string: "Cancelling this order will release unfulfilled stocks, so they can be bought by other customers. <b>Order will not be refunded when cancelling order - You need to do it manually.</b> Are you sure you want to cancel this order?"
};
const NhQboB = {
  context: "dialog header",
  string: "Saleor couldn\u2019t cancel order"
};
const FIZvTx = {
  context: "dialog content",
  string: "Select method you want to use to change address"
};
const RzDYi8 = {
  context: "checkbox label",
  string: "Set the same for billing address"
};
const vf56In = {
  context: "address type",
  string: "Use one of customer addresses"
};
const qov29K = {
  context: "dialog content",
  string: "Select one of customer addresses or add a new address:"
};
const Qph0GE = {
  context: "dialog content",
  string: "Add a new address:"
};
const xWEFrR = {
  context: "dialog content",
  string: "This customer doesn't have any addresses in the address book. Provide address for order:"
};
const zqarUF = {
  context: "modal information under title",
  string: "Select an address you want to use from the list below"
};
const txOXvy = {
  context: "checkbox label",
  string: "Set the same for shipping address"
};
const s6lW8R = {
  context: "option label",
  string: "Change address"
};
const Zg0dRo = {
  context: "dialog description",
  string: "You have changed customer assigned to this order. What would you like to do with the shipping address?"
};
const iue6mV = {
  context: "option label",
  string: "Keep address"
};
const uD93er = {
  context: "dialog header",
  string: "Changed Customer"
};
const VrFy8e = {
  string: "No notes from customer"
};
const puALFo = {
  context: "notes about customer, header",
  string: "Notizen"
};
const hkSkNx = {
  string: "Search Customers"
};
const VCzrEZ = {
  context: "link",
  string: "Profil anzeigen"
};
const DP5VOH = {
  string: "Versandadresse"
};
const Y7M1YQ = {
  context: "section header",
  string: "Kunde"
};
const GLX9II = {
  context: "billing address",
  string: "An Versandadresse"
};
const Qovenh = {
  string: "Anonymous user"
};
const R98JLZ = {
  context: "OrderCustomer Fulfillment from All Warehouses",
  string: "Fulfill from All Warehouses"
};
const AqXzM2 = {
  string: "Order #{orderNumber}"
};
const QSnh4Y = {
  context: "add button label",
  string: "Hinzuf\xFCgen"
};
const nvSJNR = {
  context: "discount reason input lavel",
  string: "Reason"
};
const GAmGog = {
  context: "value input label",
  string: "Rabatt-Wert"
};
const fo7nfa = {
  context: "fixed amount",
  string: "Fixed Amount"
};
const IN5iJz = {
  context: "value input helper text",
  string: "Invalid value"
};
const WTj17Z = {
  context: "dialog title item discount",
  string: "Discount Item"
};
const YFDAaX = {
  context: "dialog title order discount",
  string: "Discount this Order by:"
};
const WUf3Iu = {
  context: "percentage option",
  string: "Percentage"
};
const APcoSA = {
  context: "dialog header",
  string: "Delete Daft Order"
};
const mxtAFx = {
  string: "Wollen Sie wirklich Entwurf #{orderNumber} l\xF6schen?"
};
const lVwmf5 = {
  context: "total price of ordered products",
  string: "Gesamtsumme"
};
const Myx1Qp = {
  context: "add discount button",
  string: "Add Discount"
};
const BjxQ3u = {
  context: "add shipping address first label",
  string: "add shipping address first"
};
const M9LXb5 = {
  context: "no shipping carriers title",
  string: "No applicable shipping carriers"
};
const xUvWaP = {
  context: "subtotal price",
  string: "Zwischensumme"
};
const mQtoRO = {
  context: "taxes title",
  string: "Steuern (inkl. MwSt.)"
};
const C50ahv = {
  context: "button",
  string: "Produkte hinzuf\xFCgen"
};
const w2eTzO = {
  context: "placed orders counter",
  string: "{count}/{max} orders"
};
const LshEVn = {
  context: "button",
  string: "Bestellung erstellen"
};
const NJEe12 = {
  string: "Entwurf suchen"
};
const vwMO04 = {
  context: "draft order",
  string: "Erstellt"
};
const iEeIhY = {
  context: "draft order",
  string: "Kunde"
};
const KIh25E = {
  string: "Keine Entw\xFCrfe gefunden"
};
const ps0WUQ = {
  string: "No. of Order"
};
const hkENym = {
  string: "Kunde"
};
const mCP0UD = {
  context: "order draft creation date",
  string: "Datum"
};
const PAqicb = {
  context: "button",
  string: "Bestellung abbrechen"
};
const kPIZ65 = {
  context: "page header",
  string: "Order #{orderNumber}"
};
const CJpx4E = {
  context: "page header",
  string: "Order no. {orderNumber} - Add Fulfillment"
};
const N5UuEK = {
  context: "header",
  string: "Items ready to ship"
};
const vW3tb6 = {
  context: "name",
  string: "Produktname"
};
const Kg0Fiu = {
  context: "quantity of fulfilled products",
  string: "Quantity to fulfill"
};
const BLX9dz = {
  context: "fulfill order, button",
  string: "Abschluss"
};
const Uh9R9m = {
  context: "prepare order fulfillment, button",
  string: "Prepare fulfillment"
};
const bS7A8u = {
  context: "add tracking button",
  string: "Sendungsverfolgung hinzuf\xFCgen"
};
const dTkmON = {
  context: "edit tracking button",
  string: "Sendungsverfolgung bearbeiten"
};
const EHsnZX = {
  context: "dialog description",
  string: "Are you sure you want to approve this fullfillment?"
};
const SqhC7g = {
  context: "checkbox label, fulfillment approval",
  string: "Send shipment details to customer"
};
const UQu75k = {
  context: "dialog header",
  string: "Approve this fulfillment"
};
const xco5tZ = {
  string: "Are you sure you want to cancel fulfillment? Canceling a fulfillment will restock products at a selected warehouse."
};
const aHc89n = {
  context: "select warehouse to restock items",
  string: "Select Warehouse"
};
const bb4nSp = {
  context: "dialog header",
  string: "Auftragsabwicklung abbrechen"
};
const XwQQ1f = {
  context: "checkbox label description",
  string: "All fulfillments will be automatically approved"
};
const l9ETHu = {
  context: "checkbox label description",
  string: "You will be able to fulfill products without capturing payment for the order."
};
const G3ay2p = {
  context: "section header",
  string: "Fulfillment settings"
};
const RLTaAR = {
  context: "order history message",
  string: "Order address was updated"
};
const fkplbE = {
  context: "order history message",
  string: "Order was marked as paid"
};
const nHmugP = {
  context: "order history message",
  string: "Fulfilled {quantity} items"
};
const GLy2UR = {
  context: "order history message",
  string: "Fulfillment was cancelled"
};
const GJAX0z = {
  context: "order history message",
  string: "Bestellung wurde aufgegeben"
};
const OzHN0Z = {
  context: "order history message",
  string: "Links to the order's digital goods were sent"
};
const wOeIR4 = {
  context: "order history message",
  string: "Restocked {quantity} items"
};
const oQ27V4 = {
  context: "order history message",
  string: "Order placed information was sent to customer"
};
const cqZ5UH = {
  context: "order history message",
  string: "Order confirmation was sent to customer"
};
const chvryR = {
  context: "order history message",
  string: "Invoice was requested by {requestedBy}"
};
const fehqPs = {
  context: "order history message",
  string: "Products were deleted from an order"
};
const ubasgL = {
  context: "order history message",
  string: "Order was confirmed"
};
const aq5ZiN = {
  context: "order history message",
  string: "Fulfillment confirmation was sent to customer"
};
const hWO1SD = {
  context: "order history message",
  string: "Bestellentwurf angelegt"
};
const j3yE7I = {
  context: "order history message",
  string: "Shipping tracking number was sent to customer"
};
const U1eJIw = {
  context: "order history message",
  string: "Products were added to an order"
};
const PcPMjC = {
  context: "order history message",
  string: "Fulfillment awaits approval"
};
const XBfvKN = {
  string: "Order History"
};
const Fl3ORD = {
  context: "order history message",
  string: "Oversold {quantity} items"
};
const e92Uxp = {
  context: "order history message",
  string: "Updated fulfillment group's tracking number"
};
const TCR639 = {
  context: "order history message",
  string: "Zahlung fehlgeschlagen."
};
const BCPrmK = {
  context: "order history message",
  string: "Shipping details was sent to customer"
};
const pTpx0p = {
  context: "order history message",
  string: "Invoice no. {invoiceNumber} was generated by {generatedBy}"
};
const qddy2Z = {
  context: "order history message",
  string: "Invoice was sent to customer by {sentBy}"
};
const zRrcOG = {
  context: "order history message",
  string: "Order was cancelled"
};
const kVOslW = {
  context: "reason for discount label",
  string: "Reason for discount"
};
const kvSYZh = {
  context: "replacement created order history message description",
  string: "was created for replaced products"
};
const A0Wlg7 = {
  context: "product discount removed title",
  string: "{productName} discount was removed by"
};
const MPfyne = {
  string: "Are you sure you want to send this invoice: {invoiceNumber} to the customer?"
};
const Gzg8hy = {
  context: "section header",
  string: "Invoices"
};
const F0AXNs = {
  context: "invoice create date prefix",
  string: "erstellt"
};
const hPB89Y = {
  string: "No invoices to be shown"
};
const m6IBe5 = {
  context: "invoice number prefix",
  string: "Rechnung"
};
const zyceue = {
  context: "placed order counter",
  string: "{count}/{max} orders"
};
const WbV1Xm = {
  context: "button",
  string: "Order Settings"
};
const wTHjt3 = {
  string: "Search Orders..."
};
const WRkCFt = {
  context: "tab name",
  string: "All Orders"
};
const lJP1iw = {
  context: "order",
  string: "Channel"
};
const biAxKR = {
  context: "click and collect",
  string: "Click&Collect"
};
const PzXIXh = {
  context: "order",
  string: "Kunde"
};
const JUQwne = {
  context: "order",
  string: "Gift Card"
};
const s5v6m0 = {
  context: "order",
  string: "Gift Card ordered"
};
const Kgxlsf = {
  context: "order",
  string: "Paid with Gift Card"
};
const NWxomz = {
  string: "Fulfillment status"
};
const PHUcrU = {
  context: "date when order was placed",
  string: "Datum"
};
const k9hf7F = {
  context: "total order price",
  string: "Gesamtsumme"
};
const EbVf0Z = {
  context: "transaction reference",
  string: "Transaction reference"
};
const rwOx2s = {
  string: "Please provide a transaction reference using the input below:"
};
const sfEbeB = {
  string: "You're going to mark this order as paid."
};
const OhdPS1 = {
  context: "amount of refunded money",
  string: "Betrag"
};
const KszPFx = {
  context: "dialog header",
  string: "Ung\xFCltige Zahlung"
};
const BZ7BkQ = {
  context: "capture payment, button",
  string: "Einl\xF6sen"
};
const ghGLbJ = {
  context: "OrderPayment click&collect shipping method",
  string: "click&collect"
};
const u0S2be = {
  context: "order discount",
  string: "Rabatt"
};
const pr513b = {
  context: "ordered products",
  string: "{quantity} Artikel"
};
const vM9quW = {
  context: "order payment",
  string: "Paid with Gift Card"
};
const uUsZ7m = {
  context: "order payment",
  string: "Autorisierter Betrag"
};
const HaQ8cg = {
  context: "button",
  string: "Erstatten"
};
const uzAPCt = {
  context: "OrderPayment does not require shipping",
  string: "wird nicht angewandt"
};
const Rsknyh = {
  context: "order does not require shipping",
  string: "wird nicht angewandt"
};
const T8rvXs = {
  context: "order subtotal price",
  string: "Zwischensumme"
};
const zb4eBO = {
  context: "order total price",
  string: "Gesamtsumme"
};
const dJVXIb = {
  context: "vat included in order price",
  string: "VAT included"
};
const sEjRyz = {
  context: "voucher type order discount",
  string: "Gutschein"
};
const myyWNp = {
  context: "dialog header",
  string: "Add Product"
};
const b810WJ = {
  context: "product price",
  string: "Preis"
};
const WE8IFE = {
  context: "product name",
  string: "Produkt"
};
const tvpAXl = {
  context: "ordered product quantity",
  string: "Menge"
};
const qT6YYk = {
  context: "order line total price",
  string: "Gesamtsumme"
};
const MewrtN = {
  context: "section header",
  string: "Fulfillment"
};
const rVIlBs = {
  context: "page header with order number",
  string: "Order #{orderNumber}"
};
const JEIN47 = {
  context: "label",
  string: "Automatic Amount"
};
const zzfj8H = {
  context: "label",
  string: "No refund"
};
const fbH51z = {
  context: "Amount error message",
  string: "Amount cannot be bigger than max refund"
};
const lrq8O6 = {
  context: "order refund amount, input label",
  string: "Betrag"
};
const I7HyJZ = {
  context: "order refund amount",
  string: "Max Refund"
};
const Q55cTG = {
  context: "order refund amount",
  string: "Previously refunded"
};
const wDUBLR = {
  context: "order refund amount",
  string: "Proposed refund amount"
};
const QkFeOa = {
  context: "order refund amount button",
  string: "Erstatten"
};
const AKv2BI = {
  context: "order refund subtitle",
  string: "Refunded items can't be fulfilled"
};
const C6bb6x = {
  context: "order refund amount",
  string: "Refund total amount"
};
const i56GGQ = {
  context: "order refund amount",
  string: "Replaced Products Value"
};
const kak5vT = {
  context: "order refund amount",
  string: "Selected Products Value"
};
const iUIn50 = {
  context: "section notice",
  string: "Unfulfilled products will be restocked"
};
const bqAJCT = {
  context: "section header",
  string: "Refund Order"
};
const LKpQYh = {
  context: "refund type",
  string: "Miscellaneous Refund"
};
const CLB1k9 = {
  context: "refund type",
  string: "Produkte zur\xFCckerstatten"
};
const Y299ST = {
  context: "table column header",
  string: "Preis"
};
const aAAxKp = {
  context: "table column header",
  string: "Produkt"
};
const ikM00B = {
  context: "table column header",
  string: "Replace"
};
const EtkIxE = {
  context: "cancelled fulfillment, section header",
  string: "Cancelled ({quantity})"
};
const RlbhwF = {
  context: "product no longer exists error description",
  string: "This product is no longer in database so it can\u2019t be replaced, nor returned"
};
const iJrw63 = {
  context: "section header",
  string: "Fulfilled ({quantity})"
};
const ZPOyI1 = {
  context: "fulfilled fulfillment, section header",
  string: "Fulfilled from {warehouseName}"
};
const oQhFlK = {
  context: "refunded fulfillment, section header",
  string: "Refunded ({quantity})"
};
const jNSOSu = {
  context: "cancelled fulfillment, section header",
  string: "Refunded and Returned ({quantity})"
};
const eCRaHe = {
  context: "refunded fulfillment, section header",
  string: "Returned ({quantity})"
};
const p4zuQp = {
  context: "product no longer exists error title",
  string: "Product no longer exists"
};
const NxRsHQ = {
  context: "section header",
  string: "Fulfillment - #{fulfilmentId}"
};
const BkFke9 = {
  context: "section header",
  string: "Unfulfilled Items"
};
const BBIQxQ = {
  context: "page header",
  string: "Order no. {orderNumber} - Replace/Return"
};
const Vu9nol = {
  context: "header",
  string: "Order settings"
};
const yuiyES = {
  string: "General Settings"
};
const CLYlsu = {
  context: "section header",
  string: "Einstellungen"
};
const Nfh9QM = {
  context: "checkbox gift cards label description",
  string: "when activated non-shippable gift cards will be automatically set as fulfilled and sent to customer"
};
const wpAXKX = {
  context: "checkbox label description",
  string: "Alle Bestellungen werden automatisch best\xE4tigt und Zahlungen werden erfasst."
};
const RLYfMF = {
  context: "checkbox label",
  string: "Automatically confirm all orders"
};
const c4gbXr = {
  string: "Bestellentwurf erfolgreich fertiggestellt"
};
const CZmloB = {
  string: "Fulfillment successfully updated"
};
const Fn3bE0 = {
  string: "Order line updated"
};
const lL1HTg = {
  string: "Order marked as paid"
};
const HlCkMT = {
  string: "Order line added"
};
const KmPicj = {
  string: "Note successfully added"
};
const L87bp7 = {
  string: "Zahlung erfolgreich entwertet."
};
const j2fPVo = {
  string: "Order successfully updated"
};
const PKJqcq = {
  string: "Invoice is Generating"
};
const uMpv1v = {
  string: "Fulfillment successfully cancelled"
};
const TLNf6K = {
  context: "window title",
  string: "Bestellentwurf #{orderNumber}"
};
const GbBCmr = {
  context: "window title",
  string: "Order #{orderNumber}"
};
const qbmeUI = {
  context: "dialog header",
  string: "Auftragsentw\xFCrfe l\xF6schen"
};
const ra2O4j = {
  string: "Deleted draft orders"
};
const Q6VRrE = {
  context: "dialog content",
  string: "{counter,plural,one {Wollen Sie diesen Entwurf wirklich l\xF6schen?} other {Wollen Sie wirklich {displayQuantity} Entw\xFCrfe l\xF6schen?}}"
};
const CYEnGq = {
  context: "order fulfilled success message",
  string: "Fulfilled Items"
};
const NzifUg = {
  context: "window title",
  string: "Fulfill Order"
};
const XRf1Bi = {
  context: "order refunded success message",
  string: "Refunded Items"
};
const XQBVEJ = {
  context: "order return error description when cannot refund",
  string: "We\u2019ve encountered a problem while refunding the products. Product\u2019s were not refunded. Please try again."
};
const l9Lwjh = {
  context: "order return error title when cannot refund",
  string: "Couldn't refund products"
};
const a9S9Je = {
  context: "page types section name",
  string: "Page Types"
};
const kTr2o8 = {
  string: "Attribute name"
};
const uxPpRx = {
  context: "button",
  string: "Assign attribute"
};
const nf3XSt = {
  context: "attribute internal name",
  string: "Slug"
};
const iQxjow = {
  context: "section header",
  string: "Content Attributes"
};
const CcEwXH = {
  context: "dialog header",
  string: "Delete Page Types"
};
const RZmdM3 = {
  context: "delete page types",
  string: "{counter,plural,one {Are you sure you want to delete this page type? After doing so you won\u2019t be able to revert changes.} other {Are you sure you want to delete {displayQuantity} page types? After doing so you won\u2019t be able to revert changes.}}"
};
const OVOU1z = {
  context: "section header",
  string: "Metadata"
};
const caqRmN = {
  context: "header",
  string: "Create Page Type"
};
const lct0qd = {
  string: "This list shows all attributes that will be assigned to pages that have this page type assigned."
};
const jWna9Q = {
  string: "Content Type Name"
};
const umsU70 = {
  string: "Search Page Type"
};
const oVDZUb = {
  context: "tab name",
  string: "All Page Types"
};
const BQ2NVl = {
  context: "page type name",
  string: "Content Type Name"
};
const oHbgcK = {
  context: "PageTypeDeleteWarningDialog title",
  string: "Delete page {selectedTypesCount,plural,one{type} other{types}}"
};
const I8mqqj = {
  context: "PageTypeDeleteWarningDialog single assigned items button label",
  string: "View pages"
};
const NGc9kE = {
  string: "Page type deleted"
};
const WFG7Zk = {
  context: "unassign attribute from page type, button",
  string: "Zuordnung aufheben"
};
const H6NsC1 = {
  context: "pages section name",
  string: "Seiten"
};
const gr53VQ = {
  context: "page header",
  string: "Seite erstellen"
};
const jZbT0O = {
  string: "F\xFCgen Sie Titel und -Beschreibung hinzu, um diese Seite leichter zu finden (F\xFCr Suchmaschinen)."
};
const GZgjK7 = {
  context: "page",
  string: "Wird sichtbar ab dem {date}"
};
const X26jCC = {
  context: "page label",
  string: "Sichtbar"
};
const gMwpNC = {
  context: "page content",
  string: "Inhalt"
};
const AHRDWt = {
  context: "button",
  string: "Create page"
};
const Of19Pn = {
  context: "Types",
  string: "Page Types"
};
const UW1fLs = {
  context: "search pages placeholder",
  string: "Search Pages"
};
const iMJka8 = {
  string: "No pages found"
};
const I8dAAe = {
  context: "page internal name",
  string: "Slug"
};
const G1KzEx = {
  context: "page status",
  string: "Ver\xF6ffentlicht"
};
const UN3qWD = {
  context: "page status",
  string: "Nicht ver\xF6ffentlicht"
};
const ufD5Jr = {
  string: "Content type"
};
const jU9GPX = {
  context: "section header",
  string: "Organize Content"
};
const mX7zJJ = {
  context: "header",
  string: "Seite erstellen"
};
const JMbFNo = {
  string: "Successfully created new page"
};
const C1luwg = {
  context: "dialog header",
  string: "Seite l\xF6schen"
};
const yEmwxD = {
  context: "publish page, button",
  string: "Ver\xF6ffentlichen"
};
const yHQQMQ = {
  context: "dialog header",
  string: "Unpublish Pages"
};
const Wd8vG7 = {
  context: "dialog content",
  string: "{counter,plural,one {Are you sure you want to unpublish this page?} other {Are you sure you want to unpublish {displayQuantity} pages?}}"
};
const F8gsds = {
  context: "unpublish page, button",
  string: "Ver\xF6ffentlichung entfernen"
};
const wyvzh9 = {
  context: "dialog header",
  string: "Publish Pages"
};
const WRPQMM = {
  context: "dialog content",
  string: "{counter,plural,one {Are you sure you want to publish this page?} other {Are you sure you want to publish {displayQuantity} pages?}}"
};
const AzshS2 = {
  context: "notification",
  string: "Published pages"
};
const PbqNhi = {
  context: "order status",
  string: "Zum Teil abgeschlossen"
};
const INNPVX = {
  context: "payment status",
  string: "Partially paid"
};
const OGemtu = {
  context: "payment status",
  string: "Teilweise zur\xFCckerstattet"
};
const hLUYBt = {
  context: "payment status",
  string: "Pending"
};
const DNTxWr = {
  context: "permission groups section name",
  string: "Berechtigungsgruppen"
};
const jUuHVn = {
  context: "description",
  string: "No members found"
};
const UQ4Kuh = {
  context: "input label",
  string: "Search Staff Members"
};
const DPz5y6 = {
  context: "dialog header",
  string: "Assign Staff Members"
};
const lT5MYM = {
  context: "dialog title",
  string: "Unassign users"
};
const mAabef = {
  context: "checkbox label",
  string: "Group has full access to the store"
};
const CYZse9 = {
  context: "card description",
  string: "Expand or restrict group's permissions to access certain part of saleor system."
};
const sR0urA = {
  context: "dialog content",
  string: "Sind Sie sicher, dass Sie {name} l\xF6schen wollen?"
};
const O22NIZ = {
  context: "deletion error message",
  string: "Cant's delete group which is out of your permission scope"
};
const rs815i = {
  context: "text field label",
  string: "Group name"
};
const szXISP = {
  context: "permission group name",
  string: "Permission Group Name"
};
const wL7VAE = {
  string: "Aktionen"
};
const CXn88q = {
  string: "Keine Berechtigungsgruppen gefunden"
};
const gVD1os = {
  context: "empty list message",
  string: "You haven\u2019t assigned any member to this permission group yet."
};
const qrWOxx = {
  string: "No members found"
};
const xxQxLE = {
  string: "E-Mail Adresse"
};
const OhFGpX = {
  context: "button",
  string: "Assign members"
};
const lGlDEH = {
  context: "header",
  string: "Group members"
};
const W32xfN = {
  context: "staff member full name",
  string: "Name"
};
const XGBsoK = {
  context: "dialog content",
  string: "Are you sure you want to unassign {counter,plural,one{this member} other{{displayQuantity} members}}?"
};
const eUjFjW = {
  string: "Permission group created"
};
const DovGIa = {
  string: "Permission Group Deleted"
};
const WhvuCb = {
  context: "plugins section name",
  string: "Plugins"
};
const Aeq79M = {
  context: "PluginDetailsChannelsCard no channels subtitle",
  string: "Plugin\u2019s settings are set common across all channels"
};
const IUeGzv = {
  context: "plugin name",
  string: "Plugin Name"
};
const w424P4 = {
  context: "section header",
  string: "Plugin Information and Status"
};
const qCH2eZ = {
  context: "header",
  string: "Add Value to Authorization Field"
};
const Egyh2T = {
  context: "section header",
  string: "Plugin Settings"
};
const EtGDeK = {
  context: "header",
  string: "{pluginName} Details"
};
const BtErCZ = {
  string: "Plugins suchen..."
};
const aOelhW = {
  context: "tab name",
  string: "Alle Plugins"
};
const zQnYKn = {
  context: "status section subtitle",
  string: "Channel status"
};
const gZHmaV = {
  context: "plugin filters error messages channels",
  string: "No channels selected"
};
const cwoN25 = {
  context: "config type section title",
  string: "Configuration Type"
};
const Co2U4u = {
  string: "Keine Plugins gefunden"
};
const rQOS7K = {
  context: "status label active",
  string: "Aktiv"
};
const ycrTBX = {
  context: "table header channel col label",
  string: "Channel"
};
const HedXnw = {
  context: "plugin channel availability status title",
  string: "{activeChannelsCount,plural, =0 {Deactivated} other {Active in {activeChannelsCount}}}"
};
const AijtXU = {
  context: "table header configuration col label",
  string: "Konfiguration"
};
const ho75Lr = {
  context: "status label deactivated",
  string: "Deactivated"
};
const reP5Uf = {
  context: "global config plugin status popup description",
  string: "Global plugins are set across all channels in your ecommerce. Only status is shown for those types of plugins"
};
const QH74y5 = {
  context: "table header name col label",
  string: "Name"
};
const JRfJD9 = {
  string: "The plugin may stop working after this field is cleared. Are you sure you want to proceed?"
};
const YQ3EXR = {
  context: "product types section name",
  string: "Product Types"
};
const kp2IYP = {
  context: "option",
  string: "Gift card product type"
};
const xRbqcg = {
  context: "option",
  string: "Regular product type"
};
const H5yp8O = {
  context: "label",
  string: "Product Type Name"
};
const rpFdD1 = {
  string: "Search Product Type"
};
const QY7FSs = {
  context: "button",
  string: "Produkttyp erstellen"
};
const X90t9n = {
  context: "product type",
  string: "Konfigurierbar"
};
const dS8Adx = {
  context: "product",
  string: "Digital"
};
const U5aVd8 = {
  context: "product",
  string: "Shippable"
};
const TalJlD = {
  context: "tax rate for a product type",
  string: "Steuer"
};
const jyTwDR = {
  context: "product type is either simple or configurable",
  string: "Typ"
};
const hHOI7D = {
  context: "product type name",
  string: "Type Name"
};
const asdvmK = {
  context: "product type",
  string: "Digital"
};
const ADTNND = {
  context: "product type",
  string: "Physical"
};
const IBw72y = {
  context: "switch button",
  string: "Is this product shippable?"
};
const VOiUXQ = {
  string: "Wird zur Berechnung von Versandkosten f\xFCr Produkte dieses Produkttyps verwendet, wenn kein spezifisches Gewicht gegeben ist"
};
const zCb8fX = {
  string: "Gewicht"
};
const vlLyvk = {
  string: "{inputType} attributes cannot be used as variant selection attributes."
};
const x3leH4 = {
  context: "ProductTypeDeleteWarningDialog title",
  string: "Delete product {selectedTypesCount,plural,one{type} other{types}}"
};
const GCPzKf = {
  context: "ProductTypeDeleteWarningDialog single assigned items button label",
  string: "View products"
};
const paa4m0 = {
  string: "Successfully created product type"
};
const F3Upht = {
  string: "Product type deleted"
};
const UJnqdm = {
  context: "dialog header",
  string: "Unassign Attribute From Product Type"
};
const r1aQ2f = {
  context: "dialog header",
  string: "Unassign Attribute from Product Type"
};
const K8xNLe = {
  context: "products section name",
  string: "Produkte"
};
const ulh3kf = {
  string: "Sammlungen"
};
const fyE8BN = {
  context: "product organization, header",
  string: "Organization"
};
const LKoIB1 = {
  string: "F\xFCgen Sie Titel und -Beschreibung hinzu, um dieses Produkt leichter zu finden (F\xFCr Suchmaschinen)."
};
const JGm7E5 = {
  context: "option",
  string: "Export stock for all warehouses"
};
const ZDJEat = {
  context: "button",
  string: "Load More"
};
const qEZ463 = {
  context: "export selected items to csv file",
  string: "Ausgew\xE4hlte Produkte ({number})"
};
const g6yuk2 = {
  context: "export items to csv file, choice field label",
  string: "Export information for:"
};
const xtUXnK = {
  context: "export all items to csv file",
  string: "Alle Produkte ({number})"
};
const WQMTKI = {
  context: "list of warehouses",
  string: "Warehouses A to Z"
};
const z1puMb = {
  context: "export items as csv or spreadsheet file",
  string: "Export as:"
};
const ki7Mr8 = {
  context: "product export to csv file, header",
  string: "Export Settings"
};
const lQRnYK = {
  context: "selectt all options",
  string: "Select All"
};
const YicEbK = {
  string: "Search Atrtibuttes"
};
const ZRz3hM = {
  string: "Export Product Stock Quantity to CSV"
};
const xjpTLF = {
  context: "informations about product stock, header",
  string: "Inventory Information"
};
const li1BBk = {
  context: "export items as csv file",
  string: "Plain CSV file"
};
const Jwuu4X = {
  context: "select product informations to be exported",
  string: "Information exported:"
};
const SZt9kC = {
  context: "export filtered items to csv file",
  string: "Current search ({number})"
};
const jj3Cb8 = {
  context: "informations about product prices etc, header",
  string: "Financial Information"
};
const oOFrUd = {
  context: "export products to csv file, button",
  string: "Produkte exportieren"
};
const dc5KWn = {
  context: "products export type label",
  string: "Produkt"
};
const xkjRu5 = {
  context: "export products to csv file, dialog header",
  string: "Export Information"
};
const zDvDnG = {
  context: "modal header",
  string: "Media from the URL you supply will be shown in the media gallery. You will be able to define the order of the gallery."
};
const FwHWUm = {
  context: "alert",
  string: "SKU limit reached"
};
const Kw0jHS = {
  context: "created products counter",
  string: "{count}/{max} SKUs used"
};
const kIvvax = {
  string: "Produkte suchen..."
};
const aFLtLk = {
  context: "tab name",
  string: "Alle Produkte"
};
const diOQm7 = {
  context: "product status",
  string: "Verf\xFCgbar"
};
const pbGIUg = {
  context: "sales channel",
  string: "Channel"
};
const Bx367s = {
  context: "product is hidden",
  string: "Versteckt"
};
const pBTTtU = {
  context: "product kind",
  string: "Product Kind"
};
const b1zuN9 = {
  string: "Preis"
};
const Sd0Ppm = {
  context: "product publication date",
  string: "Published on {date}"
};
const qpQ0uB = {
  context: "product publication date",
  string: "Unpublished"
};
const kVTWtR = {
  context: "product updated at",
  string: "Last updated"
};
const r0hgpM = {
  context: "product publication date",
  string: "Becomes published on {date}"
};
const XUU9sU = {
  context: "section header",
  string: "All Media"
};
const Ihp4D3 = {
  context: "header",
  string: "Edit Media"
};
const cW1RIo = {
  context: "section header",
  string: "Media View"
};
const Q2UXlW = {
  context: "modal button url upload",
  string: "Upload URL"
};
const mGiA6q = {
  context: "modal button upload",
  string: "Hochladen"
};
const JjeZEG = {
  context: "section header",
  string: "Organize Product"
};
const anK7jD = {
  string: "Product Type"
};
const SUbxSK = {
  context: "product weight",
  string: "Gewicht"
};
const xB7BTp = {
  string: "SKU (Stock Keeping Unit)"
};
const KTAg0f = {
  context: "tabel column header",
  string: "Warehouse Name"
};
const NcY4ph = {
  string: "Threshold that cannot be exceeded even if per channel thresholds are still available"
};
const JyQEHU = {
  context: "tabel column header",
  string: "Channels"
};
const TjGYna = {
  context: "product inventory, checkbox",
  string: "Track Inventory"
};
const cBHRxx = {
  context: "button",
  string: "Assign Warehouse"
};
const REVk27 = {
  context: "info text",
  string: "Set up an end date of preorder. When end date will be reached product will be automatically taken from preorder to standard selling"
};
const ekXood = {
  string: "Unlimited"
};
const RJ5QxE = {
  string: "Global threshold"
};
const jABdx1 = {
  string: "Active inventory tracking will automatically calculate changes of stock"
};
const CdIHMu = {
  context: "select tax ratte",
  string: "Tax Rate"
};
const iYH3Y7 = {
  context: "checkbox",
  string: "Override the product type's tax rate"
};
const C7I2lg = {
  context: "input label",
  string: "Limit quantity per checkout (optional)"
};
const W3JbSQ = {
  context: "create product variants",
  string: "How would you like to create variants:"
};
const vXA5xU = {
  context: "option description",
  string: "Use variant creator to create matrix of selected attribute values to create variants"
};
const uxZZXx = {
  context: "option",
  string: "Create multiple variant via variant creator"
};
const EzU7KV = {
  context: "option description",
  string: "Create new variant using variant details view"
};
const XDeh5D = {
  context: "dialog header",
  string: "Create Variants"
};
const f3B4tc = {
  context: "attributes, section header",
  string: "Variant Attributes"
};
const o6260f = {
  context: "attributes, section header",
  string: "Variant Selection Attributes"
};
const sw8Wl2 = {
  context: "variant pricing section subtitle",
  string: "There is no channel to define prices for. You need to first add variant to channels to define prices."
};
const U9CIo7 = {
  context: "button",
  string: "Save variant"
};
const TPCRKr = {
  context: "variant price, header",
  string: "Preis"
};
const ClFzoD = {
  string: "Selected values will be used to create variants for the configurable product."
};
const Xr5zxu = {
  string: "You choices will add {variantsNumber} SKUs to your catalog which will exceed your limit by {aboveLimitVariantsNumber}. If you would like to up your limit, contact your administration staff about raising your limits."
};
const lVZ5n7 = {
  context: "variant attribute",
  string: "Attribute"
};
const TDXskW = {
  context: "variant attribute",
  string: "Select Attribute"
};
const GQcp83 = {
  context: "variant stock, header",
  string: "Stock and Warehousing"
};
const NXpFlL = {
  context: "product attribute values, page title",
  string: "Choose Values"
};
const rVaB7c = {
  context: "attribute values, variant creation step",
  string: "Select Values"
};
const ucYPtV = {
  context: "variant attribute",
  string: "Eigenschaft ausw\xE4hlen"
};
const slKV5G = {
  context: "variant creation step",
  string: "Zusammenfassung"
};
const lra7Ej = {
  string: "Apply single price to all SKUs"
};
const J0UdxG = {
  string: "Skip pricing for now"
};
const L5rthO = {
  string: "Apply unique stock by attribute to each SKU"
};
const STp3Hl = {
  string: "Apply single stock to all SKUs"
};
const rHXF43 = {
  string: "Here is the summary of variants that will be created. You can change prices, stocks an SKU for each one created."
};
const BIqhVQ = {
  string: "Skip stock for now"
};
const S5PVx1 = {
  context: "variant creator summary card header",
  string: "Created Variants"
};
const k4brJy = {
  string: "Artikelnummer"
};
const Sx7QVu = {
  context: "variant creation step",
  string: "Prices and SKU"
};
const iigydN = {
  string: "Based on your selections we will create 8 products. Use this step to customize price and stocks for your new products."
};
const esg2wu = {
  context: "previous step, button",
  string: "Previous"
};
const V76IV7 = {
  context: "variant name",
  string: "Variante"
};
const rbkmfG = {
  context: "button",
  string: "Variante l\xF6schen"
};
const WwNtFn = {
  context: "delete product variant",
  string: "Sind Sie sicher, dass Sie {name} l\xF6schen wollen?"
};
const GFJabu = {
  context: "dialog header",
  string: "Delete Variant"
};
const dTCWqt = {
  string: "You are about to end your products preorder. You have sold {variantGlobalSoldUnits} units of this variant. Sold units will be allocated at appropriate warehouses. Remember to add remaining threshold stock to warehouses."
};
const Y4cy0i = {
  context: "dialog header",
  string: "Ending preorder"
};
const iPk640 = {
  context: "dialog header",
  string: "Media Selection"
};
const JfKvrV = {
  context: "select variant media",
  string: "Select a specific variant media from product media"
};
const vZMs8f = {
  context: "default product variant indicator",
  string: "Default"
};
const gF7hbK = {
  context: "variant name",
  string: "New Variant"
};
const EsZH44 = {
  context: "VariantDetailsChannelsAvailabilityCard item subtitle hidden",
  string: "Versteckt"
};
const rJ3lkW = {
  context: "VariantDetailsChannelsAvailabilityCard item subtitle published",
  string: "Published since {publicationDate}"
};
const jqJqdE = {
  context: "VariantDetailsChannelsAvailabilityCard no items available",
  string: "This variant is not available at any of the channels"
};
const sedoZ3 = {
  context: "VariantDetailsChannelsAvailabilityCard title",
  string: "Verf\xFCgbarkeit"
};
const Xm9qOu = {
  context: "product pricing, section header",
  string: "Preis"
};
const JFtFgc = {
  context: "tabel column header",
  string: "Selling Price"
};
const KQSONM = {
  context: "tabel column header",
  string: "Kosten"
};
const VvA7ai = {
  context: "info text",
  string: "Channels that don\u2019t have assigned prices will use their parent channel to define the price. Price will be converted to channel\u2019s currency"
};
const c8UT0c = {
  context: "tabel column header",
  string: "Channel Name"
};
const SZH0fw = {
  context: "set variant as default, button",
  string: "Set as default"
};
const n02c9W = {
  context: "product variant price",
  string: "Preis"
};
const uVssds = {
  context: "product variant inventory",
  string: "{stockQuantity,plural,one {{stockQuantity} available} other {{stockQuantity} available}}"
};
const FSinkL = {
  context: "variant stock status",
  string: "Available inventory at:"
};
const HcjV6k = {
  context: "button",
  string: "Create variants"
};
const qbqMpk = {
  context: "product variant preorder threshold",
  string: "In preorder"
};
const wWYYBR = {
  context: "product variant inventory",
  string: "{numLocations,plural,one {{numAvailable} available at {numLocations} location} other {{numAvailable} available at {numLocations} locations}}"
};
const rIJbNC = {
  string: "Verwenden Sie Varianten f\xFCr Produkte, die in verschiedenen Ausf\xFChrungen vorhanden sind, wie z. B. verschiedene Gr\xF6\xDFen oder Farben"
};
const JtZ71e = {
  context: "filtering option",
  string: "Alle Lagerh\xE4user"
};
const OTek3r = {
  context: "product variant name",
  string: "Variante"
};
const BUKMzM = {
  string: "Variant removed"
};
const T6dXGG = {
  context: "header",
  string: "Create Variant"
};
const MyM2oR = {
  context: "window title",
  string: "Create variant"
};
const PXx4Jk = {
  context: "window title",
  string: "Produkt erstellen"
};
const NBP8uu = {
  context: "page header",
  string: "New Product"
};
const yDkmX7 = {
  context: "dialog content",
  string: "{counter,plural,one {Wollen Sie wirklich dieses Produkt l\xF6schen?} other {Wollen Sie wirklich {displayQuantity} Produkte l\xF6schen?}}"
};
const F4WdSO = {
  context: "dialog header",
  string: "Produkte l\xF6schen"
};
const dPYqy0 = {
  string: "We are currently exporting your requested CSV. As soon as it is available it will be sent to your email address"
};
const YKyNm9 = {
  context: "label",
  string: "Gift Card"
};
const vlVTmY = {
  string: "Product removed"
};
const ZHF4Z9 = {
  context: "delete product dialog subtitle",
  string: "Sind Sie sicher, dass Sie {name} l\xF6schen wollen?"
};
const TWVx7O = {
  context: "delete product dialog title",
  string: "Produkt l\xF6schen"
};
const ukdRUv = {
  context: "delete variant dialog subtitle",
  string: "{counter,plural,one {Are you sure you want to delete this variant?} other {Are you sure you want to delete {displayQuantity} variants?}}"
};
const oChkS4 = {
  context: "success message",
  string: "Varianten erfolgreich angelegt"
};
const IeoGgH = {
  context: "variant created success message",
  string: "Variant created"
};
const aI80kg = {
  string: "Eigenschaften"
};
const kFYlu2 = {
  string: "Saleor runs in read-only mode. Changes not saved."
};
const rqtV5d = {
  context: "order status",
  string: "Ready to capture"
};
const oLMXDv = {
  context: "order status",
  string: "Ready to fulfill"
};
const XJSRDK = {
  context: "payment status",
  string: "Voll zur\xFCckerstattet"
};
const nwnwJ0 = {
  context: "payment status",
  string: "Refused"
};
const kJQczl = {
  context: "sales section name",
  string: "Angebote"
};
const RaycYK = {
  context: "button",
  string: "Sichern"
};
const rqiCWU = {
  string: "Saved changes"
};
const rfvBAF = {
  context: "select all options, button",
  string: "Select All"
};
const byP6IC = {
  string: "Selected"
};
const hqVMLQ = {
  context: "button",
  string: "Send"
};
const Fvvgoi = {
  string: "Your session has expired. Please log in again to continue."
};
const D9ie4n = {
  context: "shipping section name",
  string: "Shipping Methods"
};
const nNeWAx = {
  context: "dialog header",
  string: "Delete Shipping Method"
};
const aZDHYr = {
  context: "price rates info",
  string: "This rate will apply to all orders"
};
const kN6SLs = {
  string: "Min Value"
};
const yatGsm = {
  context: "card title",
  string: "Order Value"
};
const Dgp38J = {
  context: "checkbox label",
  string: "Restrict order value"
};
const vjsfyn = {
  string: "Max Value"
};
const r2dojI = {
  context: "checkbox label",
  string: "Restrict order weight"
};
const vWapBZ = {
  context: "card title",
  string: "Order Weight"
};
const u0V06N = {
  string: "Max. Order Weight"
};
const FzEew9 = {
  context: "assign products to shipping rate and save, button",
  string: "Assign and save"
};
const xZhxBJ = {
  context: "dialog header",
  string: "Produkte zuweisen"
};
const t3aiWF = {
  context: "section header",
  string: "Ausgeschlossene Produkte"
};
const ZIc5lM = {
  string: "Produktname"
};
const TLYeo5 = {
  context: "label",
  string: "Shipping Rate Description"
};
const v17Lly = {
  context: "label",
  string: "Max Delivery Time"
};
const FkDObY = {
  context: "label",
  string: "Shipping rate name"
};
const AjInNW = {
  context: "dialog description",
  string: "Choose countries you want to add to shipping zone from list below"
};
const f2F1NJ = {
  context: "section title",
  string: "Quick Pick"
};
const pGDYG5 = {
  context: "search label",
  string: "Search Countries"
};
const y7mfbl = {
  string: "Currently, there are no countries assigned to this shipping zone"
};
const YpukUN = {
  context: "label",
  string: "Name der Versandzone"
};
const axFFaD = {
  context: "range input label",
  string: "Postal codes (end)"
};
const Pyjarj = {
  string: "This shipping rate has no postal codes assigned"
};
const ju8zHP = {
  string: "Added postal codes will be excluded from using this delivery methods. If none are added all postal codes will be able to use that shipping rate"
};
const ud0w8h = {
  context: "number of postal code ranges",
  string: "{number} postal code ranges"
};
const FcTTvh = {
  context: "postal codes, header",
  string: "Postal codes"
};
const YpLVVc = {
  context: "action",
  string: "Exclude postal codes"
};
const PRlD0A = {
  string: "Versand"
};
const NDm2Fe = {
  context: "page title",
  string: "Weight Rate Create"
};
const EKoPNg = {
  context: "shipping method price",
  string: "Preis"
};
const WR8rir = {
  context: "button",
  string: "Create rate"
};
const njUQPz = {
  context: "shipping method price range",
  string: "Value Range"
};
const RUzdUH = {
  string: "No shipping rates found"
};
const FjrExY = {
  context: "price based shipping methods, section header",
  string: "Price Based Rates"
};
const aYhcie = {
  context: "shipping method weight range",
  string: "Gewichtsspanne"
};
const foB6wx = {
  context: "weight based shipping methods, section header",
  string: "Gewichtsabh\xE4ngige Preise"
};
const aPCrsp = {
  context: "shipping method name",
  string: "Name"
};
const wjKYSU = {
  context: "WarehousesSection subtitle",
  string: "Bitte w\xE4hlen Sie das Lager, von dem aus Produkte f\xFCr diese Versandzone verschickt werden. Die Lageradresse wird auch f\xFCr die Berechnung von Steuern verwendet."
};
const uULcph = {
  context: "header",
  string: "Versand"
};
const mIUNgR = {
  context: "button",
  string: "Create shipping zone"
};
const aMwxYb = {
  string: "L\xE4nder"
};
const IhK1F3 = {
  string: "No shipping zones found"
};
const Gfbp36 = {
  context: "dialog header",
  string: "Produkte vom Versand entfernen"
};
const pwqwcy = {
  context: "error message",
  string: "Maximum price cannot be lower than minimum"
};
const cpZLRH = {
  context: "dialog header",
  string: "Delete Shipping Zones"
};
const C9pcQx = {
  context: "dialog content",
  string: "{counter,plural,one {Are you sure you want to delete this shipping zone?} other {Are you sure you want to delete {displayQuantity} shipping zones?}}"
};
const YdeHZX = {
  context: "unassign products from shipping method, button",
  string: "Zuordnung aufheben"
};
const MXZuVP = {
  context: "remove country from shipping zone and save, button",
  string: "Remove and save"
};
const viFkCw = {
  context: "site settings section name",
  string: "Webseiteneinstellungen"
};
const QclvqG = {
  context: "input label",
  string: "Checkout line limit"
};
const X7fqfM = {
  context: "title",
  string: "Reserved stock"
};
const C4aDMy = {
  context: "description",
  string: "Set up time amount that stock in checkout is reserved for the customer. You can set separate values for authenticated and anonymous customers."
};
const OaKyz4 = {
  context: "input label",
  string: "Stock reservation for authenticated user (in minutes)"
};
const Av74Fa = {
  context: "section description",
  string: "You can set basic checkout rules that will be applied globally to all your channels"
};
const DASYUF = {
  context: "section title",
  string: "Checkout Configuration"
};
const pRYGUR = {
  context: "section description",
  string: "This address will be used to generate invoices and calculate shipping rates. Email address you provide here will be used as a contact address for your customers."
};
const rPX1f2 = {
  context: "section title",
  string: "Unternehmensinformationen"
};
const LVa5ew = {
  string: "Ein unerwartetes Problem ist aufgetreten"
};
const AQFMYU = {
  context: "staff section name",
  string: "Staff Members"
};
const hw9Fah = {
  context: "button",
  string: "Send invite"
};
const XMrYaA = {
  context: "checkbox label",
  string: "Benutzer aktiv"
};
const YJ4TXc = {
  context: "tab name",
  string: "All Staff Members"
};
const OaA0f9 = {
  string: "You have reached your staff member limit, you will be no longer able to add staff members to your store. If you would like to up your limit, contact your administration staff about raising your limits."
};
const pA8Mlv = {
  context: "alert",
  string: "Staff Member limit reached"
};
const aDbrOK = {
  string: "Search Staff Member"
};
const HR9OTW = {
  context: "staff member's account",
  string: "Aktiv"
};
const Fc3O3r = {
  context: "staff member's account",
  string: "Deactivated"
};
const utaSh3 = {
  context: "staff member's account",
  string: "Status"
};
const xJQX5t = {
  string: "No staff members found"
};
const cMFlOp = {
  context: "input label",
  string: "Neues Passwort"
};
const qEJT8e = {
  string: "New password must be at least 8 characters long"
};
const GXdwyR = {
  context: "input label",
  string: "Previous Password"
};
const mm0CXe = {
  string: "You should change your password every month to avoid security issues."
};
const N3Zot1 = {
  context: "button",
  string: "Change your password"
};
const ZhDQel = {
  context: "header",
  string: "Passwort"
};
const JJgJwi = {
  string: "Selecting this will change the language of your dashboard"
};
const e822us = {
  string: "Please note, while all currency and date adjustments are complete, language translations are at varying degrees of completion."
};
const mr9jbO = {
  string: "Preferred Language"
};
const CLeDae = {
  context: "section header",
  string: "Preferences"
};
const VTITVe = {
  context: "section header",
  string: "Staff Member Information"
};
const fzpXvv = {
  string: "Are you sure you want to remove {email} avatar?"
};
const VKWPBf = {
  context: "dialog header",
  string: "Delete Staff User Avatar"
};
const gxPjIQ = {
  string: "Are you sure you want to delete {email} from staff members?"
};
const QirE3M = {
  string: "Start Date"
};
const tWbE34 = {
  string: "Start Hour"
};
const tzMNF3 = {
  string: "Status"
};
const RrCui3 = {
  string: "Zusammenfassung"
};
const lnQAos = {
  context: "header",
  string: "Steuern"
};
const la9cZ4 = {
  string: "Tax Rate"
};
const QHB48n = {
  context: "header",
  string: "Tax Rates in {countryName}"
};
const FNKhkx = {
  string: "Versandkosten versteuern"
};
const HtQGEH = {
  string: "Successfully fetched tax rates"
};
const u3sYPH = {
  context: "independent of any particular day, eg. 11:35",
  string: "Time"
};
const QUyUJy = {
  string: "Main Product"
};
const Xtd0AT = {
  string: "Original String"
};
const vTN5DZ = {
  context: "button",
  string: "Discard"
};
const DRMMDs = {
  string: "Attribute Name"
};
const SPBLzT = {
  context: "header",
  string: 'Translation Attribute "{attribute}" - {languageCode}'
};
const JE0TAx = {
  context: "section name",
  string: "Werte"
};
const US3IPU = {
  string: "Search Engine Description"
};
const HlEpii = {
  string: "Search Engine Title"
};
const Bphmwe = {
  context: "header",
  string: 'Translation Collection "{collectionName}" - {languageCode}'
};
const GsBRWL = {
  string: "Sprachen"
};
const FemBUF = {
  context: "header",
  string: "\xDCbersetzungen nach {language}"
};
const CxfKLC = {
  string: "Seiten"
};
const VKb1MS = {
  string: "Kategorien"
};
const RzsKm8 = {
  string: "Versandarten"
};
const c8nvms = {
  string: "Angebote"
};
const ikRuLs = {
  context: "translation progress",
  string: "{current} von {max}"
};
const vcwrgW = {
  string: "No translatable entities found"
};
const LWmYSU = {
  string: "Abgeschlossene \xDCbersetzungen"
};
const X6PF8z = {
  context: "entity (product, collection, shipping method) name",
  string: "Name"
};
const ptPPVk = {
  string: "No languages found"
};
const y1Z3or = {
  string: "Sprache"
};
const oUWXLO = {
  context: "header",
  string: 'Translation Page "{pageName}" - {languageCode}'
};
const PajjqE = {
  context: "attribute list",
  string: "Attribute {number}"
};
const gvOzOl = {
  string: "Page Title"
};
const T1f2Yl = {
  string: "Variant Name"
};
const zjkAMs = {
  context: "header",
  string: 'Translation Sale "{saleName}" - {languageCode}'
};
const s40PZt = {
  string: "Angebotsbezeichnung"
};
const GpqEl5 = {
  context: "shipping method description",
  string: "Beschreibung"
};
const MKtgZB = {
  string: "Only staff users can access the dashboard"
};
const vN3qdA = {
  context: "button",
  string: "Undo"
};
const oB0y5Y = {
  context: "order status",
  string: "Noch nicht abgeschlossen"
};
const FBtqtl = {
  context: "payment status",
  string: "Unbezahlt"
};
const Lx1ima = {
  context: "button",
  string: "Bild hochladen"
};
const m8cjcK = {
  context: "add authorization key error",
  string: "Authorization key with this type already exists"
};
const aggaJg = {
  string: "This attribute has already been assigned to this product type"
};
const u24Ppd = {
  string: "Dieses Attribut kann nicht zu diesem Produkttypen hinzugef\xFCgt werden"
};
const cd13nN = {
  context: "product attribute error",
  string: "All attributes should have value"
};
const lLwtgs = {
  string: "Variants are disabled in this product type"
};
const IFWHn0 = {
  context: "error message",
  string: "Rechnungsadresse ist nicht gesetzt"
};
const ij7olm = {
  context: "error message",
  string: "This fulfillment cannot be cancelled"
};
const BM1JiJ = {
  context: "error message",
  string: "This order cannot be cancelled"
};
const nOo0oL = {
  context: "error message",
  string: "Not enough items to fulfill"
};
const Xb6BJ9 = {
  context: "error message",
  string: "Manuelle Zahlungen k\xF6nnen nicht erstattet werden"
};
const WzA5Ll = {
  string: "Cannot remove user from last group"
};
const sZ27WU = {
  context: "error message",
  string: "Nur vor-authorisierte Zahlungen k\xF6nnen freigegeben werden"
};
const gKdGxP = {
  context: "error message",
  string: "Nur vorauthorisierte Zahlungen k\xF6nnen abgerufen werden"
};
const V2BBQu = {
  string: "Currency in both channels must be the same"
};
const QFCUEt = {
  string: "Slug must be unique"
};
const AY7Tuz = {
  string: "The same object cannot be in both lists"
};
const abTH5q = {
  context: "error message",
  string: "Email address is not set"
};
const c5pMZ8 = {
  string: "API Fehler"
};
const d9UqaJ = {
  context: "error message",
  string: "Cannot change the quantity because of insufficient stock"
};
const eu98dw = {
  string: "Ung\xFCltiges Passwort"
};
const D2qihU = {
  string: "Permission is invalid"
};
const AdmPca = {
  context: "error message",
  string: "Max value cannot be less than min value"
};
const FuAV5G = {
  string: "This name is already taken. Please provide another."
};
const Wlc67M = {
  context: "error message",
  string: "Cannot choose a shipping method for an order without the shipping address"
};
const PCoO4D = {
  context: "error message",
  string: "Seite nicht gefunden"
};
const Fz3kic = {
  context: "error message",
  string: "Billing address is not set or invoice is not ready to be send"
};
const C4hCsD = {
  string: "App is out of your permissions scope"
};
const KRqgfo = {
  string: "User is out of your permissions scope"
};
const cY42ht = {
  string: "Password cannot be entirely numeric"
};
const Y1B0PN = {
  context: "error message",
  string: "F\xFCr diese Bestellung gibt es noch keine Zahlung."
};
const vVviA2 = {
  string: "Those permissions are out of your scope"
};
const mYs3tb = {
  string: "Der Produktpreis darf nicht unter 0 liegen."
};
const VEE4gD = {
  context: "error message",
  string: "Die Versandart ist f\xFCr die gew\xE4hlte Versandadresse nicht verf\xFCgbar"
};
const ychKsb = {
  context: "error message",
  string: "Shipping method is required for this order"
};
const rZf1qL = {
  context: "bulk variant create error",
  string: "SKUs must be unique"
};
const nKjLjT = {
  context: "error message",
  string: "Slug must be unique for each warehouse"
};
const wn3di2 = {
  string: "This password is too commonly used"
};
const LR3HlT = {
  string: "This password is too short"
};
const TDhHMi = {
  string: "This needs to be unique"
};
const qDwvZ4 = {
  string: "Unbekannter Fehler"
};
const DILs4b = {
  string: "Unsupported media provider or incorrect URL"
};
const Z6QAbw = {
  string: "This variant does not have any digital content"
};
const i3Mvj8 = {
  context: "product attribute error",
  string: "Diese Variante existiert bereits"
};
const iUy2dx = {
  context: "vouchers section name",
  string: "Gutscheine"
};
const ycMLN9 = {
  context: "warehouses section name",
  string: "Lagerh\xE4user"
};
const GhcypC = {
  context: "header",
  string: "Create Warehouse"
};
const DTL7sE = {
  context: "dialog content",
  string: "Are you sure you want to delete {warehouseName}?"
};
const ny4zrH = {
  context: "dialog title",
  string: "L\xF6sche Lagerhaus"
};
const YkOzse = {
  context: "used warehouses counter",
  string: "{count}/{max} warehouses used"
};
const caMMWN = {
  string: "Suche Lagerhaus"
};
const kFQvXv = {
  string: "You have reached your warehouse limit, you will be no longer able to add warehouses to your store. If you would like to up your limit, contact your administration staff about raising your limits."
};
const wmdHhD = {
  context: "button",
  string: "Create Warehouse"
};
const PFXGaR = {
  string: "Shipping Zones"
};
const aCJwVq = {
  context: "warehouse",
  string: "Name"
};
const LEZZkK = {
  context: "WarehouseSettings all warehouses description",
  string: "If selected customer will be able to choose this warehouse as pickup point. Ordered products can be shipped here from a different warehouse"
};
const M8z5WZ = {
  context: "WarehouseSettings disabled warehouse description",
  string: "If selected customer won't be able to choose this warehouse as pickup point"
};
const tVybuM = {
  context: "WarehouseSettings local warehouse label",
  string: "Local stock only"
};
const cdiRSf = {
  context: "WarehouseSettings no shipping zones assigned",
  string: "This warehouse has no shipping zones assigned."
};
const MIC9W7 = {
  context: "WarehouseSettings pickup title",
  string: "Pickup"
};
const wpli4O = {
  context: "WarehouseSettings private stock label",
  string: "Private Stock"
};
const XlPKAR = {
  context: "WarehouseSettings private stock description",
  string: "If enabled stock in this warehouse won't be shown"
};
const o5KXAN = {
  context: "delete webhook",
  string: "Sind Sie sicher, dass Sie {name} l\xF6schen wollen?"
};
const X90ElB = {
  context: "dialog header",
  string: "Delete Webhook"
};
const snUby7 = {
  context: "header",
  string: "Unnamed Webhook Details"
};
const Ryh3iR = {
  context: "header",
  string: "Create Webhook"
};
const OPtrMg = {
  context: "header",
  string: "{webhookName} Details"
};
const QEvH8Q = {
  context: "section description",
  string: "Assign permissions to register asynchronous events for this webhook."
};
const fHopox = {
  context: "section description",
  string: "Assign permissions to register synchronous events for this webhook."
};
const TjbB4Y = {
  context: "section subheader",
  string: "Asynchronous events"
};
const GLewww = {
  context: "section header",
  string: "Events"
};
const dQdxLT = {
  context: "section subheader",
  string: "Synchronous events"
};
const NPfmdK = {
  context: "webhook input label",
  string: "Secret Key"
};
const tA5HJx = {
  context: "webhook input help text",
  string: "Der geheime Schl\xFCssel wird verwendet um bei jeder Anfrage eine Hash-Signatur zu erzeugen. * optionales Feld"
};
const WDy0tF = {
  context: "section header",
  string: "Webhook Informationen"
};
const D0KaT6 = {
  context: "webhook input label",
  string: "Webhook Name"
};
const IBCBi1 = {
  context: "webhook active description",
  string: "Wenn Sie diesen Webhook deaktivieren m\xF6chten, deaktivieren Sie das folgende Kontrollk\xE4stchen."
};
const wbjuR4 = {
  string: "No webhooks found"
};
const wlr0Si = {
  context: "button",
  string: "Create Webhook"
};
const jqnwW9 = {
  context: "header",
  string: "Webhooks"
};
const OTpV1t = {
  context: "webhook name",
  string: "Name"
};
const JVaz1C = {
  context: "window title",
  string: "Create Webhook"
};
const a5msuh = {
  string: "Ja"
};
const MNZY28 = {
  context: "table column header",
  string: "Channel threshold"
};
const HcQEUk = {
  context: "table column header, sold units preorder quantity",
  string: "Sold units"
};
const XWGZLL = {
  context: "transaction reference subtitle",
  string: "Transaction reference"
};
const Y9lv8z = {
  context: "product unavailability",
  string: "Unavailable for purchase"
};
const CEavJt = {
  context: "section header",
  string: "Unlimited"
};
const Gkip05 = {
  context: "button",
  string: "Zuordnung aufheben"
};
const hAcUEl = {
  context: "product publication date label",
  string: "will become published on {date}"
};
const de = {
  PRXpBm,
  ll2dE6,
  qu8b3v,
  tQxBXs,
  RZ32u5,
  "VvFJ/T": {
    context: "PageTypeDeleteWarningDialog single no assigned items description",
    string: "Are you sure you want to delete <b>{typeName}</b>? If you remove it you won\u2019t be able to assign it to created pages."
  },
  TnyLrZ,
  "2Sx05f": {
    context: "Previous discount label",
    string: "Previous discount value"
  },
  aPqizA,
  "0em8tI": {
    context: "ProductTypeDeleteWarningDialog multiple consent label",
    string: "Yes, I want to delete those products types and assigned products"
  },
  ZFfG4L,
  bk9KUX,
  HivFnX,
  "3dVKNR": {
    context: "ProductTypeDeleteWarningDialog with items multiple description",
    string: "You are about to delete multiple product types. Some of them are assigned to products. Deleting those product types will also delete those products"
  },
  nngeI3,
  xrPv2K,
  beuxAP,
  DIrxt7,
  nfbabo,
  mDgOmP,
  "19/lwV": {
    string: "Verwaltung von Attributen zur Erstellung von Produkttypen"
  },
  "8vJCJ4": {
    string: "Verwaltung von Vertriebskan\xE4len"
  },
  hpMcW8,
  "JPH/uP": {
    string: "Arten von Inhaltsseiten im Shop verwalten"
  },
  ivJ1qt,
  n0RwMK,
  zxs6G3,
  "5BajZK": {
    string: "Verwaltung von Webseiteneinstellungen"
  },
  RQUkVW,
  EIULpW,
  "5RmuD+": {
    string: "Verwaltung von Lagerinformationen"
  },
  m19JfL,
  yJynYK,
  ojHyj3,
  sHON47,
  "nki0o/": {
    context: "replaced products list title",
    string: "Products replaced"
  },
  L5io1l,
  a1uffz,
  "undefined": {
    context: "error message",
    string: "Status ist ung\xFCltig"
  },
  "/0JckE": {
    context: "order marked as paid event title",
    string: "Order was marked as paid by"
  },
  AQSmqG,
  "/KWNJW": {
    context: "order discount was updated event title",
    string: "Order discount was updated by"
  },
  Zptsep,
  nayZY0,
  Zhxu58,
  T4wa2Y,
  BXkF8Z,
  wWTUrM,
  zWgbGg,
  By5ZBp,
  aCX8rl,
  "0opVvi": {
    context: "number of ordered products",
    string: "{amount, plural,one {Eins bestellet} other {{amount} bestellt}}"
  },
  rr8fyf,
  Q1Uzbb,
  "6L6Fy2": {
    context: "header",
    string: "Haftungsausschluss"
  },
  "5LRkEs": {
    string: "The new dashboard and the GraphQL API are preview-quality software."
  },
  G7mu0y,
  QBxN6z,
  "I+UwqI": {
    context: "is filter range or value",
    string: "gleich"
  },
  "0OtaXa": {
    context: "dialog header",
    string: "Men\xFC erstellen"
  },
  "jhh/D6": {
    string: "Men\xFC Titel"
  },
  "G/SYtU": {
    string: "M\xF6chten Sie wirklich das Menu {menuName} l\xF6schen?"
  },
  QzseV7,
  E54eoT,
  H3Uirw,
  fzDI3A,
  KKQUMK,
  Urh2N3,
  "28GZnc": {
    string: "Start typing to begin search..."
  },
  "0Vyr8h": {
    context: "menu item name",
    string: "Name"
  },
  Uf3oHA,
  dEUZg2,
  WwZfNK,
  ugnggZ,
  bj1U23,
  "svK+kv": {
    string: "{counter,plural,one {Are you sure you want to delete this menu?} other {Are you sure you want to delete {displayQuantity} menus?}}"
  },
  "1LBYpE": {
    context: "dialog header",
    string: "Men\xFCs l\xF6schen"
  },
  "OwG/0z": {
    string: "Gel\xF6schtes Men\xFC"
  },
  "0nL1D6": {
    context: "number of menu items",
    string: "Items"
  },
  DWs4ba,
  JXRYQg,
  MTl5o6,
  saKXY3,
  YI6Fhj,
  PX2zWy,
  e7yOai,
  "5Jo3C5": {
    context: "vat not included in order price",
    string: "wird nicht angewandt"
  },
  BftZHy,
  W5SK5c,
  gz9v22,
  "P/oGtb": {
    context: "product availability",
    string: "Available for purchase"
  },
  KupNHw,
  jxoMLL,
  YVIajc,
  W8i2Ez,
  "6y+k8V": {
    context: "product field",
    string: "Produktbilder"
  },
  "7JAAul": {
    context: "product field",
    string: "Produktgewicht exportieren"
  },
  QVNg8A,
  "Q/Nbku": {
    context: "product field",
    string: "Typ"
  },
  HYHLsB,
  Uo5MoU,
  "5kvaFR": {
    context: "product field",
    string: "Varianten-Artikelnummer exportieren"
  },
  XBwpUv,
  "4qe6hO": {
    context: "product stock, section header",
    string: "Lager"
  },
  SSWFo8,
  bq1eEx,
  mUb8Gt,
  "9xUIAh": {
    string: "Tax group"
  },
  OgFBAj,
  vuKrlW,
  ABgQcF,
  oIMMcO,
  Gjo89T,
  D8nsBc,
  RLBLPQ,
  qJedl0,
  ppLwx3,
  QdGzUf,
  bNw8PM,
  HVlMK2,
  YFQBs1,
  "Krzyo+": {
    context: "shipment refund title",
    string: "Shipment was refunded"
  },
  mLZMb6,
  yOaNWB,
  LsgHmZ,
  PV0SQd,
  skPoVe,
  Y7UlMR,
  "59XppT": {
    context: "button",
    string: "Approve"
  },
  "9q562c": {
    context: "apps section name",
    string: "Apps"
  },
  "D/+84n": {
    context: "snackbar text",
    string: "App activated"
  },
  USO8PB,
  YHNozE,
  Q47ovw,
  JbUg2b,
  D3E2b5,
  "W+AFZY": {
    context: "button label",
    string: "Deaktivieren"
  },
  yMi8I8,
  "73RU3R": {
    context: "deactivate app",
    string: "Are you sure you want to disable this app? Your data will be kept until you reactivate the app. You will be still billed for the app."
  },
  "7O2EsY": {
    context: "deactivate local app",
    string: "Are you sure you want to disable this app? Your data will be kept until you reactivate the app."
  },
  Np7J92,
  w6Gau0,
  LtqrM8,
  zQX6xO,
  "6hLZNA": {
    context: "delete app",
    string: "Are you sure you want to delete this app?"
  },
  "EWD/wU": {
    context: "delete app",
    string: "Deleting {name}, you will remove installation of the app. If you are paying for app subscription, remember to unsubscribe from the app in Saleor Marketplace. Are you sure you want to delete the app?"
  },
  whTEcF,
  Go50v2,
  "89PSdB": {
    context: "link",
    string: "Einstellungen bearbeiten"
  },
  a55zOn,
  Gjb6eq,
  "HtfL5/": {
    context: "button",
    string: "App \xF6ffnen"
  },
  P5twxk,
  VsGcdP,
  "7oQUMG": {
    context: "apps about permissions",
    string: "This app has permissions to:"
  },
  jDIRQV,
  MSItJD,
  WnlZMO,
  "4yRwN+": {
    context: "content",
    string: "Saleor couldn\u2019t fetch crucial information regarding installation. Without those System can\u2019t install the app in your Saleor. Please use the button below to get back to system\u2019s dashboard."
  },
  "906uUr": {
    context: "button",
    string: "Back to homepage"
  },
  k5lHFp,
  PkCmGU,
  "BL/Lbk": {
    context: "install app permissions",
    string: "Installing this app will give it following permissions:"
  },
  Id7C0X,
  UCHtG6,
  hdcGSJ,
  llC1q8,
  nIrjSR,
  "+c/f61": {
    context: "retry installation",
    string: "Retry"
  },
  JufWFT,
  "1qRwgQ": {
    context: "app installation",
    string: "Installing app..."
  },
  D4nzdD,
  flP8Hj,
  GjH9uy,
  Kxiige,
  DGCzal,
  r86alc,
  "HVFq//": {
    context: "button",
    string: "Token kopieren"
  },
  ixjvkM,
  imYxM9,
  foNlhn,
  MAsLIT,
  bsP4f3,
  "0Mg8o5": {
    context: "header",
    string: "Tokens"
  },
  RMB6fU,
  "0DRBjg": {
    string: "Token Note"
  },
  VHuzgq,
  XB2Jj9,
  voRaz3,
  "5+Xcrz": {
    context: "app deactivated",
    string: "Deactivated"
  },
  TBaMo2,
  "9tgY4G": {
    context: "apps content",
    string: "You don\u2019t have any installed apps in your dashboard"
  },
  ZeD2TK,
  SwISVH,
  NskBjH,
  wxFwUW,
  LATpSE,
  k0rGBI,
  t9a9GQ,
  T5nU7u,
  isM94c,
  quV5zH,
  "2VSP8C": {
    context: "delete token",
    string: "Are you sure you want to delete token {token}?"
  },
  "ac+Y98": {
    context: "app settings error",
    string: "Failed to fetch app settings"
  },
  "2cjt25": {
    context: "window title",
    string: "Install App"
  },
  "5t/4um": {
    context: "message title",
    string: "Couldn\u2019t Install {name}"
  },
  "0fM/pV": {
    context: "message title",
    string: "App installed"
  },
  ZprV2g,
  uIPD1i,
  agZQkB,
  oiuwOl,
  l2oVCF,
  eWV760,
  "J/QqOI": {
    string: "This value already exists within this attribute"
  },
  "lG/MDw": {
    context: "dialog content",
    string: "{counter,plural,one {Are you sure you want to delete this attribute?} other {Are you sure you want to delete {displayQuantity} attributes?}}"
  },
  rKf4LU,
  JI2Xwp,
  h1rPPg,
  "5XG1CO": {
    context: "acre-ft unit",
    string: "acre-ft"
  },
  jBu2yj,
  A9QSur,
  xOEZjV,
  P79U4b,
  Q7uuDr,
  l5V0QT,
  "fU+a9k": {
    context: "date attribute type",
    string: "Datum"
  },
  DzPVnj,
  "k/mTEl": {
    context: "distance units type",
    string: "Distance"
  },
  bZksto,
  LnRlch,
  z1y9oL,
  YgE6ga,
  oIvtua,
  ZayvsI,
  cKjFfl,
  SNiyXb,
  Iafyt5,
  B0PaVS,
  "5TUpjG": {
    context: "product attribute entity type",
    string: "Produkte"
  },
  "5dLpx0": {
    context: "references attribute type",
    string: "References"
  },
  PiSXjb,
  gx4wCT,
  fdbqIs,
  Orgqv4,
  zWM89r,
  ghje1I,
  njBulj,
  cy8sV7,
  Vdy5g7,
  dKPMyh,
  IGvQ8k,
  "1div9r": {
    string: "Search Attribute"
  },
  "PsRG+v": {
    context: "use attribute in filtering",
    string: "Filterable in Storefront"
  },
  rvk9ls,
  HQR2y0,
  cvbqJu,
  ztQgD8,
  yKuba7,
  oJkeS6,
  k6WDZl,
  HjUoHK,
  ErNH3D,
  nwvQPg,
  v1pNHW,
  zbJHl7,
  qkRuT0,
  "8cUEPV": {
    context: "page title",
    string: "Create New Attribute"
  },
  jswILH,
  AzMSmb,
  lCxfDe,
  "RH+aOF": {
    context: "use attribute in filtering",
    string: "Use in Filtering"
  },
  Q9wTrz,
  AgY5Mv,
  cJ5ASN,
  "x8V/xS": {
    context: "attribute visibility in storefront",
    string: "Public"
  },
  h2Hta6,
  I2wCwj,
  "0DgA8v": {
    context: "swatch attribute color picker label",
    string: "Picker"
  },
  JyQoES,
  WWV8aZ,
  no3Ygn,
  XYhE8p,
  PqMbma,
  UhcALJ,
  H60H6L,
  "3psvRS": {
    context: "attribute values list: slug column header",
    string: "Admin"
  },
  g5zIpS,
  J3uE0t,
  "+iVKR1": {
    context: "assign attribute value button",
    string: "Assign value"
  },
  NUevU9,
  "jTifz+": {
    string: "Attribut erfolgreich angelegt"
  },
  "7H2D5m": {
    context: "attribute value deleted",
    string: "Value deleted"
  },
  "V/VAHG": {
    string: "Attribute deleted"
  },
  xVn5B0,
  lw9WIk,
  vzgZ3U,
  "AubJ/S": {
    context: "button",
    string: "Anmelden"
  },
  "5sg7KC": {
    string: "Passwort"
  },
  "3tbL7x": {
    context: "description",
    string: "Passwort vergessen?"
  },
  ENBELI,
  M4q0Ye,
  tTtoKd,
  ChGI4V,
  Ev6SEF,
  S22jIs,
  "vfG+nh": {
    string: "Passwort best\xE4tigen"
  },
  "m0Dz+2": {
    string: "Please set up a new password for your account. Repeat your new password to make sure you will be able to remember it."
  },
  "7Chrsf": {
    string: "Passw\xF6rter stimmen nicht \xFCberein"
  },
  WhKGPA,
  lm9NSK,
  "Yy/yDL": {
    string: "Passwort zur\xFCcksetzen"
  },
  "54M0Gu": {
    string: "Provide us with an email - if we find it in our database we will send you a link to reset your password. You should be able to find it in your inbox in the next couple of minutes."
  },
  "2oyWT9": {
    context: "button",
    string: "Zur\xFCck zur Anmeldung"
  },
  "2ob30/": {
    string: "Success! In a few minutes you\u2019ll receive a message with instructions on how to reset your password."
  },
  C0JLNW,
  hOxIeP,
  "0OfZJA": {
    context: "button",
    string: "Zur\xFCck"
  },
  "7+GBlj": {
    string: "Error code {errorCode} {fieldError}"
  },
  "9uNz+T": {
    context: "button",
    string: "Abbrechen"
  },
  dOQB9o,
  o8f4Sg,
  GOdq5V,
  sK1FPw,
  DP6b8U,
  "0iMYc+": {
    context: "field is optional",
    string: "(Optional)"
  },
  "cgsY/X": {
    context: "page header",
    string: "Kategorie anlegen"
  },
  wQdR8M,
  xo5UIb,
  dJQxHt,
  vEYtiq,
  "8HRy+U": {
    string: "Kategorie Beschreibung"
  },
  vof5TR,
  JiXNEV,
  vy7fjd,
  dM86a2,
  rrbzZt,
  BHQrgz,
  k8ZJ5L,
  VQLIXd,
  "x/pIZ9": {
    context: "button",
    string: "Produkt hinzuf\xFCgen"
  },
  z8jo8h,
  "+43JV5": {
    context: "header",
    string: "Produkte in {categoryName}"
  },
  JDz5h8,
  "V+fkAO": {
    context: "number of products in category",
    string: "Produkte"
  },
  NivJal,
  UycVMp,
  Irflxf,
  "7l5Bh9": {
    string: "{counter,plural,one {Wollen Sie dieses Produkt wirklich l\xF6schen?} other {Wollen Sie wirklich {displayQuantity} Produkte l\xF6schen?}}"
  },
  xl7Fag,
  "Pp/7T7": {
    string: "{counter,plural,one {Wollen Sie diese Kategorie wirklich l\xF6schen?} other {Wollen Sie wirklich {displayQuantity} Kategorien l\xF6schen?}}"
  },
  HvJPcU,
  KCjd1o,
  "3DGvA/": {
    string: "Remember this will also unpin all products assigned to this category, making them unavailable in storefront."
  },
  "e+L+q3": {
    string: "Dies wird auch alle Produkte in dieser Kategorie l\xF6schen."
  },
  sG0w22,
  xRkj2h,
  KeO51o,
  NlSJMM,
  "37U5su": {
    context: "all variants label",
    string: "All variants"
  },
  "1w06LC": {
    context: "variants count label",
    string: "{variantsCount} variants"
  },
  "8qL/tV": {
    context: "CannotDefineChannelsAvailabilityCard subtitle",
    string: "You will be able to define availability of product after creating variants."
  },
  CT5PAn,
  QZoU0r,
  "Mz0cx+": {
    context: "delete channel",
    string: "Deleting channel will delete all product data regarding this channel. Are you sure you want to delete this channel?"
  },
  sidKce,
  BXMSl4,
  SZJhvK,
  "3y4r+z": {
    context: "channel settings",
    string: "Channel Settings"
  },
  "39yi8w": {
    context: "selected currency",
    string: "Selected currency"
  },
  "74Zo/H": {
    context: "channel slug",
    string: "Slug"
  },
  ZhaXLU,
  "9Sz0By": {
    context: "channel currency",
    string: "Currency"
  },
  "tV+Dcm": {
    string: "Default country"
  },
  UymotP,
  "G/pgG3": {
    context: "dialog header",
    string: "Select a channel"
  },
  nKwgxY,
  X8qjg3,
  "+tIkAe": {
    context: "status",
    string: "Status"
  },
  MHVglr,
  TSJRiZ,
  QiN4hv,
  MQwT1W,
  "p/EWEZ": {
    context: "channels variants availability dialog title",
    string: "Manage Channels"
  },
  yHaQWG,
  "8CbACQ": {
    context: "add shipping zone title",
    string: "Add Shipping Zones"
  },
  "+G9l7u": {
    context: "all selected zones card message",
    string: "All available shipping zones have been selected"
  },
  Ic7Wln,
  gtKcPf,
  PTW56s,
  rZMT44,
  "j/vV0n": {
    context: "channel name",
    string: "Channel Name"
  },
  "/glQgs": {
    string: "No channels found"
  },
  OGm8wO,
  ZMy18J,
  J7mFhU,
  DnghuS,
  "OrMr/k": {
    context: "window title",
    string: "Create Channel"
  },
  "D9Rg+F": {
    context: "window title",
    string: "Channel details"
  },
  AkyGP2,
  eWcvOc,
  "2FQsYj": {
    context: "button",
    string: "Zur\xFCcksetzen"
  },
  "Qox+kb": {
    string: "on field {fieldName}"
  },
  NNT3Lp,
  "9vQR6c": {
    context: "collection label",
    string: "Sichtbar"
  },
  V8FhTt,
  Rj8LxK,
  Fxa6xp,
  "/WXs6H": {
    context: "collection name",
    string: "Name"
  },
  G4g5Ii,
  s97tLq,
  jyaAlB,
  "9eC0MZ": {
    context: "collection",
    string: "Versteckt"
  },
  lL3YJO,
  "Yw+9F7": {
    string: "Keine Kollektion gefunden."
  },
  mWQt3s,
  VZsE96,
  UxdBmI,
  dpY94C,
  FcVEpe,
  Mee46w,
  "k+HcTv": {
    context: "product type",
    string: "Typ"
  },
  Oe62bR,
  "/dnWE8": {
    context: "products in collection",
    string: "Produkte in {name}"
  },
  scHVdW,
  "6AMFki": {
    context: "product name",
    string: "Name"
  },
  Q8wHwJ,
  "56vUeQ": {
    string: "Produkt zu Kollektion hinzugef\xFCgt"
  },
  MxhVZv,
  "AulH/n": {
    string: "{counter,plural,one {Wollen Sie wirklich die Zuordnung von {displayQuantity} Produkten aufheben?} other {Wollen Sie wirklich die Zuordnung von {displayQuantity} Produkten aufheben?}}"
  },
  "WW+Ruy": {
    string: "Produkt von Kollektion entfernt."
  },
  I1Mz7h,
  "5OtU+V": {
    context: "dialog title",
    string: "Zuordnung der Produkte zur Kollektion aufheben"
  },
  pVFoOk,
  ttMauu,
  "+wpvnk": {
    context: "dialog title",
    string: "Kollektion l\xF6schen"
  },
  "67V0c0": {
    context: "unassign product from collection, button",
    string: "Zuordnung aufheben"
  },
  fzk04H,
  yT5zvU,
  Ykw8k5,
  C7eDb9,
  Fbr4Vp,
  "6cS4Rd": {
    context: "card section description",
    string: "Available permissions"
  },
  MVU6ol,
  VmMDLN,
  "vONi+O": {
    string: "Land"
  },
  O95R3Z,
  "B52Em/": {
    string: "Adresszeile 1"
  },
  TE4fIS,
  oYGfnY,
  oQY0a2,
  "9YazHG": {
    string: "Unternehmen"
  },
  AuwpCm,
  "Nv/toB": {
    context: "button",
    string: "Assign and save"
  },
  PkjQS6,
  fP9FXB,
  auxEP1,
  QM9P8G,
  ylobu9,
  GUlwXU,
  RoKOQJ,
  NsgWhZ,
  sW9IyX,
  "8hrH/z": {
    context: "dialog header",
    string: "Kategorie suchen"
  },
  sf6FMK,
  XLYssG,
  "5tMkmJ": {
    context: "dialog header",
    string: "Kollektion zuweisen"
  },
  f0hXz4,
  JiRKgJ,
  cryH86,
  "/TF6BZ": {
    string: "Suche Produkte"
  },
  SHm7ee,
  dTCDMn,
  WQnltU,
  "p4X/0H": {
    context: "button, assign variants to sale and save",
    string: "Assign and save"
  },
  "K+vjtE": {
    string: "Search Variants"
  },
  xAHOGV,
  "+HuipK": {
    context: "variant sku",
    string: "SKU {sku}"
  },
  "QZVD+5": {
    context: "button, unassign attribute from object",
    string: "Unassign and save"
  },
  "8uLHBY": {
    context: "unassign attribute from object",
    string: "Are you sure you want to unassign {attributeName} from {itemTypeName}?"
  },
  "z0gGP+": {
    context: "number of attributes",
    string: "{number} Eigenschaften"
  },
  "3ukd9/": {
    context: "attributes, section header",
    string: "Attribute"
  },
  j8PV7E,
  "Ediw6/": {
    context: "button label",
    string: "Assign references"
  },
  fgHLXc,
  jHJmjf,
  XwrKOi,
  B2LE7A,
  "5A6/2C": {
    context: "section header",
    string: "Verf\xFCgbarkeit"
  },
  vY2lpx,
  "2i81/P": {
    context: "section header button",
    string: "Manage"
  },
  "0cVk9I": {
    string: "Show in product listings"
  },
  UjsI4o,
  Jt3DwJ,
  "5ukAFZ": {
    string: "Entfernen Sie diese Markierung, um das Produkt von der Suche und Kategorieseiten zu entfernen. Es wird weiterhin auf Kollektionsseiten verf\xFCgbar sein."
  },
  Y7Vy19,
  U3BQKA,
  tQuE1q,
  B9yrkK,
  "2/L4zZ": {
    string: "Select all channels"
  },
  EWCUdP,
  ybaLoZ,
  "/lBLBI": {
    context: "channels alphabetically title",
    string: "Channels from A to Z"
  },
  PctLol,
  zR9Ozi,
  "7scATx": {
    context: "select title",
    string: "Select channels you want for {contentType} to be available on"
  },
  cFVgOo,
  T0Mfxq,
  JgXBAw,
  sdA14A,
  GzbSQk,
  s2y5eG,
  rHoRbE,
  TOMgXz,
  "62Ywh2": {
    context: "number of countries",
    string: "{number} L\xE4nder"
  },
  zZCCqz,
  "7NfoiJ": {
    context: "custom search delete, dialog header",
    string: "Delete Search"
  },
  UaYJJ8,
  LmKz3g,
  "/X8Mjx": {
    string: "<emphasis>GraphQL API</emphasis> ausprobieren"
  },
  xwEc8K,
  "4gZl/n": {
    string: "See <emphasis>DEMO STOREFRONT</emphasis>"
  },
  "7v2oBd": {
    context: "header",
    string: "Fehler"
  },
  "9mGA/Q": {
    context: "button linking to dashboard",
    string: "Dashboard"
  },
  ypW4Mi,
  p0SUJj,
  "AQ+9Ez": {
    string: "We\u2019ve encountered an unexpected error"
  },
  "OA+fw+": {
    context: "conjunction, choice between going to dashboard or refreshing page",
    string: "or"
  },
  QCwBUI,
  qIgdO6,
  DEa1T1,
  zSOvI0,
  HnVtSS,
  "PLCwT/": {
    context: "search results",
    string: "Show more"
  },
  FNpv6K,
  erC44f,
  "34F7Jk": {
    context: "filter range separator",
    string: "and"
  },
  USS3Q7,
  XkX56I,
  sn2awN,
  MPOj0I,
  "1eEyJv": {
    context: "ExitFormPrompt continue editing button",
    string: "Continue editing"
  },
  "TtZg/K": {
    context: "ExitFormPrompt title",
    string: "Would you like to save changes?"
  },
  FWWliu,
  NxeDbG,
  "62T585": {
    context: "button",
    string: "{languageName} - {languageCode}"
  },
  LkuDEb,
  "VcI+Zh": {
    context: "header",
    string: "Metadata"
  },
  ETHnjq,
  "2+v1wX": {
    context: "number of metadata fields in model",
    string: "{number,plural,one {{number} string} other {{number} strings}}"
  },
  nudPsY,
  cY6H2C,
  GiDxS4,
  nEixpu,
  hptDxW,
  zTdwWM,
  lW5uJO,
  U2WgwW,
  hX5PAb,
  "EEW+ND": {
    string: "Navigator"
  },
  "8B8E+3": {
    context: "navigator placeholder",
    string: "Order Number"
  },
  NqxvFh,
  TpPx7V,
  "4gT3eD": {
    context: "navigator section header",
    string: "Search in Customers"
  },
  BooQvo,
  AOI4LW,
  YYkkhx,
  "EM+30g": {
    context: "navigator notification",
    string: "Our new feature to help you with your daily tasks. Run Navigator using {keyboardShortcut} shortcut."
  },
  me585h,
  Gxm7Qx,
  ccXLVi,
  phAZoj,
  M1uijW,
  CjSRT1,
  VdDcxc,
  "QooeI/": {
    context: "button",
    string: "Kunden anlegen"
  },
  cfQf0w,
  V1mqpZ,
  JFmOfi,
  y9cvqE,
  "1gzck6": {
    string: "{firstName} {lastName}"
  },
  IyHQr0,
  kbkjEc,
  "3TlhJS": {
    context: "navigator command mode description",
    string: "Search Command"
  },
  DHBlFi,
  "Xel9C+": {
    context: "navigator default mode description",
    string: "Search Views and Actions"
  },
  iAvKNf,
  usSkzP,
  k8bltk,
  "x/ZVlU": {
    string: "Produkt"
  },
  "yH56V+": {
    string: "Ooops!..."
  },
  bj6pTd,
  "nRiOg+": {
    string: "Sorry, the page was not found"
  },
  "95oJ5d": {
    context: "button",
    string: "Zur\xFCck zum Dashboard"
  },
  p3eRUm,
  KGsaGO,
  "WHkx+F": {
    string: "Price cannot be lower than 0"
  },
  fNFEkh,
  liLrVs,
  QcIFCs,
  w2Cewo,
  CXTIq8,
  s5Imt5,
  TGX4T1,
  IoDlcd,
  ChAjJu,
  "s/sTT6": {
    string: "Wenn diese Feld nicht ausgef\xFCllt wird, zeigt die Vorschau was automatisch generiert wird."
  },
  "450Fty": {
    string: "None"
  },
  "qu/hXD": {
    string: "Selected {number} items"
  },
  "2HfSiT": {
    context: "pagination",
    string: "No. of rows"
  },
  "v/1VA6": {
    context: "add order note, button",
    string: "Send"
  },
  "3evXPj": {
    string: "Leave your note here..."
  },
  zyXcBP,
  qLbse5,
  "X8+Lpa": {
    context: "button",
    string: "Einstellungen"
  },
  "2r4cTE": {
    context: "button",
    string: "Enable Dark Mode"
  },
  akXDST,
  qMB6d2,
  "5x6yT9": {
    context: "weight",
    string: "{fromValue} {fromUnit} - {toValue} {toUnit}"
  },
  LICZeR,
  NtFVFS,
  s8FlDW,
  w9xgN9,
  xfGZsi,
  YZl6cv,
  gTr0qE,
  "HP6m+q": {
    string: "Attributes and Product Types"
  },
  "UN+yTt": {
    string: "Staff Settings"
  },
  HjXnIf,
  jFrdB5,
  MWSacl,
  DJFPzq,
  yHeZRQ,
  CJEIRC,
  JTcz2G,
  ryAyPr,
  "i+Vox0": {
    context: "invoice generating has finished, header",
    string: "Invoice Generated"
  },
  Dhherd,
  "8aV98y": {
    context: "new version notification content",
    string: "You need to update Saleor to the newest version. Before doing so - save all changes to prevent loss of data. To update use the button below."
  },
  tEdFmZ,
  XCIV8H,
  Rjs1CD,
  H5NKfr,
  "w4R/SO": {
    string: "Local Apps"
  },
  "McN+wq": {
    context: "customers section name",
    string: "Kunden"
  },
  gQGUsN,
  "W0kQd+": {
    context: "dialog title",
    string: "Adresse hinzuf\xFCgen"
  },
  "rjy9/k": {
    context: "button",
    string: "Adresse hinzuf\xFCgen"
  },
  kErneR,
  n5vskv,
  MpR4zK,
  "y/UWBR": {
    string: "There is no address to show for this customer"
  },
  CWqmRU,
  hMRP6J,
  VyzsWZ,
  nLML8Y,
  puikeb,
  "w+8BfK": {
    context: "button",
    string: "Adresse bearbeiten"
  },
  hLOEeb,
  "+7OsyM": {
    context: "button",
    string: "Als standard Versandadresse setzen"
  },
  BfJGij,
  Zd3Eew,
  bHdFph,
  "3d1RXL": {
    string: "This customer has no addresses yet"
  },
  biVFKU,
  jGGnSZ,
  "wNQzS/": {
    string: "The primary address of this customer."
  },
  fjPWOA,
  qNcoRY,
  "uUQ+Al": {
    context: "note about customer",
    string: "Notiz"
  },
  w3sGrD,
  N76zUg,
  "+NUzaQ": {
    context: "check to mark this account as active",
    string: "User account active"
  },
  MjUyhA,
  SMakqb,
  "4v5gfh": {
    context: "account information, header",
    string: "Account Information"
  },
  "2mRLis": {
    string: "Search Customer"
  },
  QLVddq,
  xQK2EC,
  "icz/jb": {
    context: "customer",
    string: "Join Date"
  },
  fhksPD,
  E8VDeH,
  FpIcp9,
  "97l2MO": {
    string: "Kundenemail"
  },
  Gr1SAu,
  pURrk1,
  "1LiVhv": {
    context: "section header",
    string: "Letzte Bestellungen"
  },
  nTF6tG,
  "3+990c": {
    context: "button",
    string: "Alle Bestellungen anzeigen"
  },
  ri3kK9,
  "taX/V3": {
    context: "order total amount",
    string: "Gesamtsumme"
  },
  RlfqSV,
  FNAZoh,
  "HMD+ib": {
    string: "Letzte Bestellung"
  },
  e7Nyu7,
  "2p0tZx": {
    context: "delete customer, dialog content",
    string: "Are you sure you want to delete {email}?"
  },
  ey0lZj,
  qLOBff,
  nX2pCU,
  "/kWzY1": {
    string: "Are you sure you want to delete this address from users address book?"
  },
  PXatmC,
  ftcHpD,
  N2SbNc,
  q8ep2I,
  hzSNj4,
  P7PLVj,
  JqiqNj,
  ufmuTp,
  i0AcKY,
  Q8Qw5B,
  bZenNC,
  "n+Gwbu": {
    string: "Rabatte"
  },
  g3qjSf,
  AbyDC7,
  Uu76vj,
  fV6yX5,
  QGjJcT,
  "/6uK4C": {
    context: "button",
    string: "Kollektion zuweisen"
  },
  XNeJay,
  IoCMjg,
  ht9yOD,
  "cvVIV/": {
    context: "dialog header",
    string: "Assign Countries"
  },
  dGqEJ9,
  "dWK/Ck": {
    string: "Choose countries, you want voucher to be limited to, from the list below"
  },
  "8EGagh": {
    context: "search box label",
    string: "Filter Countries"
  },
  wgA48T,
  AVF5T5,
  zKOGkU,
  U8eeLW,
  "xqXYF+": {
    context: "section header",
    string: "Eligible Products"
  },
  OrR3Qy,
  "6cMkfT": {
    context: "table head",
    string: "Produktname"
  },
  bPFp8B,
  ySqrUU,
  lehGc9,
  "/oyr6M": {
    context: "no variants",
    string: "Keine Varianten gefunden."
  },
  Q1HhPk,
  lHccsy,
  "2E1xZ0": {
    context: "page header",
    string: "Angebot erstellen"
  },
  F56hOz,
  "MSD3A/": {
    string: "Angebot suchen"
  },
  JHfbXR,
  Yjhgle,
  AnqH4p,
  "1BNKCZ": {
    context: "sale channel",
    string: "Channel"
  },
  RBxYJf,
  "XDBeA+": {
    context: "discount type",
    string: "Fixed amount"
  },
  s17U7u,
  BanAhF,
  zjHH6b,
  SpngiS,
  KHZlmi,
  XZR590,
  giF5UV,
  "51HE+Q": {
    string: "Keine Angebote gefunden"
  },
  iBSq6l,
  "WkxE8/": {
    context: "percentage or fixed, header",
    string: "Discount Type"
  },
  JnzDrI,
  wHdMAX,
  x3g4Ry,
  cehiWu,
  Hj3T7P,
  PsclSa,
  YjcN9w,
  "jd/LWa": {
    string: "Gutschein gilt f\xFCr alle L\xE4nder"
  },
  glT6fm,
  ibnmEd,
  jvKNMP,
  mSLr9d,
  vTgRTZ,
  "Qj/3sH": {
    context: "limit voucher",
    string: "Limit number of times this discount can be used in total"
  },
  "+jHXT3": {
    context: "limit voucher",
    string: "Limit to staff only"
  },
  s51tHd,
  "pzSF+b": {
    context: "voucher usage limit, header",
    string: "Nutzungsbeschr\xE4nkung"
  },
  o8S0Ac,
  pNrF72,
  IruP2T,
  GbhZJ4,
  amQg6f,
  NLybdq,
  t7UwLY,
  Jj0de8,
  ujFo4A,
  "uy+tB8": {
    context: "voucher status",
    string: "Status"
  },
  h75GAF,
  "JV+EiM": {
    context: "voucher value",
    string: "Wert"
  },
  tuYPlG,
  b6L9n7,
  yHwvLL,
  "5u7b3V": {
    context: "voucher is active from date",
    string: "Started"
  },
  JsPIOX,
  U2mOqA,
  GVinbz,
  "K+ROF8": {
    string: "Channels that don\u2019t have assigned discounts will use their parent channel to define the price. Price will be converted to channel\u2019s currency"
  },
  yhv3HX,
  "u/hkKO": {
    context: "voucher has no requirements",
    string: "None"
  },
  "XT/ZvF": {
    context: "voucher requirement",
    string: "Minimum quantity of items"
  },
  "bh9+8A": {
    context: "voucher requirement",
    string: "Minimal order value"
  },
  "FOa+Xd": {
    context: "voucher value requirement",
    string: "Min. Order Value"
  },
  bcf60I,
  HLqWXA,
  "6cq+c+": {
    context: "header",
    string: "Discount Type"
  },
  sS5aVm,
  fEfCtO,
  vXFPD6,
  "1shOIS": {
    context: "column title",
    string: "Preis"
  },
  "/oaqFS": {
    context: "section header",
    string: "Wert"
  },
  mmcHeH,
  "5c2JVF": {
    context: "voucher application, switch button",
    string: "Only once per order"
  },
  "9UHfux": {
    string: "Gutschein Spezifische Informationen"
  },
  ObRk1O,
  bP7ZLP,
  "45zP+r": {
    context: "voucher discount",
    string: "Bestimmte Produkte"
  },
  WasHjQ,
  n7Fg8i,
  "ESDTC/": {
    string: "Manage Sales Channel Availability"
  },
  SqcGBK,
  "4C7I61": {
    context: "sale Details delete button",
    string: "Angebot entfernt"
  },
  V3fvcD,
  "1/oG76": {
    context: "dialog header",
    string: "Angebot l\xF6schen"
  },
  "/V7UOC": {
    context: "unassign category from sale and save, button",
    string: "Unassign and save"
  },
  GiJm1v,
  B5yE8S,
  "6RtE2D": {
    context: "unassign collection from sale and save, button",
    string: "Unassign and save"
  },
  UjoSZB,
  "0TQuo4": {
    context: "dialog header",
    string: "Unassign Collection From Sale"
  },
  rNzkIt,
  opIDox,
  AHK0K9,
  n7GIKB,
  Uj2hj8,
  iWyoZn,
  Kyyr7D,
  ZWIjvr,
  FPzzh7,
  Eau5AV,
  Q8mpW3,
  MmGlkp,
  cNSLLO,
  cKCfSW,
  LOSNq0,
  NEJo1I,
  EM730i,
  Hgz44z,
  O9QPe1,
  Q0JJ4F,
  eOrLzG,
  toDL5R,
  YMBn8d,
  "2atspc": {
    string: "Entw\xFCrfe"
  },
  Ja7gHc,
  hJZwTS,
  T4GOiX,
  "juBV+h": {
    string: "End Hour"
  },
  KN7zKn,
  j0ugM4,
  Q6wcZ5,
  pkjXPD,
  pkUbrL,
  "HSmg1/": {
    context: "gift cards section name",
    string: "Gift Cards"
  },
  mbj1J5,
  "3bQz2o": {
    context: "bulk issue gift cards success alert title",
    string: "Gift Cards Issued"
  },
  "7dhhzL": {
    context: "bulk issue gift cards dialog title",
    string: "Bulk Issue Gift Cards"
  },
  YBqLHs,
  fzl482,
  "rl1t+o": {
    context: "expires in label",
    string: "Expires in"
  },
  rzhcpD,
  n9JOI3,
  vDnheO,
  NZtcLb,
  IVOjqW,
  WyPitj,
  hnBvH7,
  RXbkle,
  zjZuhM,
  WzHfj8,
  MgdgpT,
  uilt7q,
  PilTI6,
  UKgP89,
  ZuqkSp,
  ArctEg,
  vCw7BP,
  JftRtx,
  uQk8gB,
  n97Ii0,
  CRAfpd,
  "dsJ+Wv": {
    context: "note on export gift cards",
    string: "Note: Only active and not used gift cards will be expored"
  },
  "3kwdxJ": {
    context: "gift card export type label",
    string: "gift cards"
  },
  bDHiYK,
  YEpYMB,
  DQJnB4,
  KqbfFa,
  "1J/bhZ": {
    context: "expiry date section header",
    string: "Expiry date"
  },
  xHj9Qe,
  "fExm0/": {
    context: "gift card history message",
    string: "Gift card was activated by {activatedBy}"
  },
  pCy5EP,
  aEc9Ar,
  PcQRxi,
  gAqkrG,
  "NvwS/N": {
    context: "gift card history message",
    string: "Gift card was deactivated"
  },
  vQunFc,
  fLhj3a,
  "4Z0O2B": {
    context: "section header title",
    string: "Gift Card Timeline"
  },
  "30X9S8": {
    context: "gift card history message",
    string: "Gift card was issued by {issuedBy}"
  },
  jDovoJ,
  JgNb8X,
  WS4ov0,
  gj3MUg,
  tsL3IW,
  vkAWwY,
  Uu2B2G,
  "408KSO": {
    context: "gift card history message",
    string: "Gift card was used as a payment method on order {orderLink}"
  },
  "v01/tY": {
    context: "consent to send gift card to different address checkbox label",
    string: "Yes, I want to send gift card to different address"
  },
  ttk0w7,
  AqHafs,
  NLNonj,
  s1IQuN,
  "JQH+Iy": {
    context: "resent code success message",
    string: "Successfully resent code to customer!"
  },
  mslSpp,
  STJx8N,
  kFkMoG,
  tKxCld,
  XAPFHP,
  "Z/7hyu": {
    context: "card balance label",
    string: "Card Balance"
  },
  PJDcQs,
  "a+kgkq": {
    context: "tag label",
    string: "Card Tag"
  },
  "Zj/7QZ": {
    context: "details title",
    string: "Details"
  },
  "//k1GX": {
    context: "expired on label",
    string: "Expired on {date}"
  },
  JKbpH9,
  nFTvQW,
  Jo01VZ,
  "/AbHy4": {
    context: "creation label",
    string: "Creation"
  },
  "+pLi+M": {
    context: "issued by app label",
    string: "Issued by app"
  },
  "V+h02n": {
    context: "issued by label",
    string: "Issued by"
  },
  "9TbDQD": {
    context: "order number label",
    string: "Order number"
  },
  kc4c4d,
  Eh6KYS,
  kS5Qgk,
  EA7rjI,
  aPYFO1,
  lCPxtT,
  GibKGn,
  bDO9DN,
  "29L5Yq": {
    context: "gift card not found message",
    string: "Couldn't find gift card"
  },
  xPnZ0R,
  kcMVsB,
  "bVbEZ/": {
    context: "amount filter label",
    string: "Betrag"
  },
  tXIgmR,
  osPBn1,
  "e/61NZ": {
    context: "current balance filter label",
    string: "Current balance"
  },
  tTuCYj,
  "+WTmpr": {
    context: "disabled status option label",
    string: "Disabled"
  },
  vC8vyb,
  VceXrc,
  Sjd7wm,
  labkPK,
  D4CsYK,
  "mE+fru": {
    context: "tag filter label",
    string: "Tags"
  },
  WMGoqz,
  "31hYP2": {
    context: "alert card message",
    string: "Gift cards will appear after their order is fullfilled. <link>View Orders with Gift Cards</link>"
  },
  "qkt/Km": {
    context: "bulk delete label",
    string: "L\xF6schen"
  },
  IzEVek,
  "hz+9ES": {
    context: "bulk activate label",
    string: "Aktivieren"
  },
  KcsJKm,
  C90nKP,
  IwEQvz,
  SO56cv,
  "9hab/1": {
    context: "bulk issue menu item",
    string: "Bulk Issue"
  },
  "38dS1A": {
    context: "code ending with label",
    string: "Code ending with {last4CodeChars}"
  },
  HqeqEV,
  "8Hq18g": {
    context: "create gift card product type alert message",
    string: "Create a gift card product type"
  },
  "exvX+Z": {
    context: "export card codes menu item",
    string: "Export card codes"
  },
  rCy3Fe,
  "l1/Hwb": {
    context: "invalid date in expirydate field header",
    string: "Incorrect date entered"
  },
  AJgl5u,
  MbZHXE,
  MJBAqv,
  eLJQSh,
  bwJc6V,
  "FEWgh/": {
    context: "column title tag",
    string: "Tag"
  },
  RfPJ1E,
  E22x4H,
  Rd0s80,
  "VI+X8H": {
    context: "no gift card product types alert message",
    string: "{createGiftCardProductType} to start selling gift cards in your store."
  },
  jmh0rV,
  U9o2bV,
  F69lwk,
  vWTM6Y,
  Utq1fE,
  mbOuGl,
  vzce9B,
  FQc5z6,
  Yxihwg,
  S52JMl,
  zLtb4N,
  "a+iRI1": {
    context: "single gift card title",
    string: "{selectedItemsCount,plural,one {Delete Gift Card} other {Delete Gift Cards}}"
  },
  RLZ1jd,
  LA13a5,
  "0sd4ez": {
    context: "selected customer channel subtitle",
    string: "Customer will be sent the gift card code via this channels email address"
  },
  FRJRmi,
  Um3g00,
  "ps+h1G": {
    context: "checkbox label description",
    string: "Expiration date will be automatically set, once gift card is issued"
  },
  j2l6cL,
  XFtKV5,
  mzASqs,
  "54KYgS": {
    context: "months after label",
    string: "months after issue"
  },
  "15fg+o": {
    context: "weeks after label",
    string: "weeks after issue"
  },
  Qe4XHv,
  "4JW9iJ": {
    context: "home section name",
    string: "Start"
  },
  sjRXXz,
  BNTZLv,
  "5SPHkk": {
    string: "Order #{orderId} was fully paid"
  },
  "0dPP8O": {
    string: "Bestellung #{orderId} wurde aufgegeben"
  },
  "Nuq83+": {
    string: "Create new channel"
  },
  E9Jssl,
  "5dyOs0": {
    string: "Keine ausstehenden Zahlungen"
  },
  bFhzRX,
  c0H45L,
  md326v,
  cdxwA8,
  gSQ0Ge,
  "26+K4N": {
    string: "There was a poblem with the file you uploaded as an image and it couldn't be used. Please try a different file."
  },
  "Yo2kC+": {
    string: "Couldn't process image"
  },
  DOkfyZ,
  aheQdn,
  "8oIbMI": {
    string: "Reached limit for this plan"
  },
  IbVKSH,
  "9C7PZE": {
    context: "navigation section name",
    string: "Navigation"
  },
  wlQTfb,
  oUWADl,
  s9sOcC,
  lzdvwp,
  Ta9j04,
  "+svQBN": {
    context: "alert",
    string: "Order limit reached"
  },
  TPW2tP,
  aY0HAT,
  NJbzcP,
  "i+JSEZ": {
    string: "{counter,plural,one {Are you sure you want to cancel this order?} other {Are you sure you want to cancel {displayQuantity} orders?}}"
  },
  VSztEE,
  NhQboB,
  "b+jcaN": {
    string: "There are still fulfillments created for this order. Cancel the fulfillments first before you cancel the order."
  },
  FIZvTx,
  "D4W/LE": {
    context: "dialog header",
    string: "Change customer billing address"
  },
  RzDYi8,
  "r4g/vD": {
    context: "search modal billing title",
    string: "Rechnungsadresse"
  },
  vf56In,
  qov29K,
  "CG+awx": {
    context: "dialog content",
    string: "Which address would you like to use as shipping address for selected customer:"
  },
  "PBd/e+": {
    context: "dialog header",
    string: "Change address for order"
  },
  "9gb9b4": {
    context: "address type",
    string: "Neue Adresse hinzuf\xFCgen"
  },
  Qph0GE,
  xWEFrR,
  "kQq6/o": {
    context: "info when addresses search is unsuccessful",
    string: "No results found"
  },
  zqarUF,
  "129wyQ": {
    context: "dialog header",
    string: "Change customer shipping address"
  },
  txOXvy,
  "2OH46U": {
    context: "search modal shipping title",
    string: "Versandadresse"
  },
  s6lW8R,
  Zg0dRo,
  iue6mV,
  uD93er,
  VrFy8e,
  puALFo,
  "4Jp83O": {
    context: "subheader",
    string: "Kontaktinformationen"
  },
  hkSkNx,
  VCzrEZ,
  DP5VOH,
  Y7M1YQ,
  GLX9II,
  Qovenh,
  "c7/79+": {
    string: "Rechnungsadresse"
  },
  R98JLZ,
  "/w919H": {
    context: "OrderCustomer Fulfillment from Local Warehouse",
    string: "Fulfill from Local Stock"
  },
  AqXzM2,
  "9ZtJhn": {
    context: "cancel button",
    string: "Bestellung abbrechen"
  },
  "maxT+q": {
    context: "save button",
    string: "Confirm order"
  },
  "+RjQjs": {
    context: "return button",
    string: "Return / Replace order"
  },
  QSnh4Y,
  nvSJNR,
  GAmGog,
  fo7nfa,
  IN5iJz,
  WTj17Z,
  YFDAaX,
  WUf3Iu,
  APcoSA,
  mxtAFx,
  "32dfzI": {
    context: "price or ordered products",
    string: "Preis"
  },
  "nEWp+k": {
    context: "quantity of ordered products",
    string: "Menge"
  },
  "UD7/q8": {
    string: "Keine Produkte zur Bestellung hinzugef\xFCgt"
  },
  lVwmf5,
  Myx1Qp,
  BjxQ3u,
  "Jb1/3V": {
    context: "add shipping carrier button",
    string: "Versanddienstleister hinzuf\xFCgen"
  },
  "+8v1ny": {
    context: "discount button",
    string: "Rabatt"
  },
  M9LXb5,
  xUvWaP,
  mQtoRO,
  "S/yAtJ": {
    context: "total price",
    string: "Gesamtsumme"
  },
  "18wvf7": {
    context: "section header",
    string: "Order Details"
  },
  C50ahv,
  w2eTzO,
  LshEVn,
  "7a1S4K": {
    context: "tab name",
    string: "Alle Entw\xFCrfe"
  },
  NJEe12,
  vwMO04,
  iEeIhY,
  KIh25E,
  ps0WUQ,
  hkENym,
  mCP0UD,
  "1Uj0Wd": {
    context: "order draft total price",
    string: "Gesamtsumme"
  },
  PAqicb,
  "4Z14xW": {
    context: "button",
    string: "Finalize"
  },
  "6u4K7e": {
    context: "page header",
    string: "Bestellung"
  },
  kPIZ65,
  CJpx4E,
  N5UuEK,
  "z9wQ/U": {
    context: "no variant stock in warehouse",
    string: "No Stock"
  },
  vW3tb6,
  Kg0Fiu,
  "0VDwAP": {
    context: "checkbox label",
    string: "Send shipment details to customer"
  },
  "fw+VAN": {
    context: "product's sku",
    string: "Artikelnummer"
  },
  BLX9dz,
  Uh9R9m,
  bS7A8u,
  dTkmON,
  "K//bUK": {
    context: "refund button",
    string: "Erstatten"
  },
  "4PlW0w": {
    context: "tracking number",
    string: "Tracking Number: {trackingNumber}"
  },
  EHsnZX,
  SqhC7g,
  UQu75k,
  xco5tZ,
  aHc89n,
  bb4nSp,
  "2MKkgX": {
    context: "checkbox label",
    string: "Allow fulfillment without payment"
  },
  XwQQ1f,
  "05hqq6": {
    context: "checkbox label",
    string: "Automatically approve all fulfillments"
  },
  l9ETHu,
  G3ay2p,
  "yT/GAp": {
    string: "Sendungsnummer"
  },
  "/BJQIq": {
    context: "dialog header",
    string: "Add Tracking Code"
  },
  RLTaAR,
  fkplbE,
  nHmugP,
  GLy2UR,
  GJAX0z,
  OzHN0Z,
  "OKGd/k": {
    context: "order history message",
    string: "Bestellung aus Entwurf angelegt"
  },
  "4Z6BtA": {
    context: "order history message",
    string: "Zahlungsbest\xE4tigung wurde an den Kunden versendet."
  },
  wOeIR4,
  "6WRFp2": {
    context: "order history message",
    string: "Note was added to the order"
  },
  oQ27V4,
  cqZ5UH,
  chvryR,
  "8RnPGF": {
    context: "order history message",
    string: "Zahlung wurde entwertet."
  },
  fehqPs,
  "P/EDn1": {
    context: "order history message",
    string: "Bestellung wurde vollst\xE4ndig bezahlt"
  },
  ubasgL,
  "2yV+s8": {
    context: "order history message",
    string: "Zahlung wurde erfasst."
  },
  aq5ZiN,
  hWO1SD,
  "GVM/fi": {
    context: "order history message",
    string: "Zahlung wurde autorisiert."
  },
  j3yE7I,
  "3fgyFh": {
    context: "order history message",
    string: "Zahlung wurde zur\xFCckerstattet."
  },
  "D3WUc/": {
    context: "order history message",
    string: "Order was refunded by {refundedBy}"
  },
  U1eJIw,
  PcPMjC,
  "6RQKxH": {
    context: "order history message",
    string: "Invoice no. {invoiceNumber} was updated"
  },
  XBfvKN,
  Fl3ORD,
  "9piUVz": {
    context: "order history message",
    string: "Order refund information was sent to customer"
  },
  e92Uxp,
  TCR639,
  "06bR4Z": {
    context: "order history message",
    string: "Order cancel information was sent to customer"
  },
  BCPrmK,
  pTpx0p,
  qddy2Z,
  zRrcOG,
  kVOslW,
  kvSYZh,
  A0Wlg7,
  MPfyne,
  "5JT4v2": {
    context: "dialog header",
    string: "Send Invoice"
  },
  Gzg8hy,
  F0AXNs,
  hPB89Y,
  m6IBe5,
  "e0RKe+": {
    context: "generate invoice button",
    string: "Generate"
  },
  zyceue,
  WbV1Xm,
  wTHjt3,
  WRkCFt,
  lJP1iw,
  biAxKR,
  PzXIXh,
  JUQwne,
  s5v6m0,
  Kgxlsf,
  "a4qX2+": {
    context: "order",
    string: "Erstellt"
  },
  "JYvf8/": {
    context: "is preorder",
    string: "Preorder"
  },
  NWxomz,
  "p+UDec": {
    context: "payment status",
    string: "Zahlungen"
  },
  "5blVMu": {
    context: "e-mail or full name",
    string: "Kunde"
  },
  PHUcrU,
  k9hf7F,
  EbVf0Z,
  rwOx2s,
  sfEbeB,
  "+B25o/": {
    context: "dialog header",
    string: "Mark Order as Paid"
  },
  "+PbHKD": {
    context: "dialog header",
    string: "Zahlung annehmen"
  },
  OhdPS1,
  "euRfu+": {
    string: "Wollen Sie diese Zahlung wirklich entwerten?"
  },
  KszPFx,
  BZ7BkQ,
  "V+gwx7": {
    context: "order payment",
    string: "Eingel\xF6ste Summe"
  },
  ghGLbJ,
  u0S2be,
  pr513b,
  "+5HkZN": {
    context: "order, button",
    string: "Als bezahlt markieren"
  },
  "5te3Tp": {
    context: "order payment",
    string: "Outstanding Balance"
  },
  vM9quW,
  "SC/eNC": {
    context: "Payment card title",
    string: "Zahlungsstatus"
  },
  uUsZ7m,
  HaQ8cg,
  "q+gCyP": {
    context: "order payment",
    string: "Refunded amount"
  },
  "+CeEe3": {
    context: "order shipping method name",
    string: "Versand"
  },
  uzAPCt,
  Rsknyh,
  "QJG+d/": {
    context: "staff added type order discount",
    string: "Staff added"
  },
  T8rvXs,
  "r+dgiv": {
    string: "Steuern"
  },
  zb4eBO,
  dJVXIb,
  sEjRyz,
  myyWNp,
  b810WJ,
  WE8IFE,
  tvpAXl,
  "8J81ri": {
    context: "ordered product sku",
    string: "Artikelnummer"
  },
  qT6YYk,
  "5aiFbL": {
    context: "tabel column header",
    string: "Preis"
  },
  "FNT4b+": {
    context: "tabel column header",
    string: "Produkt"
  },
  "Tl+7X4": {
    context: "tabel column header",
    string: "Refunded Qty"
  },
  "2W4EBM": {
    context: "button",
    string: "Set maximal quantities"
  },
  "+PclgM": {
    context: "tabel column header",
    string: "Gesamtsumme"
  },
  "xoyCZ/": {
    context: "error message",
    string: "Improper value"
  },
  MewrtN,
  "H/f9KR": {
    context: "section header returned",
    string: "Fulfillment returned"
  },
  "i/ZhxL": {
    context: "section header returned",
    string: "Fulfillment waiting for approval"
  },
  "0krqBj": {
    context: "page header",
    string: "Order no. {orderNumber} - Refund"
  },
  rVIlBs,
  "0oo+BT": {
    context: "section header",
    string: "Refunded Amount"
  },
  "8F2D1H": {
    context: "order refund amount, input button",
    string: "Refund {currency} {amount}"
  },
  JEIN47,
  "FOehC/": {
    context: "label",
    string: "Manual Amount"
  },
  "EP+jcU": {
    context: "checkbox",
    string: "Refund shipment costs"
  },
  zzfj8H,
  fbH51z,
  "IKvOK+": {
    context: "Amount error message",
    string: "Amount must be bigger than 0"
  },
  "L/O4LQ": {
    context: "order refund amount",
    string: "Authorized Amount"
  },
  lrq8O6,
  I7HyJZ,
  Q55cTG,
  wDUBLR,
  QkFeOa,
  AKv2BI,
  C6bb6x,
  i56GGQ,
  "bgO+7G": {
    context: "order return amount button",
    string: "Return & Replace products"
  },
  "Uo5/Ov": {
    context: "order return subtitle",
    string: "Returned items can't be fulfilled"
  },
  kak5vT,
  "WGp+Fw": {
    context: "order refund amount",
    string: "Shipment Cost"
  },
  "B/y6LC": {
    context: "section header",
    string: "Unfulfilled Products"
  },
  iUIn50,
  bqAJCT,
  LKpQYh,
  CLB1k9,
  Y299ST,
  "0qg33z": {
    context: "table column header",
    string: "Return"
  },
  aAAxKp,
  ikM00B,
  EtkIxE,
  RlbhwF,
  iJrw63,
  ZPOyI1,
  oQhFlK,
  jNSOSu,
  "3stu21": {
    context: "refunded fulfillment, section header",
    string: "Replaced ({quantity})"
  },
  eCRaHe,
  p4zuQp,
  NxRsHQ,
  BkFke9,
  "/xXvjF": {
    context: "section header",
    string: "Noch nicht abgeschlossen"
  },
  "9ssWj+": {
    context: "unapproved fulfillment, section header",
    string: "Waiting for approval ({quantity})"
  },
  BBIQxQ,
  Vu9nol,
  yuiyES,
  CLYlsu,
  "7UG1Lx": {
    context: "checkbox gift cards label",
    string: "Automatically fulfill non shippable gift cards"
  },
  Nfh9QM,
  wpAXKX,
  RLYfMF,
  "V/YxJa": {
    context: "dialog header",
    string: "Edit Shipping Method"
  },
  "/Xwjww": {
    context: "button",
    string: "Abschluss"
  },
  "ND5x+V": {
    string: "We\u2019re generating the invoice you requested. Please wait a couple of moments"
  },
  "9RCuN3": {
    string: "Zahlung erfolgreich erfasst."
  },
  c4gbXr,
  "9OtpHt": {
    string: "Order line deleted"
  },
  CZmloB,
  Fn3bE0,
  lL1HTg,
  HlCkMT,
  "W/Es0H": {
    string: "Order successfully cancelled"
  },
  KmPicj,
  L87bp7,
  j2fPVo,
  "+sX7yS": {
    string: "Fulfillment successfully approved"
  },
  "3u+4NZ": {
    string: "Invoice email sent"
  },
  PKJqcq,
  "7U8GRy": {
    string: "Shipping method successfully updated"
  },
  uMpv1v,
  TLNf6K,
  GbBCmr,
  qbmeUI,
  ra2O4j,
  "6udlH+": {
    string: "Entwurf erfolgreich angelegt"
  },
  Q6VRrE,
  "2MKBk2": {
    context: "window title",
    string: "Fulfill Order #{orderNumber}"
  },
  CYEnGq,
  NzifUg,
  XRf1Bi,
  XQBVEJ,
  l9Lwjh,
  "/z9uo1": {
    context: "order returned success message",
    string: "Successfully returned products!"
  },
  a9S9Je,
  kTr2o8,
  uxPpRx,
  nf3XSt,
  iQxjow,
  "2Zyit2": {
    context: "delete page types with its pages",
    string: "{counter,plural,one {Page Type you want to delete is used by some pages. Deleting this page type will also delete those pages. Are you sure you want to delete this page type? After doing so you won\u2019t be able to revert changes.} other {Page Types you want to delete are used by some pages. Deleting these page types will also delete those pages. Are you sure you want to delete {displayQuantity} page types? After doing so you won\u2019t be able to revert changes.}}"
  },
  CcEwXH,
  RZmdM3,
  "kZfIl/": {
    string: "These are general information about this Content Type."
  },
  OVOU1z,
  caqRmN,
  lct0qd,
  jWna9Q,
  umsU70,
  "6JlXeD": {
    context: "button",
    string: "Create page type"
  },
  oVDZUb,
  BQ2NVl,
  "6fORLY": {
    string: "No page types found"
  },
  oHbgcK,
  I8mqqj,
  "Rpfa+t": {
    context: "dialog header",
    string: "Unassign Attribute from Page Type"
  },
  NGc9kE,
  WFG7Zk,
  "/L8wzi": {
    context: "dialog header",
    string: "Unassign Attribute From Page Type"
  },
  "5bJ26s": {
    string: "Successfully created page type"
  },
  H6NsC1,
  gr53VQ,
  jZbT0O,
  GZgjK7,
  X26jCC,
  "/TK7QD": {
    context: "page label",
    string: "Versteckt"
  },
  gMwpNC,
  "gr+oXW": {
    context: "page title",
    string: "Titel"
  },
  AHRDWt,
  Of19Pn,
  UW1fLs,
  "V2+HTM": {
    context: "dialog header",
    string: "Titel"
  },
  "5GSYCR": {
    context: "page status",
    string: "Sichtbarkeit"
  },
  iMJka8,
  I8dAAe,
  G1KzEx,
  UN3qWD,
  ufD5Jr,
  jU9GPX,
  mX7zJJ,
  JMbFNo,
  C1luwg,
  "4B32Ba": {
    context: "delete page",
    string: "Are you sure you want to delete {title}?"
  },
  "41z2Qi": {
    context: "notification",
    string: "Removed pages"
  },
  yEmwxD,
  yHQQMQ,
  Wd8vG7,
  F8gsds,
  wyvzh9,
  WRPQMM,
  "UNwG+4": {
    context: "dialog content",
    string: "{counter,plural,one {Are you sure you want to delete this page?} other {Are you sure you want to delete {displayQuantity} pages?}}"
  },
  AzshS2,
  "3Sz1/t": {
    context: "dialog header",
    string: "Seiten l\xF6schen"
  },
  "2pw5dQ": {
    context: "payment status",
    string: "Bezahlt"
  },
  PbqNhi,
  INNPVX,
  OGemtu,
  "26BKkX": {
    context: "order status",
    string: "Partially returned"
  },
  "6D+yYX": {
    string: "Zahlungsstatus"
  },
  hLUYBt,
  DNTxWr,
  jUuHVn,
  UQ4Kuh,
  "6sjBvJ": {
    context: "input placeholder",
    string: "Search by name, email, etc..."
  },
  "9Zlogd": {
    context: "staff member status",
    string: "Aktiv"
  },
  "7WzUxn": {
    context: "staff member status",
    string: "Inaktiv"
  },
  DPz5y6,
  lT5MYM,
  "H/o4Ex": {
    context: "dialog content",
    string: "You are not able to modify this group members. Solve this problem to continue with request."
  },
  mAabef,
  CYZse9,
  sR0urA,
  "L6+p8a": {
    context: "dialog title",
    string: "Delete permission group"
  },
  O22NIZ,
  rs815i,
  "5ftg/B": {
    context: "button",
    string: "create permission group"
  },
  szXISP,
  "+a+2ug": {
    string: "Members"
  },
  wL7VAE,
  CXn88q,
  gVD1os,
  qrWOxx,
  xxQxLE,
  OhFGpX,
  "zD7/M6": {
    context: "empty list message",
    string: "Please use Assign Members button to do so."
  },
  lGlDEH,
  W32xfN,
  XGBsoK,
  eUjFjW,
  "15PiOX": {
    context: "button title",
    string: "Zuordnung aufheben"
  },
  DovGIa,
  WhvuCb,
  "6aBkJm": {
    context: "section header",
    string: "Autorisierung"
  },
  Aeq79M,
  IUeGzv,
  "bL/Wrc": {
    context: "plugin status",
    string: "Status"
  },
  w424P4,
  "FA+MRz": {
    string: "Set plugin as active"
  },
  qCH2eZ,
  "Xy2T+y": {
    context: "header",
    string: "Edit Authorization Field"
  },
  Egyh2T,
  EtGDeK,
  BtErCZ,
  aOelhW,
  "T/dYnE": {
    context: "plugin filters error messages status",
    string: "Status is not selected"
  },
  zQnYKn,
  "TC/EOG": {
    context: "status section title",
    string: "Status in channel"
  },
  gZHmaV,
  cwoN25,
  Co2U4u,
  rQOS7K,
  ycrTBX,
  HedXnw,
  AijtXU,
  ho75Lr,
  reP5Uf,
  "xTIKA/": {
    context: "PluginChannelConfigurationCell global title",
    string: "Global"
  },
  QH74y5,
  "8u7els": {
    context: "channel config plugin status popup title",
    string: "Assigned to {activeChannelsCount} of {allChannelsCount} channels"
  },
  "N6lfS/": {
    context: "header",
    string: "Authorization Field Delete"
  },
  JRfJD9,
  "6QjMei": {
    string: "Preorder end time needs to be set in the future"
  },
  YQ3EXR,
  "9scTQ0": {
    context: "section header",
    string: "Product Attributes"
  },
  "5pHBSU": {
    context: "switch button",
    string: "Product type uses Variant Attributes"
  },
  "7nKXni": {
    context: "option description",
    string: "This product will act as a payment method"
  },
  kp2IYP,
  xRbqcg,
  H5yp8O,
  "1KSqnn": {
    context: "tab name",
    string: "All Product Types"
  },
  rpFdD1,
  QY7FSs,
  X90t9n,
  dS8Adx,
  U5aVd8,
  "Jsh6+U": {
    context: "product type is digital or physical",
    string: "Typ"
  },
  "0nLsyM": {
    string: "Keine Produkttypen gefunden."
  },
  "yNb+dT": {
    context: "product type",
    string: "Simple product"
  },
  TalJlD,
  jyTwDR,
  hHOI7D,
  asdvmK,
  ADTNND,
  "/2OOMe": {
    context: "product type shipping settings, section header",
    string: "Versand"
  },
  IBw72y,
  VOiUXQ,
  zCb8fX,
  vlLyvk,
  "4k9rMQ": {
    context: "variant attribute checkbox",
    string: "Variant Selection"
  },
  "skEK/i": {
    context: "section header",
    string: "Variant Attributes"
  },
  x3leH4,
  GCPzKf,
  paa4m0,
  F3Upht,
  UJnqdm,
  r1aQ2f,
  "S7j+Wf": {
    context: "unassign attribute from product type, button",
    string: "Zuordnung aufheben"
  },
  K8xNLe,
  ulh3kf,
  fyE8BN,
  LKoIB1,
  "L7N+0y": {
    context: "product rating",
    string: "Product Rating"
  },
  JGm7E5,
  ZDJEat,
  "ve/Sph": {
    context: "there are more elements of list that are hidden",
    string: "and {number} more"
  },
  "64aYF0": {
    context: "informations about product organization, header",
    string: "Product Organization"
  },
  "6xC/Ls": {
    context: "informations about product seo, header",
    string: "SEO Information"
  },
  qEZ463,
  g6yuk2,
  xtUXnK,
  WQMTKI,
  "/68iG8": {
    context: "product export to csv file, header",
    string: "Information exported"
  },
  z1puMb,
  ki7Mr8,
  lQRnYK,
  "9Tl/bT": {
    context: "export items as spreadsheet",
    string: "Spreadsheet for Excel, Numbers etc."
  },
  YicEbK,
  ZRz3hM,
  xjpTLF,
  li1BBk,
  Jwuu4X,
  "tIc2/h": {
    context: "input helper text, search attributes",
    string: "Search by attribute name"
  },
  SZt9kC,
  jj3Cb8,
  oOFrUd,
  dc5KWn,
  xkjRu5,
  zDvDnG,
  "4W/CKn": {
    context: "modal button",
    string: "Upload URL"
  },
  "7FL+WZ": {
    context: "export products to csv file, button",
    string: "Produkte exportieren"
  },
  FwHWUm,
  Kw0jHS,
  kIvvax,
  aFLtLk,
  "5Vwnu+": {
    string: "You have reached your SKU limit, you will be no longer able to add SKUs to your store. If you would like to up your limit, contact your administration staff about raising your limits."
  },
  diOQm7,
  pbGIUg,
  Bx367s,
  pBTTtU,
  "Sna+WK": {
    context: "product status",
    string: "Out Of Stock"
  },
  b1zuN9,
  "3Z8972": {
    context: "product",
    string: "Stock quantity"
  },
  "g+GAf4": {
    context: "product visibility",
    string: "Sichtbarkeit"
  },
  "6Y1nQd": {
    context: "product is visible",
    string: "Sichtbar"
  },
  "Jz/Cb+": {
    context: "product type",
    string: "Simple"
  },
  "+VEhV8": {
    context: "product channels",
    string: "Verf\xFCgbarkeit"
  },
  Sd0Ppm,
  qpQ0uB,
  kVTWtR,
  r0hgpM,
  XUU9sU,
  Ihp4D3,
  "9RvXNg": {
    context: "section header",
    string: "Media Information"
  },
  cW1RIo,
  "9CEu8k": {
    context: "modal button images upload",
    string: "Upload Images"
  },
  Q2UXlW,
  "/Mcvt4": {
    context: "section header",
    string: "Media"
  },
  mGiA6q,
  JjeZEG,
  "Be+J13": {
    string: "Konfigurierbar"
  },
  "v+Pkm+": {
    context: "field is optional",
    string: "*Optional. Hinzuf\xFCgen von Produkten zur Sammlung hilft den Benutzern, sie zu finden."
  },
  anK7jD,
  "3rIMq/": {
    context: "product shipping",
    string: "Versand"
  },
  SUbxSK,
  "eAFU/E": {
    context: "product inventory, checkbox",
    string: "Variant currently in preorder"
  },
  "2qJc9y": {
    string: "CANCEL END DATE"
  },
  "7wkGxW": {
    context: "app has been installed",
    string: "{unitsLeft} units left"
  },
  xB7BTp,
  KTAg0f,
  "7Ii5ZQ": {
    string: "SETUP END DATE"
  },
  "bp/i0x": {
    context: "header",
    string: "Menge"
  },
  "Gz+4CI": {
    context: "info text",
    string: "Preordered products will be available in all warehouses. You can set a threshold for sold quantity. Leaving input blank will be interpreted as no limit to sale. Sold items will be allocated at the warehouse assigned to chosen shipping zone."
  },
  NcY4ph,
  JyQEHU,
  TjGYna,
  cBHRxx,
  REVk27,
  ekXood,
  RJ5QxE,
  jABdx1,
  "TfY/Pi": {
    context: "checkbox",
    string: "Dieses Produkt versteuern"
  },
  CdIHMu,
  iYH3Y7,
  "8uo4v1": {
    context: "title",
    string: "Checkout limits"
  },
  "n3+6w5": {
    context: "input helper text",
    string: "Your customer won't be allowed to buy bigger quantity per checkout than shown here."
  },
  C7I2lg,
  W3JbSQ,
  vXA5xU,
  uxZZXx,
  EzU7KV,
  "9EMudJ": {
    context: "option",
    string: "Create single variant"
  },
  XDeh5D,
  f3B4tc,
  o6260f,
  "7hNjaI": {
    context: "button",
    string: "Delete Variant"
  },
  sw8Wl2,
  U9CIo7,
  TPCRKr,
  ClFzoD,
  Xr5zxu,
  lVZ5n7,
  TDXskW,
  "7WEC+G": {
    context: "page title",
    string: "Price and SKUs"
  },
  GQcp83,
  NXpFlL,
  "CrbI/c": {
    context: "variant channel price",
    string: "{channel} Price"
  },
  rVaB7c,
  ucYPtV,
  slKV5G,
  lra7Ej,
  J0UdxG,
  L5rthO,
  "EGG8f+": {
    string: "Apply unique prices by attribute to each SKU"
  },
  STp3Hl,
  "+bFHzi": {
    context: "button",
    string: "Next"
  },
  rHXF43,
  BIqhVQ,
  "Q3j++G": {
    context: "create multiple variants, button",
    string: "Erstellen"
  },
  S5PVx1,
  "/Qb92c": {
    string: "Basierend auf Ihrer Auswahl werden wir {numberOfProducts} Produkte erstellen. Passen Sie jetzt Preise und verf\xFCgbare Mengen am Lager an."
  },
  k4brJy,
  Sx7QVu,
  iigydN,
  esg2wu,
  V76IV7,
  rbkmfG,
  WwNtFn,
  GFJabu,
  "XMvH/d": {
    context: "button label",
    string: "ACCEPT"
  },
  dTCWqt,
  Y4cy0i,
  iPk640,
  "2J6EFz": {
    context: "button",
    string: "Choose media"
  },
  JfKvrV,
  vZMs8f,
  "1kdQdO": {
    context: "section header",
    string: "Varianten"
  },
  "3C3Nj5": {
    context: "button",
    string: "Variante hinzuf\xFCgen"
  },
  gF7hbK,
  EsZH44,
  rJ3lkW,
  jqJqdE,
  "3+KwtP": {
    context: "VariantDetailsChannelsAvailabilityCard subtitle",
    string: "Available in {publishedInChannelsCount} out of {availableChannelsCount}"
  },
  sedoZ3,
  Xm9qOu,
  JFtFgc,
  KQSONM,
  VvA7ai,
  c8UT0c,
  "2zCmiR": {
    context: "tabel column header",
    string: "Einkaufspreis"
  },
  SZH0fw,
  "6+sMz4": {
    context: "product variant inventory",
    string: "Unavailable in all locations"
  },
  "7mK2vs": {
    context: "product variant inventory",
    string: "Unavailable"
  },
  n02c9W,
  uVssds,
  FSinkL,
  HcjV6k,
  qbqMpk,
  wWYYBR,
  rIJbNC,
  JtZ71e,
  "kL3C+K": {
    context: "product variant inventory status",
    string: "Lager"
  },
  "3VyHbJ": {
    context: "button",
    string: "Create variant"
  },
  "9PmyrU": {
    context: "product variant inventory",
    string: "Not stocked"
  },
  "80FeaT": {
    context: "product variant preorder threshold",
    string: "{globalThreshold} Global threshold"
  },
  OTek3r,
  "uCn/rd": {
    context: "dialog header",
    string: "Image entfernen"
  },
  BUKMzM,
  T6dXGG,
  "VEext+": {
    string: "Are you sure you want to delete this image?"
  },
  MyM2oR,
  PXx4Jk,
  NBP8uu,
  "DO8+uV": {
    string: "Produkt angelegt"
  },
  yDkmX7,
  "5QKsu+": {
    context: "waiting for export to end, header",
    string: "Exporting CSV"
  },
  F4WdSO,
  dPYqy0,
  YKyNm9,
  "X/K4LI": {
    context: "label",
    string: "Normal"
  },
  vlVTmY,
  ZHF4Z9,
  TWVx7O,
  ukdRUv,
  "6iw4VR": {
    context: "delete variant dialog title",
    string: "Delete Product Variants"
  },
  oChkS4,
  "z+wMgQ": {
    context: "window title",
    string: "Create Variants"
  },
  IeoGgH,
  aI80kg,
  kFYlu2,
  rqtV5d,
  oLMXDv,
  XJSRDK,
  nwnwJ0,
  "bu/fC1": {
    context: "button",
    string: "Entfernen"
  },
  "TKmub+": {
    string: "This field is required"
  },
  "4Dc2j0": {
    context: "order status",
    string: "Returned"
  },
  kJQczl,
  RaycYK,
  rqiCWU,
  "a+x05s": {
    context: "select option, button",
    string: "Select"
  },
  rfvBAF,
  byP6IC,
  hqVMLQ,
  "8xsKUv": {
    context: "service accounts section name",
    string: "Service Accounts"
  },
  Fvvgoi,
  D9ie4n,
  nNeWAx,
  "ER/yBq": {
    context: "max price in channel",
    string: "Max. value"
  },
  aZDHYr,
  kN6SLs,
  "u5c/tR": {
    context: "channels discount info",
    string: "Channels that don\u2019t have assigned discounts will use their parent channel to define the price. Price will be converted to channel\u2019s currency"
  },
  yatGsm,
  Dgp38J,
  vjsfyn,
  "0FexL7": {
    context: "min price in channel",
    string: "Min. value"
  },
  "7v8suW": {
    context: "info text",
    string: "This rate will apply to all orders"
  },
  r2dojI,
  vWapBZ,
  "w+5Djm": {
    string: "Min. Order Weight"
  },
  u0V06N,
  "TnTi/a": {
    context: "pricing card title",
    string: "Preis"
  },
  "5ZvuVw": {
    string: "Kein passendes Produkt gefunden"
  },
  FzEew9,
  xZhxBJ,
  "Gg4+K7": {
    string: "Anzahl Produkte"
  },
  t3aiWF,
  ZIc5lM,
  TLYeo5,
  v17Lly,
  "GD/bom": {
    context: "label",
    string: "Min Delivery Time"
  },
  FkDObY,
  "4Kq3O6": {
    string: "This unit will be used as default shipping weight"
  },
  "Rp/Okl": {
    string: "Shipping Weight Unit"
  },
  "llBnr+": {
    string: "Warehouse Name"
  },
  "yzYXW/": {
    context: "header, dialog",
    string: "Create New Warehouse"
  },
  "LHPVg/": {
    context: "assign countries to shipping zone and save, button",
    string: "Assign and save"
  },
  AjInNW,
  f2F1NJ,
  "K/ic0P": {
    context: "checkbox label",
    string: "Rest of the World"
  },
  "G+9nOZ": {
    context: "checkbox description",
    string: "Bei Auswahl dieser Option werden alle nicht ausgew\xE4hlten L\xE4nder zu anderen Versandzonen hinzugef\xFCgt"
  },
  pGDYG5,
  "1rpzrM": {
    context: "search placeholder",
    string: "Search by country name"
  },
  "55LMJv": {
    context: "country list header",
    string: "L\xE4nder"
  },
  "6fxdUO": {
    context: "section header",
    string: "Create New Shipping Zone"
  },
  y7mfbl,
  "G0+gAp": {
    context: "shipping section header",
    string: "Shipping"
  },
  "FkRNk+": {
    context: "field placeholder",
    string: "Description of a shipping zone."
  },
  YpukUN,
  axFFaD,
  "DM/Ha1": {
    context: "add postal code range, button",
    string: "Hinzuf\xFCgen"
  },
  "1T1fP8": {
    context: "range input label",
    string: "Postal codes (start)"
  },
  "8InCjD": {
    string: "Please provide range of postal codes you want to add to the include/exclude list."
  },
  "2Xt+sw": {
    context: "dialog header",
    string: "Add postal codes"
  },
  Pyjarj,
  "7qsOwa": {
    context: "action",
    string: "Include postal codes"
  },
  ju8zHP,
  "1lk/oS": {
    context: "button",
    string: "Add postal code range"
  },
  ud0w8h,
  "/Zee1r": {
    string: "Only added postal codes will be able to use this shipping rate"
  },
  FcTTvh,
  YpLVVc,
  "RXPGi/": {
    context: "page title",
    string: "Price Rate Create"
  },
  PRlD0A,
  NDm2Fe,
  EKoPNg,
  WR8rir,
  njUQPz,
  RUzdUH,
  FjrExY,
  aYhcie,
  foB6wx,
  aPCrsp,
  "n25d+d": {
    context: "WarehousesSection select field add text",
    string: "Add New Warehouse"
  },
  "/cow4T": {
    context: "WarehousesSection select field placeholder",
    string: "Select Warehouse"
  },
  wjKYSU,
  "t/R8nK": {
    context: "ShippingZoneSettingsCard title",
    string: "Einstellungen"
  },
  uULcph,
  "h5r9+x": {
    context: "sort shipping methods by zone, section header",
    string: "Shipping By Zone"
  },
  mIUNgR,
  aMwxYb,
  "gRa/TS": {
    context: "shipping zone",
    string: "Name"
  },
  IhK1F3,
  Gfbp36,
  "p/Fd7s": {
    context: "unassign products from shipping rate and save, button",
    string: "Unassign and save"
  },
  "57IYpr": {
    context: "error message",
    string: "Wert ist ung\xFCltig"
  },
  pwqwcy,
  "qf/m5l": {
    string: "Are you sure you want to delete {shippingZoneName} shipping zone?"
  },
  "k3EI/U": {
    context: "dialog header",
    string: "Delete Shipping Zone"
  },
  cpZLRH,
  C9pcQx,
  YdeHZX,
  "1zuQ2P": {
    context: "unassign country",
    string: "Are you sure you want to remove {countryName} from this shipping zone?"
  },
  "M6s/9e": {
    context: "unassign country, dialog header",
    string: "Remove from Shipping Zone"
  },
  MXZuVP,
  "H27/Gy": {
    context: "error message",
    string: "Maximum weight cannot be lower than minimum"
  },
  "/8/Ffn": {
    context: "button",
    string: "Show"
  },
  viFkCw,
  "+do3gl": {
    context: "input helper text",
    string: "This number defines quantity of items in checkout line that can be bought. You can override this setting per variant. Leaving this setting empty mean that there is no limits."
  },
  QclvqG,
  X7fqfM,
  C4aDMy,
  "+T0oJ7": {
    context: "input label",
    string: "Stock reservation for anonymous user (in minutes)"
  },
  OaKyz4,
  "YEv+6G": {
    context: "input helper text",
    string: "Leaving this setting empty will mean that stock won\u2019t be reserved"
  },
  "+jCDvp": {
    context: "section header",
    string: "Store Information"
  },
  Av74Fa,
  DASYUF,
  pRYGUR,
  rPX1f2,
  LVa5ew,
  AQFMYU,
  "23g7PY": {
    context: "dialog header",
    string: "Invite Staff Member"
  },
  hw9Fah,
  "P+kVxW": {
    context: "card description",
    string: "User is assigned to:"
  },
  XMrYaA,
  YJ4TXc,
  OaA0f9,
  pA8Mlv,
  aDbrOK,
  "4JcNaA": {
    context: "button",
    string: "Invite staff member"
  },
  "9xlPgt": {
    context: "used staff users counter",
    string: "{count}/{max} members"
  },
  HR9OTW,
  Fc3O3r,
  utaSh3,
  xJQX5t,
  cMFlOp,
  qEJT8e,
  "+kb2lM": {
    context: "dialog header",
    string: "Passwort \xE4ndern"
  },
  GXdwyR,
  mm0CXe,
  N3Zot1,
  ZhDQel,
  JJgJwi,
  e822us,
  mr9jbO,
  CLeDae,
  "+2VzH4": {
    context: "avatar change button",
    string: "\xC4ndern"
  },
  VTITVe,
  "11lR5V": {
    context: "avatar delete button",
    string: "L\xF6schen"
  },
  "K/gnGg": {
    string: "If you want to disable this User please uncheck the box below."
  },
  "7yKZvp": {
    context: "section header",
    string: "User Status"
  },
  "GhXwO/": {
    context: "dialog header",
    string: "Mitarbeiter Account l\xF6schen"
  },
  fzpXvv,
  VKWPBf,
  gxPjIQ,
  QirE3M,
  tWbE34,
  tzMNF3,
  RrCui3,
  "5elC9k": {
    context: "taxes section name",
    string: "Steuern"
  },
  lnQAos,
  "3BTtL2": {
    string: "No countries found"
  },
  "/JENWS": {
    string: "Reduced Tax Rates"
  },
  "07KB2d": {
    string: "Country Code"
  },
  "0GJfWd": {
    string: "Country Name"
  },
  la9cZ4,
  QHB48n,
  "Ubath+": {
    string: "No reduced tax categories found"
  },
  "4EuJKs": {
    string: "Alle Produktpreise sind inkl. Steuer"
  },
  "98isC5": {
    string: "Bruttopreise in der Storefront anzeigen"
  },
  FNKhkx,
  "+OV+Gj": {
    context: "button",
    string: "Steuers\xE4tze abrufen"
  },
  HtQGEH,
  u3sYPH,
  "+xTpT1": {
    string: "Attribute"
  },
  "5fCMUI": {
    context: "translations section name",
    string: "\xDCbersetzungen"
  },
  "tUlsq+": {
    string: "Translating"
  },
  QUyUJy,
  "bh+Keo": {
    string: "{numberOfFields} Translations, {numberOfTranslatedFields} Completed"
  },
  Xtd0AT,
  "/vCXIP": {
    string: "\xDCbersetzung"
  },
  vTN5DZ,
  "T/5OyA": {
    string: "No translation yet"
  },
  DRMMDs,
  SPBLzT,
  "UvD+xp": {
    context: "attribute values",
    string: "Value {number}"
  },
  JE0TAx,
  US3IPU,
  "XitW/z": {
    string: '\xDCbersetzung Kategorie "{categoryName}" - {languageCode}'
  },
  HlEpii,
  Bphmwe,
  GsBRWL,
  FemBUF,
  "7NFfmz": {
    string: "Produkte"
  },
  CxfKLC,
  VKb1MS,
  RzsKm8,
  c8nvms,
  "etP0+D": {
    string: "Gutscheine"
  },
  ikRuLs,
  vcwrgW,
  LWmYSU,
  X6PF8z,
  ptPPVk,
  y1Z3or,
  oUWXLO,
  PajjqE,
  gvOzOl,
  "98WMlR": {
    context: "header",
    string: 'Translation Product Variant "{productName}" - {languageCode}'
  },
  T1f2Yl,
  "22x9tu": {
    context: "header",
    string: 'Translation Product "{productName}" - {languageCode}'
  },
  zjkAMs,
  s40PZt,
  GpqEl5,
  "1UKx20": {
    context: "header",
    string: 'Translation ShippingMethod "{shippingMethodName}" - {languageCode}'
  },
  "1tXSSK": {
    context: "header",
    string: 'Translation Voucher "{voucherName}" - {languageCode}'
  },
  "sfErC+": {
    string: "Gutschein Name"
  },
  MKtgZB,
  "+x84Ji": {
    context: "order status",
    string: "Unconfirmed"
  },
  vN3qdA,
  oB0y5Y,
  FBtqtl,
  Lx1ima,
  m8cjcK,
  "+x4cZH": {
    string: "Only staff members can be assigned"
  },
  aggaJg,
  u24Ppd,
  cd13nN,
  lLwtgs,
  IFWHn0,
  ij7olm,
  BM1JiJ,
  nOo0oL,
  Xb6BJ9,
  WzA5Ll,
  sZ27WU,
  gKdGxP,
  "DK+8PB": {
    string: "This channel has already been created"
  },
  V2BBQu,
  QFCUEt,
  AY7Tuz,
  "E8T3e+": {
    string: "Cannot add and remove group the same time"
  },
  abTH5q,
  c5pMZ8,
  d9UqaJ,
  "577R2r": {
    string: "Invalid value"
  },
  "pC6/1z": {
    string: "Invalid manifest format"
  },
  eu98dw,
  D2qihU,
  "g/BrOt": {
    string: "Url has invalid format"
  },
  AdmPca,
  "0AQH0Q": {
    string: "Plugin is misconfigured and cannot be activated"
  },
  FuAV5G,
  "3AqOxp": {
    context: "no category set error",
    string: "Product category not set"
  },
  Wlc67M,
  "r+8q4B": {
    context: "error message",
    string: "Nur Bestellentw\xFCrfe k\xF6nnen bearbeitet werden"
  },
  PCoO4D,
  Fz3kic,
  "N43t3/": {
    context: "error message",
    string: "Number not set for an invoice"
  },
  C4hCsD,
  "1n1tOR": {
    string: "Group is out of your permission scope"
  },
  "4prRLv": {
    string: "Permission is out of your scope"
  },
  KRqgfo,
  cY42ht,
  Y1B0PN,
  vVviA2,
  mYs3tb,
  VEE4gD,
  ychKsb,
  rZf1qL,
  nKjLjT,
  wn3di2,
  LR3HlT,
  "1wyZpQ": {
    string: "These passwords are too similar"
  },
  TDhHMi,
  qDwvZ4,
  DILs4b,
  "vP7g2+": {
    context: "error message",
    string: "URL not set for an invoice"
  },
  Z6QAbw,
  i3Mvj8,
  iUy2dx,
  ycMLN9,
  "43Nlay": {
    context: "warehouse",
    string: "Adressinformationen"
  },
  GhcypC,
  DTL7sE,
  ny4zrH,
  YkOzse,
  caMMWN,
  "2yU+q9": {
    context: "tab name",
    string: "Alle Lagerh\xE4user"
  },
  "5HwLx9": {
    context: "alert",
    string: "Warehouse limit reached"
  },
  kFQvXv,
  wmdHhD,
  PFXGaR,
  "2gsiR1": {
    string: "No warehouses found"
  },
  aCJwVq,
  "16PGt9": {
    context: "WarehouseSettings all warehouses label",
    string: "All warehouses"
  },
  LEZZkK,
  "OBQ+th": {
    context: "WarehouseSettings disabled warehouse label",
    string: "Disabled"
  },
  M8z5WZ,
  tVybuM,
  "6G41zU": {
    context: "WarehouseSettings local warehouse description",
    string: "If selected customer will be able to choose this warehouse as pickup point. Ordered products will be only fulfilled from this warehouse stock"
  },
  cdiRSf,
  MIC9W7,
  wpli4O,
  XlPKAR,
  "86pLaG": {
    context: "WarehouseSettings public stock label",
    string: "Public Stock"
  },
  "Ff+Gsm": {
    context: "WarehouseSettings public stock description",
    string: "If enabled stock in this warehouse will be shown"
  },
  "//iaFx": {
    context: "WarehouseSettings title",
    string: "Einstellungen"
  },
  "6nSTuC": {
    context: "webhooks section name",
    string: "Webhooks"
  },
  "hS+ZjH": {
    context: "delete webhook",
    string: "Are you sure you want to delete this webhook?"
  },
  o5KXAN,
  X90ElB,
  snUby7,
  Ryh3iR,
  OPtrMg,
  QEvH8Q,
  fHopox,
  TjbB4Y,
  GLewww,
  "9Yhddc": {
    context: "input label",
    string: "Registered events"
  },
  dQdxLT,
  NPfmdK,
  tA5HJx,
  "u9/vj9": {
    context: "webhook input label",
    string: "Target URL"
  },
  "0MetrR": {
    context: "webhook input help text",
    string: "This URL will receive webhook POST requests"
  },
  WDy0tF,
  D0KaT6,
  "W+x5ZI": {
    context: "webhooks active label",
    string: "Webhook ist aktiv"
  },
  IBCBi1,
  "1+M/52": {
    context: "section header",
    string: "Webhook Status"
  },
  wbjuR4,
  wlr0Si,
  jqnwW9,
  "a/QJBx": {
    context: "user action bar",
    string: "Aktion"
  },
  "1eCau/": {
    string: "Unnamed webhook"
  },
  OTpV1t,
  JVaz1C,
  a5msuh,
  "g/FRtd": {
    context: "table column header, allocated product quantity",
    string: "Zugewiesen"
  },
  MNZY28,
  "ge/xFX": {
    context: "table column header",
    string: "Menge"
  },
  HcQEUk,
  XWGZLL,
  Y9lv8z,
  CEavJt,
  Gkip05,
  "KSp+8B": {
    context: "product available for purchase date",
    string: "will become available on {date}"
  },
  hAcUEl
};
export {
  A0Wlg7,
  A9QSur,
  ABgQcF,
  ADTNND,
  AHK0K9,
  AHRDWt,
  AJgl5u,
  AKv2BI,
  AOI4LW,
  APcoSA,
  AQFMYU,
  AQSmqG,
  AVF5T5,
  AY7Tuz,
  AbyDC7,
  AdmPca,
  Aeq79M,
  AgY5Mv,
  AijtXU,
  AjInNW,
  AkyGP2,
  AnqH4p,
  AqHafs,
  AqXzM2,
  ArctEg,
  AuwpCm,
  Av74Fa,
  AzMSmb,
  AzshS2,
  B0PaVS,
  B2LE7A,
  B5yE8S,
  B9yrkK,
  BBIQxQ,
  BCPrmK,
  BHQrgz,
  BIqhVQ,
  BLX9dz,
  BM1JiJ,
  BNTZLv,
  BQ2NVl,
  BUKMzM,
  BXMSl4,
  BXkF8Z,
  BZ7BkQ,
  BanAhF,
  BfJGij,
  BftZHy,
  BjxQ3u,
  BkFke9,
  BooQvo,
  Bphmwe,
  BtErCZ,
  Bx367s,
  By5ZBp,
  C0JLNW,
  C1luwg,
  C4aDMy,
  C4hCsD,
  C50ahv,
  C6bb6x,
  C7I2lg,
  C7eDb9,
  C90nKP,
  C9pcQx,
  CEavJt,
  CJEIRC,
  CJpx4E,
  CLB1k9,
  CLYlsu,
  CLeDae,
  CRAfpd,
  CT5PAn,
  CWqmRU,
  CXTIq8,
  CXn88q,
  CYEnGq,
  CYZse9,
  CZmloB,
  CcEwXH,
  CdIHMu,
  ChAjJu,
  ChGI4V,
  CjSRT1,
  ClFzoD,
  Co2U4u,
  CxfKLC,
  D0KaT6,
  D2qihU,
  D3E2b5,
  D4CsYK,
  D4nzdD,
  D8nsBc,
  D9ie4n,
  DASYUF,
  DEa1T1,
  DGCzal,
  DHBlFi,
  DILs4b,
  DIrxt7,
  DJFPzq,
  DNTxWr,
  DOkfyZ,
  DP5VOH,
  DP6b8U,
  DPz5y6,
  DQJnB4,
  DRMMDs,
  DTL7sE,
  DWs4ba,
  Dgp38J,
  Dhherd,
  DnghuS,
  DovGIa,
  DzPVnj,
  E22x4H,
  E54eoT,
  E8VDeH,
  E9Jssl,
  EA7rjI,
  EHsnZX,
  EIULpW,
  EKoPNg,
  EM730i,
  ENBELI,
  ETHnjq,
  EWCUdP,
  Eau5AV,
  EbVf0Z,
  Egyh2T,
  Eh6KYS,
  ErNH3D,
  EsZH44,
  EtGDeK,
  EtkIxE,
  Ev6SEF,
  EzU7KV,
  F0AXNs,
  F3Upht,
  F4WdSO,
  F56hOz,
  F69lwk,
  F8gsds,
  FBtqtl,
  FIZvTx,
  FNAZoh,
  FNKhkx,
  FNpv6K,
  FPzzh7,
  FQc5z6,
  FRJRmi,
  FSinkL,
  FWWliu,
  Fbr4Vp,
  Fc3O3r,
  FcTTvh,
  FcVEpe,
  FemBUF,
  FjrExY,
  FkDObY,
  Fl3ORD,
  Fn3bE0,
  FpIcp9,
  FuAV5G,
  Fvvgoi,
  FwHWUm,
  Fxa6xp,
  Fz3kic,
  FzEew9,
  G1KzEx,
  G3ay2p,
  G4g5Ii,
  G7mu0y,
  GAmGog,
  GCPzKf,
  GFJabu,
  GJAX0z,
  GLX9II,
  GLewww,
  GLy2UR,
  GOdq5V,
  GQcp83,
  GUlwXU,
  GVinbz,
  GXdwyR,
  GZgjK7,
  GbBCmr,
  GbhZJ4,
  Gfbp36,
  GhcypC,
  GiDxS4,
  GiJm1v,
  GibKGn,
  GjH9uy,
  Gjb6eq,
  Gjo89T,
  Gkip05,
  Go50v2,
  GpqEl5,
  Gr1SAu,
  GsBRWL,
  Gxm7Qx,
  GzbSQk,
  Gzg8hy,
  H3Uirw,
  H5NKfr,
  H5yp8O,
  H60H6L,
  H6NsC1,
  HLqWXA,
  HQR2y0,
  HR9OTW,
  HVlMK2,
  HYHLsB,
  HaQ8cg,
  HcQEUk,
  HcjV6k,
  HedXnw,
  Hgz44z,
  HivFnX,
  Hj3T7P,
  HjUoHK,
  HjXnIf,
  HlCkMT,
  HlEpii,
  HnVtSS,
  HqeqEV,
  HtQGEH,
  HvJPcU,
  I1Mz7h,
  I2wCwj,
  I7HyJZ,
  I8dAAe,
  I8mqqj,
  IBCBi1,
  IBw72y,
  IFWHn0,
  IGvQ8k,
  IN5iJz,
  INNPVX,
  IUeGzv,
  IVOjqW,
  Iafyt5,
  IbVKSH,
  Ic7Wln,
  Id7C0X,
  IeoGgH,
  IhK1F3,
  Ihp4D3,
  IoCMjg,
  IoDlcd,
  Irflxf,
  IruP2T,
  IwEQvz,
  IyHQr0,
  IzEVek,
  J0UdxG,
  J3uE0t,
  J7mFhU,
  JDz5h8,
  JE0TAx,
  JEIN47,
  JFmOfi,
  JFtFgc,
  JGm7E5,
  JHfbXR,
  JI2Xwp,
  JJgJwi,
  JKbpH9,
  JMbFNo,
  JRfJD9,
  JTcz2G,
  JUQwne,
  JVaz1C,
  JXRYQg,
  Ja7gHc,
  JbUg2b,
  JfKvrV,
  JftRtx,
  JgNb8X,
  JgXBAw,
  JiRKgJ,
  JiXNEV,
  Jj0de8,
  JjeZEG,
  JnzDrI,
  Jo01VZ,
  JqiqNj,
  JsPIOX,
  Jt3DwJ,
  JtZ71e,
  JufWFT,
  Jwuu4X,
  JyQEHU,
  JyQoES,
  K8xNLe,
  KCjd1o,
  KGsaGO,
  KHZlmi,
  KIh25E,
  KKQUMK,
  KN7zKn,
  KQSONM,
  KRqgfo,
  KTAg0f,
  KcsJKm,
  KeO51o,
  Kg0Fiu,
  Kgxlsf,
  KmPicj,
  KqbfFa,
  KszPFx,
  KupNHw,
  Kw0jHS,
  Kxiige,
  Kyyr7D,
  L5io1l,
  L5rthO,
  L87bp7,
  LA13a5,
  LATpSE,
  LEZZkK,
  LICZeR,
  LKoIB1,
  LKpQYh,
  LOSNq0,
  LR3HlT,
  LVa5ew,
  LWmYSU,
  LkuDEb,
  LmKz3g,
  LnRlch,
  LsgHmZ,
  LshEVn,
  LtqrM8,
  Lx1ima,
  M1uijW,
  M4q0Ye,
  M8z5WZ,
  M9LXb5,
  MAsLIT,
  MHVglr,
  MIC9W7,
  MJBAqv,
  MKtgZB,
  MNZY28,
  MPOj0I,
  MPfyne,
  MQwT1W,
  MSItJD,
  MTl5o6,
  MVU6ol,
  MWSacl,
  MXZuVP,
  MbZHXE,
  Mee46w,
  MewrtN,
  MgdgpT,
  MjUyhA,
  MmGlkp,
  MpR4zK,
  MxhVZv,
  MyM2oR,
  Myx1Qp,
  N2SbNc,
  N3Zot1,
  N5UuEK,
  N76zUg,
  NBP8uu,
  NDm2Fe,
  NEJo1I,
  NGc9kE,
  NJEe12,
  NJbzcP,
  NLNonj,
  NLybdq,
  NNT3Lp,
  NPfmdK,
  NUevU9,
  NWxomz,
  NXpFlL,
  NZtcLb,
  NcY4ph,
  Nfh9QM,
  NhQboB,
  NivJal,
  NlSJMM,
  Np7J92,
  NqxvFh,
  NsgWhZ,
  NskBjH,
  NtFVFS,
  NxRsHQ,
  NxeDbG,
  NzifUg,
  O22NIZ,
  O95R3Z,
  O9QPe1,
  OGemtu,
  OGm8wO,
  OPtrMg,
  OTek3r,
  OTpV1t,
  OVOU1z,
  OaA0f9,
  OaKyz4,
  ObRk1O,
  Oe62bR,
  Of19Pn,
  OgFBAj,
  OhFGpX,
  OhdPS1,
  OrR3Qy,
  Orgqv4,
  OzHN0Z,
  P5twxk,
  P79U4b,
  P7PLVj,
  PAqicb,
  PCoO4D,
  PFXGaR,
  PHUcrU,
  PJDcQs,
  PKJqcq,
  PRXpBm,
  PRlD0A,
  PTW56s,
  PV0SQd,
  PX2zWy,
  PXatmC,
  PXx4Jk,
  PajjqE,
  PbqNhi,
  PcPMjC,
  PcQRxi,
  PctLol,
  PiSXjb,
  PilTI6,
  PkCmGU,
  PkjQS6,
  PqMbma,
  PsclSa,
  Pyjarj,
  PzXIXh,
  Q0JJ4F,
  Q1HhPk,
  Q1Uzbb,
  Q2UXlW,
  Q47ovw,
  Q55cTG,
  Q6VRrE,
  Q6wcZ5,
  Q7uuDr,
  Q8Qw5B,
  Q8mpW3,
  Q8wHwJ,
  Q9wTrz,
  QBxN6z,
  QCwBUI,
  QEvH8Q,
  QFCUEt,
  QGjJcT,
  QH74y5,
  QHB48n,
  QLVddq,
  QM9P8G,
  QSnh4Y,
  QUyUJy,
  QVNg8A,
  QY7FSs,
  QZoU0r,
  QcIFCs,
  QclvqG,
  QdGzUf,
  Qe4XHv,
  QiN4hv,
  QirE3M,
  QkFeOa,
  Qovenh,
  Qph0GE,
  QzseV7,
  R98JLZ,
  RBxYJf,
  REVk27,
  RJ5QxE,
  RLBLPQ,
  RLTaAR,
  RLYfMF,
  RLZ1jd,
  RMB6fU,
  RQUkVW,
  RUzdUH,
  RXbkle,
  RZ32u5,
  RZmdM3,
  RaycYK,
  Rd0s80,
  RfPJ1E,
  Rj8LxK,
  Rjs1CD,
  RlbhwF,
  RlfqSV,
  RoKOQJ,
  RrCui3,
  Rsknyh,
  Ryh3iR,
  RzDYi8,
  RzsKm8,
  S22jIs,
  S52JMl,
  S5PVx1,
  SHm7ee,
  SMakqb,
  SNiyXb,
  SO56cv,
  SPBLzT,
  SSWFo8,
  STJx8N,
  STp3Hl,
  SUbxSK,
  SZH0fw,
  SZJhvK,
  SZt9kC,
  Sd0Ppm,
  Sjd7wm,
  SpngiS,
  SqcGBK,
  SqhC7g,
  SwISVH,
  Sx7QVu,
  T0Mfxq,
  T1f2Yl,
  T4GOiX,
  T4wa2Y,
  T5nU7u,
  T6dXGG,
  T8rvXs,
  TBaMo2,
  TCR639,
  TDXskW,
  TDhHMi,
  TE4fIS,
  TGX4T1,
  TLNf6K,
  TLYeo5,
  TOMgXz,
  TPCRKr,
  TPW2tP,
  TSJRiZ,
  TWVx7O,
  Ta9j04,
  TalJlD,
  TjGYna,
  TjbB4Y,
  TnyLrZ,
  TpPx7V,
  U1eJIw,
  U2WgwW,
  U2mOqA,
  U3BQKA,
  U5aVd8,
  U8eeLW,
  U9CIo7,
  U9o2bV,
  UCHtG6,
  UJnqdm,
  UKgP89,
  UN3qWD,
  UQ4Kuh,
  UQu75k,
  US3IPU,
  USO8PB,
  USS3Q7,
  UW1fLs,
  UaYJJ8,
  Uf3oHA,
  Uh9R9m,
  UhcALJ,
  Uj2hj8,
  UjoSZB,
  UjsI4o,
  Um3g00,
  Uo5MoU,
  Urh2N3,
  Utq1fE,
  Uu2B2G,
  Uu76vj,
  UxdBmI,
  UycVMp,
  UymotP,
  V1mqpZ,
  V2BBQu,
  V3fvcD,
  V76IV7,
  V8FhTt,
  VCzrEZ,
  VEE4gD,
  VHuzgq,
  VKWPBf,
  VKb1MS,
  VOiUXQ,
  VQLIXd,
  VSztEE,
  VTITVe,
  VZsE96,
  VceXrc,
  VdDcxc,
  Vdy5g7,
  VmMDLN,
  VrFy8e,
  VsGcdP,
  Vu9nol,
  VvA7ai,
  VyzsWZ,
  W32xfN,
  W3JbSQ,
  W5SK5c,
  W8i2Ez,
  WDy0tF,
  WE8IFE,
  WFG7Zk,
  WMGoqz,
  WQMTKI,
  WQnltU,
  WR8rir,
  WRPQMM,
  WRkCFt,
  WS4ov0,
  WTj17Z,
  WUf3Iu,
  WWV8aZ,
  WasHjQ,
  WbV1Xm,
  Wd8vG7,
  WhKGPA,
  WhvuCb,
  Wlc67M,
  WnlZMO,
  WwNtFn,
  WwZfNK,
  WyPitj,
  WzA5Ll,
  WzHfj8,
  X26jCC,
  X6PF8z,
  X7fqfM,
  X8qjg3,
  X90ElB,
  X90t9n,
  XAPFHP,
  XB2Jj9,
  XBfvKN,
  XBwpUv,
  XCIV8H,
  XDeh5D,
  XFtKV5,
  XGBsoK,
  XJSRDK,
  XLYssG,
  XMrYaA,
  XNeJay,
  XQBVEJ,
  XRf1Bi,
  XUU9sU,
  XWGZLL,
  XYhE8p,
  XZR590,
  Xb6BJ9,
  XkX56I,
  XlPKAR,
  Xm9qOu,
  Xr5zxu,
  Xtd0AT,
  XwQQ1f,
  XwrKOi,
  Y1B0PN,
  Y299ST,
  Y4cy0i,
  Y7M1YQ,
  Y7UlMR,
  Y7Vy19,
  Y9lv8z,
  YBqLHs,
  YEpYMB,
  YFDAaX,
  YFQBs1,
  YHNozE,
  YI6Fhj,
  YJ4TXc,
  YKyNm9,
  YMBn8d,
  YQ3EXR,
  YVIajc,
  YYkkhx,
  YZl6cv,
  YdeHZX,
  YgE6ga,
  YicEbK,
  YjcN9w,
  Yjhgle,
  YkOzse,
  Ykw8k5,
  YpLVVc,
  YpukUN,
  Yxihwg,
  Z6QAbw,
  ZDJEat,
  ZFfG4L,
  ZHF4Z9,
  ZIc5lM,
  ZMy18J,
  ZPOyI1,
  ZRz3hM,
  ZWIjvr,
  ZayvsI,
  Zd3Eew,
  ZeD2TK,
  Zg0dRo,
  ZhDQel,
  ZhaXLU,
  Zhxu58,
  ZprV2g,
  Zptsep,
  ZuqkSp,
  a1uffz,
  a55zOn,
  a5msuh,
  a9S9Je,
  aAAxKp,
  aCJwVq,
  aCX8rl,
  aDbrOK,
  aEc9Ar,
  aFLtLk,
  aHc89n,
  aI80kg,
  aMwxYb,
  aOelhW,
  aPCrsp,
  aPYFO1,
  aPqizA,
  aY0HAT,
  aYhcie,
  aZDHYr,
  abTH5q,
  agZQkB,
  aggaJg,
  aheQdn,
  akXDST,
  amQg6f,
  anK7jD,
  aq5ZiN,
  asdvmK,
  auxEP1,
  axFFaD,
  b1zuN9,
  b6L9n7,
  b810WJ,
  bDHiYK,
  bDO9DN,
  bFhzRX,
  bHdFph,
  bNw8PM,
  bP7ZLP,
  bPFp8B,
  bS7A8u,
  bZenNC,
  bZksto,
  bb4nSp,
  bcf60I,
  beuxAP,
  biAxKR,
  biVFKU,
  bj1U23,
  bj6pTd,
  bk9KUX,
  bq1eEx,
  bqAJCT,
  bsP4f3,
  bwJc6V,
  byP6IC,
  c0H45L,
  c4gbXr,
  c5pMZ8,
  c8UT0c,
  c8nvms,
  cBHRxx,
  cFVgOo,
  cJ5ASN,
  cKCfSW,
  cKjFfl,
  cMFlOp,
  cNSLLO,
  cW1RIo,
  cY42ht,
  cY6H2C,
  caMMWN,
  caqRmN,
  ccXLVi,
  cd13nN,
  cdiRSf,
  cdxwA8,
  cehiWu,
  cfQf0w,
  chvryR,
  cpZLRH,
  cqZ5UH,
  cryH86,
  cvbqJu,
  cwoN25,
  cy8sV7,
  d9UqaJ,
  dEUZg2,
  dGqEJ9,
  dJQxHt,
  dJVXIb,
  dKPMyh,
  dM86a2,
  dOQB9o,
  dPYqy0,
  dQdxLT,
  dS8Adx,
  dTCDMn,
  dTCWqt,
  dTkmON,
  dc5KWn,
  de as default,
  diOQm7,
  dpY94C,
  e7Nyu7,
  e7yOai,
  e822us,
  e92Uxp,
  eCRaHe,
  eLJQSh,
  eOrLzG,
  eUjFjW,
  eWV760,
  eWcvOc,
  ekXood,
  erC44f,
  esg2wu,
  eu98dw,
  ey0lZj,
  f0hXz4,
  f2F1NJ,
  f3B4tc,
  fEfCtO,
  fHopox,
  fLhj3a,
  fNFEkh,
  fP9FXB,
  fV6yX5,
  fbH51z,
  fdbqIs,
  fehqPs,
  fgHLXc,
  fhksPD,
  fjPWOA,
  fkplbE,
  flP8Hj,
  fo7nfa,
  foB6wx,
  foNlhn,
  ftcHpD,
  fyE8BN,
  fzDI3A,
  fzk04H,
  fzl482,
  fzpXvv,
  g3qjSf,
  g5zIpS,
  g6yuk2,
  gAqkrG,
  gF7hbK,
  gKdGxP,
  gMwpNC,
  gQGUsN,
  gSQ0Ge,
  gTr0qE,
  gVD1os,
  gZHmaV,
  ghGLbJ,
  ghje1I,
  giF5UV,
  gj3MUg,
  glT6fm,
  gr53VQ,
  gtKcPf,
  gvOzOl,
  gx4wCT,
  gxPjIQ,
  gz9v22,
  h1rPPg,
  h2Hta6,
  h75GAF,
  hAcUEl,
  hHOI7D,
  hJZwTS,
  hLOEeb,
  hLUYBt,
  hMRP6J,
  hOxIeP,
  hPB89Y,
  hWO1SD,
  hX5PAb,
  hdcGSJ,
  hkENym,
  hkSkNx,
  hnBvH7,
  ho75Lr,
  hpMcW8,
  hptDxW,
  hqVMLQ,
  ht9yOD,
  hw9Fah,
  hzSNj4,
  i0AcKY,
  i3Mvj8,
  i56GGQ,
  iAvKNf,
  iBSq6l,
  iEeIhY,
  iJrw63,
  iMJka8,
  iPk640,
  iQxjow,
  iUIn50,
  iUy2dx,
  iWyoZn,
  iYH3Y7,
  ibnmEd,
  iigydN,
  ij7olm,
  ikM00B,
  ikRuLs,
  imYxM9,
  isM94c,
  iue6mV,
  ivJ1qt,
  ixjvkM,
  j0ugM4,
  j2fPVo,
  j2l6cL,
  j3yE7I,
  j8PV7E,
  jABdx1,
  jBu2yj,
  jDIRQV,
  jDovoJ,
  jFrdB5,
  jGGnSZ,
  jHJmjf,
  jNSOSu,
  jU9GPX,
  jUuHVn,
  jWna9Q,
  jZbT0O,
  jj3Cb8,
  jmh0rV,
  jqJqdE,
  jqnwW9,
  jswILH,
  ju8zHP,
  jvKNMP,
  jxoMLL,
  jyTwDR,
  jyaAlB,
  k0rGBI,
  k4brJy,
  k5lHFp,
  k6WDZl,
  k8ZJ5L,
  k8bltk,
  k9hf7F,
  kErneR,
  kFQvXv,
  kFYlu2,
  kFkMoG,
  kIvvax,
  kJQczl,
  kN6SLs,
  kPIZ65,
  kS5Qgk,
  kTr2o8,
  kVOslW,
  kVTWtR,
  kak5vT,
  kbkjEc,
  kc4c4d,
  kcMVsB,
  ki7Mr8,
  kp2IYP,
  kvSYZh,
  l2oVCF,
  l5V0QT,
  l9ETHu,
  l9Lwjh,
  lCPxtT,
  lCxfDe,
  lGlDEH,
  lHccsy,
  lJP1iw,
  lL1HTg,
  lL3YJO,
  lLwtgs,
  lQRnYK,
  lT5MYM,
  lVZ5n7,
  lVwmf5,
  lW5uJO,
  la9cZ4,
  labkPK,
  lct0qd,
  lehGc9,
  li1BBk,
  liLrVs,
  ll2dE6,
  llC1q8,
  lm9NSK,
  lnQAos,
  lra7Ej,
  lrq8O6,
  lw9WIk,
  lzdvwp,
  m19JfL,
  m6IBe5,
  m8cjcK,
  mAabef,
  mCP0UD,
  mDgOmP,
  mGiA6q,
  mIUNgR,
  mLZMb6,
  mQtoRO,
  mSLr9d,
  mUb8Gt,
  mWQt3s,
  mX7zJJ,
  mYs3tb,
  mbOuGl,
  mbj1J5,
  md326v,
  me585h,
  mm0CXe,
  mmcHeH,
  mr9jbO,
  mslSpp,
  mxtAFx,
  myyWNp,
  mzASqs,
  n02c9W,
  n0RwMK,
  n5vskv,
  n7Fg8i,
  n7GIKB,
  n97Ii0,
  n9JOI3,
  nEixpu,
  nFTvQW,
  nHmugP,
  nIrjSR,
  nKjLjT,
  nKwgxY,
  nLML8Y,
  nNeWAx,
  nOo0oL,
  nTF6tG,
  nX2pCU,
  nayZY0,
  nf3XSt,
  nfbabo,
  njBulj,
  njUQPz,
  nngeI3,
  no3Ygn,
  nudPsY,
  nvSJNR,
  nwnwJ0,
  nwvQPg,
  ny4zrH,
  o5KXAN,
  o6260f,
  o8S0Ac,
  o8f4Sg,
  oB0y5Y,
  oChkS4,
  oHbgcK,
  oIMMcO,
  oIvtua,
  oJkeS6,
  oLMXDv,
  oOFrUd,
  oQ27V4,
  oQY0a2,
  oQhFlK,
  oUWADl,
  oUWXLO,
  oVDZUb,
  oYGfnY,
  oiuwOl,
  ojHyj3,
  opIDox,
  osPBn1,
  p0SUJj,
  p3eRUm,
  p4zuQp,
  pA8Mlv,
  pBTTtU,
  pCy5EP,
  pGDYG5,
  pNrF72,
  pRYGUR,
  pTpx0p,
  pURrk1,
  pVFoOk,
  paa4m0,
  pbGIUg,
  phAZoj,
  pkUbrL,
  pkjXPD,
  ppLwx3,
  pr513b,
  ps0WUQ,
  ptPPVk,
  puALFo,
  puikeb,
  pwqwcy,
  q8ep2I,
  qCH2eZ,
  qDwvZ4,
  qEJT8e,
  qEZ463,
  qIgdO6,
  qJedl0,
  qLOBff,
  qLbse5,
  qMB6d2,
  qNcoRY,
  qT6YYk,
  qbmeUI,
  qbqMpk,
  qddy2Z,
  qkRuT0,
  qov29K,
  qpQ0uB,
  qrWOxx,
  qu8b3v,
  quV5zH,
  r0hgpM,
  r1aQ2f,
  r2dojI,
  r86alc,
  rCy3Fe,
  rHXF43,
  rHoRbE,
  rIJbNC,
  rJ3lkW,
  rKf4LU,
  rNzkIt,
  rPX1f2,
  rQOS7K,
  rVIlBs,
  rVaB7c,
  rZMT44,
  rZf1qL,
  ra2O4j,
  rbkmfG,
  reP5Uf,
  rfvBAF,
  ri3kK9,
  rpFdD1,
  rqiCWU,
  rqtV5d,
  rr8fyf,
  rrbzZt,
  rs815i,
  rvk9ls,
  rwOx2s,
  ryAyPr,
  rzhcpD,
  s17U7u,
  s1IQuN,
  s2y5eG,
  s40PZt,
  s51tHd,
  s5Imt5,
  s5v6m0,
  s6lW8R,
  s8FlDW,
  s97tLq,
  s9sOcC,
  sEjRyz,
  sG0w22,
  sHON47,
  sK1FPw,
  sR0urA,
  sS5aVm,
  sW9IyX,
  sZ27WU,
  saKXY3,
  scHVdW,
  sdA14A,
  sedoZ3,
  sf6FMK,
  sfEbeB,
  sidKce,
  sjRXXz,
  skPoVe,
  slKV5G,
  sn2awN,
  snUby7,
  sw8Wl2,
  szXISP,
  t3aiWF,
  t7UwLY,
  t9a9GQ,
  tA5HJx,
  tEdFmZ,
  tKxCld,
  tQuE1q,
  tQxBXs,
  tTtoKd,
  tTuCYj,
  tVybuM,
  tWbE34,
  tXIgmR,
  toDL5R,
  tsL3IW,
  ttMauu,
  ttk0w7,
  tuYPlG,
  tvpAXl,
  txOXvy,
  tzMNF3,
  u0S2be,
  u0V06N,
  u24Ppd,
  u3sYPH,
  uD93er,
  uIPD1i,
  uMpv1v,
  uQk8gB,
  uULcph,
  uUsZ7m,
  uVssds,
  ubasgL,
  ucYPtV,
  ud0w8h,
  ufD5Jr,
  ufmuTp,
  ugnggZ,
  uilt7q,
  ujFo4A,
  ukdRUv,
  ulh3kf,
  umsU70,
  usSkzP,
  utaSh3,
  uxPpRx,
  uxZZXx,
  uzAPCt,
  v17Lly,
  v1pNHW,
  vC8vyb,
  vCw7BP,
  vDnheO,
  vEYtiq,
  vM9quW,
  vN3qdA,
  vQunFc,
  vTN5DZ,
  vTgRTZ,
  vVviA2,
  vW3tb6,
  vWTM6Y,
  vWapBZ,
  vXA5xU,
  vXFPD6,
  vY2lpx,
  vZMs8f,
  vcwrgW,
  vf56In,
  viFkCw,
  vjsfyn,
  vkAWwY,
  vlLyvk,
  vlVTmY,
  voRaz3,
  vof5TR,
  vuKrlW,
  vwMO04,
  vy7fjd,
  vzce9B,
  vzgZ3U,
  w2Cewo,
  w2eTzO,
  w3sGrD,
  w424P4,
  w6Gau0,
  w9xgN9,
  wDUBLR,
  wHdMAX,
  wL7VAE,
  wOeIR4,
  wQdR8M,
  wTHjt3,
  wWTUrM,
  wWYYBR,
  wbjuR4,
  wgA48T,
  whTEcF,
  wjKYSU,
  wlQTfb,
  wlr0Si,
  wmdHhD,
  wn3di2,
  wpAXKX,
  wpli4O,
  wxFwUW,
  wyvzh9,
  x3g4Ry,
  x3leH4,
  xAHOGV,
  xB7BTp,
  xHj9Qe,
  xJQX5t,
  xOEZjV,
  xPnZ0R,
  xQK2EC,
  xRbqcg,
  xRkj2h,
  xUvWaP,
  xVn5B0,
  xWEFrR,
  xZhxBJ,
  xco5tZ,
  xfGZsi,
  xjpTLF,
  xkjRu5,
  xl7Fag,
  xo5UIb,
  xrPv2K,
  xtUXnK,
  xwEc8K,
  xxQxLE,
  y1Z3or,
  y7mfbl,
  y9cvqE,
  yDkmX7,
  yEmwxD,
  yHQQMQ,
  yHaQWG,
  yHeZRQ,
  yHwvLL,
  yJynYK,
  yKuba7,
  yMi8I8,
  yOaNWB,
  ySqrUU,
  yT5zvU,
  yatGsm,
  ybaLoZ,
  ycMLN9,
  ychKsb,
  ycrTBX,
  yhv3HX,
  ylobu9,
  ypW4Mi,
  yuiyES,
  z1puMb,
  z1y9oL,
  z8jo8h,
  zCb8fX,
  zDvDnG,
  zKOGkU,
  zLtb4N,
  zQX6xO,
  zQnYKn,
  zR9Ozi,
  zRrcOG,
  zSOvI0,
  zTdwWM,
  zWM89r,
  zWgbGg,
  zZCCqz,
  zb4eBO,
  zbJHl7,
  zjHH6b,
  zjZuhM,
  zjkAMs,
  zqarUF,
  ztQgD8,
  zxs6G3,
  zyXcBP,
  zyceue,
  zzfj8H
};
//# sourceMappingURL=de.f3406990.js.map
